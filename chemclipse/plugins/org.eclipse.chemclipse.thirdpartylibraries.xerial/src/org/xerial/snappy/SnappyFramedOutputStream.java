 ...
 * >x-snappy-framed</a> as an {@link OutputStream} and {@link WritableByteChannel}.
 * 
 * @author Brett Okken
 * @since 1.1.0
 */
public final class SnappyFramedOutputStream extends OutputStream implements WritableByteChannel {
	...
	/**
	 * Creates a new {@link SnappyFramedOutputStream} using the {@link #DEFAULT_BLOCK_SIZE} and {@link #DEFAULT_MIN_COMPRESSION_RATIO}.
	 * 
	 * @param out
	 ...
	 *            Defines the minimum compression ratio ( {@code compressedLength / rawLength}) that must be achieved to
	...
	public SnappyFramedOutputStream(WritableByteChannel out, int blockSize, double minCompressionRatio) throws IOException {
		...
		outputBuffer = ByteBuffer.allocateDirect(Snappy.maxCompressedLength(blockSize));
		writeHeader(out);
	}
	...
	public void write(byte[] input, int offset, int length) throws IOException {
		...
		if(input == null) {
			throw new NullPointerException();
		} else if((offset < 0) || (offset > input.length) || (length < 0) || ((offset + length) > input.length) || ((offset + length) < 0)) {
	...
	public long transferFrom(InputStream is) throws IOException {
		...
		int read;
		while((read = is.read(bytes, arrayOffset + buffer.position(), buffer.remaining())) != -1) {
	...
	/**
	 * Transfers all the content from <i>rbc</i> to this {@link WritableByteChannel}. This potentially limits the amount of
	 ...
	 * the data, determines if the compression ratio is acceptable and calls {@link #writeBlock(java.nio.channels.WritableByteChannel, java.nio.ByteBuffer, boolean, int)} to
	 ...
	 * @throws IOException
	 */
	private void writeBlock(final WritableByteChannel out, ByteBuffer data, boolean compressed, int crc32c) throws IOException {

		headerBuffer.clear();
		headerBuffer.put((byte)(compressed ? COMPRESSED_DATA_FLAG : UNCOMPRESSED_DATA_FLAG));
