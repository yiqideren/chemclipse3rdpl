 ...
 * >x-snappy-framed</a> as an {@link InputStream} and {@link ReadableByteChannel}.
 * 
 * @author Brett Okken
 * @since 1.1.0
 */
	...
	private boolean ensureBuffer() throws IOException {
		...
		if(FrameAction.UNCOMPRESS == frameMetaData.frameAction) {
			...
			if(uncompressedLength > uncompressedDirect.capacity()) {
				uncompressedDirect = ByteBuffer.allocateDirect(uncompressedLength);
		...
		if(verifyChecksums) {
			final int actualCrc32c = SnappyFramed.maskedCrc32c(buffer, position, valid - position);
