...
public class ChartLayout extends Layout {
	...
	private void adjustForRotatedTickLabels(Rectangle r) {
		...
		for(IAxis axis : horizontalAxes) {
			...
			int height = Axis.MARGIN + (int)(tickLabelMaxLength * Math.sin(Math.toRadians(angle)) + Util.getExtentInGC(layoutData.axisTickLabels.getFont(), "dummy").y * Math.cos(Math.toRadians(angle)));
