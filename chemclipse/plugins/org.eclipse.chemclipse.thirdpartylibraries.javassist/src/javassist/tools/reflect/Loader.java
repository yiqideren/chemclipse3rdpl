 ...
 *
 * <p>
 * <code>javassist.tools.reflect.Loader</code> does not make a class reflective if that class is in a <code>java.*</code> or <code>javax.*</code> pacakge because of the specifications on the class loading algorithm of Java. The JVM does not allow to load such a system class with a user class loader.
 *
 * <p>
 * To avoid this limitation, those classes should be statically modified with <code>javassist.tools.reflect.Compiler</code> and the original class files should be replaced.
 *
...
public class Loader extends javassist.Loader {
	...
	/**
	 * Loads a class with an instance of <code>Loader</code> and calls <code>main()</code> in that class.
	 *
	 * @param args
	 *            command line parameters. <br>
	 *            &nbsp;&nbsp;<code>args[0]</code> is the class name to be loaded. <br>
	 *            &nbsp;&nbsp;<code>args[1..n]</code> are parameters passed
	 ...
	 *            It must be a subclass of <code>Metaobject</code>.
	 * @param metaclass
	 *            the class of the class metaobject.
	 *            It must be a subclass of <code>ClassMetaobject</code>.
