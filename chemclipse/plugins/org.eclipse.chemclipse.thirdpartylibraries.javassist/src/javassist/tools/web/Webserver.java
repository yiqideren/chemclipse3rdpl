 ...
 *
 * <p>
 * <b>Note:</b> although this class is included in the Javassist API, it is provided as a sample implementation of the web server using Javassist. Especially, there might be security flaws in this server. Please use this with YOUR OWN RISK.
 */
public class Webserver {
	...
	/**
	 * If this field is not null, the class files taken from <code>ClassPool</code> are written out under the directory
	 ...
	 *
	 * <p>
	 * If the given URL indicates a class file and the class file is not found under the directory specified by this variable, then <code>Class.getResourceAsStream()</code> is called for searching the Java class paths.
	 */
	...
	public void doReply(InputStream in, OutputStream out, String cmd) throws IOException, BadHttpRequest {
		...
		len = filename.length();
		if(fileType == typeClass && letUsersSendClassfile(out, filename, len))
			return;
