...
public class StubGenerator implements Translator {
	...
	private CtClass produceProxyClass(CtClass orgclass, Class orgRtClass) throws CannotCompileException, NotFoundException {
		...
		CtField f = new CtField(classPool.get("javassist.tools.rmi.ObjectImporter"), fieldImporter, proxy);
		...
		CtConstructor cons = CtNewConstructor.skeleton(proxyConstructorParamTypes, null, proxy);
			...
			if(m.getDeclaringClass() != Object.class && !Modifier.isFinal(mod))
				if(Modifier.isPublic(mod)) {
					...
					if(Modifier.isStatic(mod))
						...
						body = forwardMethod;
					wmethod = CtNewMethod.wrapped(toCtClass(m.getReturnType()), m.getName(), toCtClass(m.getParameterTypes()), exceptionForProxy, body, ConstParameter.integer(i), proxy);
