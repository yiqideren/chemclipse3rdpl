 ...
 *
 * <p>
 * To access the remote object, the applet first calls <code>lookupObject()</code> and obtains a proxy object, which is a reference to that object. The class name of the proxy object is identical to that of the remote object. The proxy object provides the same set of methods as the remote object. If one of the methods is invoked on the proxy object, the invocation is delegated to the remote object. From the viewpoint of the applet, therefore, the two objects are identical. The applet can access
 * the object on the server with the regular Java syntax without concern about the actual location.
 *
 * <p>
 * The methods remotely called by the applet must be <code>public</code>. This is true even if the applet's class and the remote object's classs belong to the same package.
 *
 * <p>
 ...
 *
 * <p>
 * The parameters to a remote method is passed in the <i>call-by-value</i> manner. Thus all the parameter classes must implement <code>java.io.Serializable</code>. However, if the parameter is the proxy object, the reference to the remote object instead of a copy of the object is passed to the method.
 *
 * <p>
 * Because of the limitations of the current implementation,
 * <ul>
 * <li>The parameter objects cannot contain the proxy object as a field value.
 * <li>If class <code>C</code> is of the remote object, then the applet cannot instantiate <code>C</code> locally or remotely.
 * </ul>
 *
 * <p>
 * All the exceptions thrown by the remote object are converted into <code>RemoteException</code>. Since this exception is a subclass of <code>RuntimeException</code>, the caller method does not need to catch the exception. However, good programs should catch the <code>RuntimeException</code>.
 *
...
public class ObjectImporter implements java.io.Serializable {
	 ...
	 *
	 * <p>
	 * If you run a program with <code>javassist.tools.web.Viewer</code>, you can construct an object importer as follows:
	 *
	 * <pre>
	 * 
	 * 
