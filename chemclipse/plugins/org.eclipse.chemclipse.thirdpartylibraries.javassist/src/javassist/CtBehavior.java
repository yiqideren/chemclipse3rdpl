 ...
 * It is the abstract super class of <code>CtMethod</code> and <code>CtConstructor</code>.
 *
 * <p>
...
public abstract class CtBehavior extends CtMember {
	 ...
	 *
	 * <p>
	 * If you modify the bytecode through the returned <code>MethodInfo</code> object, you might have to explicitly rebuild a stack map table. Javassist does not automatically rebuild it for avoiding unnecessary rebuilding.
	 *
	 ...
	 * Normal applications do not need calling this method. Use <code>getMethodInfo()</code>.
	 *
	 * <p>
	 * The <code>MethodInfo</code> object obtained by this method is read only. Changes to this object might not be reflected on a class file generated by <code>toBytecode()</code>, <code>toClass()</code>, etc in <code>CtClass</code>.
	 *
	 * <p>
	 * This method is available even if the <code>CtClass</code> containing this method is frozen. However, if the class is frozen, the <code>MethodInfo</code> might be also pruned.
	 *
	 * @see #getMethodInfo()
	...
	public CtClass[] getParameterTypes() throws NotFoundException {
		...
		return Descriptor.getParameterTypes(methodInfo.getDescriptor(), declaringClass.getClassPool());
	}
	...
	CtClass getReturnType0() throws NotFoundException {
		...
		return Descriptor.getReturnType(methodInfo.getDescriptor(), declaringClass.getClassPool());
	}
	 ...
	 *
	 * @param src
	 *            the source code representing the body.
	...
	static void setBody0(CtClass srcClass, MethodInfo srcInfo, CtClass destClass, MethodInfo destInfo, ClassMap map) throws CannotCompileException {
		...
		destInfo.setAccessFlags(destInfo.getAccessFlags() & ~AccessFlag.ABSTRACT);
	 ...
	 *
	 * <p>
	 * Note that an attribute is a data block specified by the class file format. It is not an annotation. See {@link javassist.bytecode.AttributeInfo}.
	 *
	 * @param name
	 *            attribute name
	 */
	 ...
	 * with Javassist requires a support class <code>javassist.runtime.Cflow</code> at runtime
	 * (other Javassist classes are not required at runtime).
	 *
	 * <p>
	 * Every <code>$cflow</code> variable is given a unique name. For example, if the given name is <code>"Point.paint"</code>, then the variable is indicated by <code>$cflow(Point.paint)</code>.
	 *
	 * @param name
	 *            <code>$cflow</code> name. It can include
	 *            alphabets, numbers, <code>_</code>, <code>$</code>, and <code>.</code> (dot).
	 *
	 ...
	 *
	 * <p>
	 * If the second parameter <code>asFinally</code> to <code>insertAfter()</code> is true, the declared local variable is not visible from the code inserted by <code>insertAfter()</code>.
	 *
	 * @param name
	 ...
	 *
	 * <p>
	 * While executing this method, only <code>replace()</code> in <code>Expr</code> is available for bytecode modification. Other methods such as <code>insertBefore()</code> may collapse the bytecode because the <code>ExprEditor</code> loses its current position.
	 *
	 * @param editor
	 *            specifies how to modify.
	 ...
	 * <code>insertBeforeBody()</code> in <code>CtConstructor</code>.
	 *
	 * @param src
	 ...
	 *            caught exception, for example, <code>$e</code>.
	 */
	 ...
	 *
	 * @param lineNum
	 *            the line number. The bytecode is inserted at the
	 ...
	 *            line number.
	 * @param modify
	 *            if false, this method does not insert the bytecode.
