...
public abstract class CtClass {
	 ...
	 * specified by this variable. For example, if the value is <code>"./debug"</code>, then all class files are saved
	 ...
	 * Normal applications do not need calling this method. Use <code>getClassFile()</code>.
	 *
	 * <p>
	 * The <code>ClassFile</code> object obtained by this method is read only. Changes to this object might not be reflected on a class file generated by <code>toBytecode()</code>, <code>toClass()</code>, etc.
	 *
	 * <p>
	 * This method is available even if <code>isFrozen()</code> is true. However, if the class is frozen, it might be also pruned.
	 *
	 ...
	 *
	 * <p>
	 * If <code>defrost()</code> will be called later, pruning must be disallowed in advance.
	 *
	 * @see #isFrozen()
	...
	/**
	 * Returns <code>true</code> if this class extends or implements <code>clazz</code>. It also returns <code>true</code> if
	 ...
	 * cc.addField(f); CtMethod m = CtNewMethod.make(&quot;public Object get(){return value;}&quot;, cc); MethodSignature ms = new MethodSignature(null, null, tvar, null); m.setGenericSignature(ms.encode()); // ()TT; cc.addMethod(m); CtMethod m2 = CtNewMethod.make(&quot;public void set(Object v){value = v;}&quot;, cc); MethodSignature ms2 = new MethodSignature(null, new Type[]{tvar}, new BaseType(&quot;void&quot;), null); m2.setGenericSignature(ms2.encode()); // (TT;)V; cc.addMethod(m2);
	 * cc.writeFile();
	 * 
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * but it includes generic signatures for the class, the field, and the methods so that the type variable <code>T</code> can be accessible through reflection.
	 *
	 * @param sig
	 *            a generic signature.
	 ...
	 *
	 * <p>
	 * All the class names appearing in the class file are tested with <code>map</code> to determine whether each class name is replaced or not. Thus this method can be used for collecting all the class names in the class file. To do that, first define a subclass of <code>ClassMap</code> so that <code>get()</code> records all the given parameters. Then, make an instance of that subclass as an empty hash-table. Finally, pass that instance to this method. After this method finishes, that instance
	 * would contain all the class names appearing in the class file.
	 *
	 * @param map
	 ...
	 *
	 * <p>
	 * If the class is a static nested class (a.k.a. static inner class), the returned modifiers include <code>Modifier.STATIC</code>.
	 *
	 * @see Modifier
	 */
	 ...
	 * For example, if an annotation <code>@Author</code> is associated
	 * with this class, the returned array contains an <code>Author</code> object. The member values can be obtained by calling methods on
	 * the <code>Author</code> object.
	 *
	...
	/**
	 * Returns the annotations associated with this class.
	 * This method is equivalent to <code>getAnnotations()</code> except that, if any annotations are not on the classpath,
	 ...
	 * class.
	 * It returns null if this object represents the <code>java.lang.Object</code> class and thus it does not have
	 * the super class.
	 *
	 * <p>
	 * If this object represents an interface, this method always returns the <code>java.lang.Object</code> class. To obtain the super interfaces extended by that interface, call <code>getInterfaces()</code>.
	 */
	 ...
	 *
	 * <p>
	 * If this object represents an interface, this method is equivalent to <code>addInterface()</code>; it appends <code>clazz</code> to the list of the super interfaces extended by that interface. Note that an interface can extend multiple super interfaces.
	 *
	 ...
	 *
	 * @param list
	 *            a list of the <code>CtClass</code> objects
	 *            representing interfaces, or <code>null</code> if the class implements
	 *            no interfaces.
	 */
	 ...
	 *
	 * <p>
	 * The current implementation only supports a static nested class. <code>isStatic</code> must be true.
	 *
	 * @param name
	 ...
	 *            the field name.
	 * @param desc
	 *            the type descriptor of the field. It is available by {@link CtField#getSignature()}.
	 ...
	 *
	 * <p>
	 * The <code>CtField</code> belonging to another <code>CtClass</code> cannot be directly added to this class. Only a field created for this class can be added.
	 *
	 ...
	 *
	 * <p>
	 * Here, the type of variable <code>cc</code> is <code>CtClass</code>. The type of <code>f</code> is <code>CtField</code>.
	 *
	 * <p>
	 * Note: do not change the modifier of the field (in particular, do not add or remove <code>static</code> to/from the modifier) after it is added to the class by <code>addField()</code>.
	 *
	 * @param init
	 ...
	 *
	 * <p>
	 * This code adds an <code>int</code> field named "i". The initial value of this field is 1.
	 *
	 * @param init
	 ...
	 *
	 * <p>
	 * This is a convenient method mainly for obtaining a user-defined attribute. For dealing with attributes, see the <code>javassist.bytecode</code> package. For example, the following expression returns all the attributes of a class file.
	 *
	 * 
	 ...
	 *
	 * <p>
	 * This is a convenient method mainly for adding a user-defined attribute. For dealing with attributes, see the <code>javassist.bytecode</code> package. For example, the following expression adds an attribute <code>info</code> to a class file.
	 *
	 * 
	 ...
	 * declared in the class. This method calls <code>instrument()</code> on every <code>CtMethod</code> and <code>CtConstructor</code> object
	 * in the class.
	 *
	 * @param converter
	 ...
	 *
	 * <p>
	 * <b>Warining:</b> A Class object returned by this method may not work with a security manager or a signed jar file because a protection domain is not specified.
	 *
	 ...
	 *
	 * <p>
	 * The class file represented by this <code>CtClass</code> is loaded by the given class loader to construct a <code>java.lang.Class</code> object. Since a private method on the class loader is invoked through the reflection API, the caller must have permissions to do that.
	 *
	 * <p>
	 * An easy way to obtain <code>ProtectionDomain</code> object is to call <code>getProtectionDomain()</code> in <code>java.lang.Class</code>. It returns the domain that the class belongs to.
	 *
	 * <p>
	 * This method is provided for convenience. If you need more complex functionality, you should write your own class loader.
	 *
	 ...
	 *            If it is null, the class loader returned by {@link ClassPool#getClassLoader()} is used.
	 * @param domain
	 ...
	 *
	 * <p>
	 * If <code>get()</code> in <code>ClassPool</code> is called with the name of the removed method, the <code>ClassPool</code> will read the class file again and constructs another <code>CtClass</code> object representing the same class.
	 */
	public void detach() {
	...
	/**
	 * Disallows (or allows) automatically pruning this <code>CtClass</code> object.
	 *
	 * <p>
	 * Javassist can automatically prune a <code>CtClass</code> object when <code>toBytecode()</code> (or <code>toClass()</code>, <code>writeFile()</code>) is called. Since a <code>ClassPool</code> holds all instances of <code>CtClass</code> even after <code>toBytecode()</code> (or <code>toClass()</code>, <code>writeFile()</code>) is called, pruning may significantly save memory consumption.
	 *
	 * <p>
	 * If <code>ClassPool.doPruning</code> is true, the automatic pruning is on by default. Otherwise, it is off. The default value of <code>ClassPool.doPruning</code> is false.
	 * 
	 * @param stop
	...
	/**
	 * Discards unnecessary attributes, in particular, <code>CodeAttribute</code>s (method bodies) of the class,
	 * to minimize the memory footprint.
	 * After calling this method, the class is read only.
	 * It cannot be modified any more.
	 * Furthermore, <code>toBytecode()</code>, <code>writeFile()</code>, <code>toClass()</code>,
	 * or <code>instrument()</code> cannot be called.
	 * However, the method names and signatures in the class etc.
	 * are still accessible.
	 *
	 * <p>
	 * <code>toBytecode()</code>, <code>writeFile()</code>, and <code>toClass()</code> internally call this method if automatic pruning is on.
	 *
	 * <p>
	 ...
	 * rebuilt when it is finally generated by <code>toBytecode()</code> and <code>writeFile()</code>.
	...
	/**
	 * Writes a class file represented by this <code>CtClass</code> object in the current directory.
	 * Once this method is called, further modifications are not
	 * possible any more.
	 *
	 * @see #debugWriteFile()
	...
	/**
	 * Writes a class file represented by this <code>CtClass</code> object on a local disk.
	 * Once this method is called, further modifications are not
	 * possible any more.
	 *
	 * @param directoryName
	 ...
	 * file. Note that, once <code>writeFile()</code> or <code>toBytecode()</code> is called, it cannot be called again since the class is pruned and frozen.
