...
public class Bytecode extends ByteVector implements Cloneable, Opcode {
	 ...
	 *
	 * <p>
	 * The parameters <code>stacksize</code> and <code>localvars</code> specify initial values of <code>max_stack</code> and <code>max_locals</code>. They can be changed later.
	 *
	 * @param cp
	 *            constant pool table.
	 * @param stacksize
	...
	/**
	 * Constructs a <code>Bytecode</code> object with an empty bytecode
	 * sequence. The initial values of <code>max_stack</code> and <code>max_locals</code> are zero.
	 * 
	 * @param cp
	 *            constant pool table.
	 ...
	 *
	 * <p>
	 * This value may be automatically updated when an instruction is appended. A <code>Bytecode</code> object maintains the current stack depth whenever an instruction is added by <code>addOpcode()</code>. For example, if DUP is appended, the current stack depth is increased by one. If the new stack depth is more than <code>max_stack</code>, then it is assigned to <code>max_stack</code>. However, if branch instructions are appended, the current stack depth may not be correctly maintained.
	 *
	 * @see #addOpcode(int)
	 */
	 ...
	 *
	 * <p>
	 * This computes the number of local variables used to pass method parameters and sets <code>max_locals</code> to that number plus <code>locals</code>.
	 *
	 * @param isStatic
	 ...
	 * The current stack depth is updated. <code>max_stack</code> is updated if the current stack depth
	 * is the deepest so far.
	 *
	 * <p>
	 * Note: some instructions such as INVOKEVIRTUAL does not update the current stack depth since the increment depends on the method signature. <code>growStack()</code> must be explicitly called.
	 */
	 ...
	 *
	 * <p>
	 * The specified method must not be an inherited method. It must be directly declared in the class specified in <code>clazz</code>.
	 *
	 * @param clazz
	 *            the target class.
	 * @param name
	 ...
	 *
	 * <p>
	 * The specified method must not be an inherited method. It must be directly declared in the class specified in <code>classname</code>.
	 *
	 * @param classname
	 *            the fully-qualified class name.
	 * @param name
	 ...
	 *
	 * <p>
	 * The specified method must not be an inherited method. It must be directly declared in the class specified by <code>clazz</code>.
	 *
	 * @param clazz
	 *            the index of <code>CONSTANT_Class_info</code> structure.
	 * @param name
