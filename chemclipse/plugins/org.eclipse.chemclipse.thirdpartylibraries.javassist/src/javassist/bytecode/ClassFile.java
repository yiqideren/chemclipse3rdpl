...
public final class ClassFile {
	...
	private ConstPool compact0() {
		...
		if(interfaces != null) {
			...
			for(int i = 0; i < n; ++i)
				interfaces[i] = cp.addClassInfo(constPool.getClassInfo(interfaces[i]));
		}
		return cp;
	}
	 ...
	 *
	 * <p>
	 * The returned value is obtained from <code>inner_class_access_flags</code> of the entry representing this nested class itself in <code>InnerClasses_attribute</code>.
	 */
	...
	/**
	 * Internal-use only. <code>CtClass.getRefClasses()</code> calls this method.
	 */
