...
/**
 * MultiType represents an unresolved type. Whenever two {@code Type} instances are merged, if they share more than one super type (either an
 * interface or a superclass), then a {@code MultiType} is used to
 * represent the possible super types. The goal of a {@code MultiType} is to reduce the set of possible types down to a single resolved type. This
 * is done by eliminating non-assignable types from the typeset when the {@code MultiType} is passed as an argument to {@link Type#isAssignableFrom(Type)}, as well as removing non-intersecting
 * types during a merge.
 *
 * Note: Currently the {@code MultiType} instance is reused as much
 * as possible so that updates are visible from all frames. In addition, all {@code MultiType} merge paths are also updated. This is somewhat
