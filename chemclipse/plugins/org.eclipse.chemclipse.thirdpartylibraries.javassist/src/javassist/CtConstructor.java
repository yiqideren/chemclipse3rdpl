 ...
 *
 * <p>
 * See the super class <code>CtBehavior</code> as well since a number of useful methods are in <code>CtBehavior</code>.
 *
...
public final class CtConstructor extends CtBehavior {
	 ...
	 *
	 * <p>
	 * The created constructor does not include a constructor body, which must be specified with <code>setBody()</code>.
	 *
	 * @param declaring
	 ...
	 *
	 * <p>
	 * All occurrences of class names in the created constructor are replaced with names specified by <code>map</code> if <code>map</code> is not <code>null</code>.
	 *
	 * <p>
	 * By default, all the occurrences of the names of the class declaring <code>src</code> and the superclass are replaced with the name of the class and the superclass that the created constructor is added to. This is done whichever <code>map</code> is null or not. To prevent this replacement, call <code>ClassMap.fix()</code> or <code>put()</code> to explicitly specify replacement.
	 *
	 * <p>
	 * <b>Note:</b> if the <code>.class</code> notation (for example, <code>String.class</code>) is included in an expression, the Javac compiler may produce a helper method. Since this constructor never copies this helper method, the programmers have the responsiblity of copying it. Otherwise, use <code>Class.forName()</code> in the expression.
	 *
	 * @param src
	 *            the source method.
	 * @param declaring
	...
	public String getLongName() {
		...
		return getDeclaringClass().getName() + (isConstructor() ? Descriptor.toString(getSignature()) : ("." + MethodInfo.nameClinit + "()"));
	}
	...
	public boolean isEmpty() {
		...
		try {
			...
			return op0 == Opcode.RETURN // empty static initializer
					|| (op0 == Opcode.ALOAD_0 && it.byteAt(pos = it.next()) == Opcode.INVOKESPECIAL && (desc = cp.isConstructor(getSuperclassName(), it.u16bitAt(pos + 1))) != 0 && "()V".equals(cp.getUtf8Info(desc)) && it.byteAt(it.next()) == Opcode.RETURN && !it.hasNext());
		} catch(BadBytecode e) {
		}
		return false;
	}
	 ...
	 *            constructor body does nothing except calling <code>super()</code>.
	 */
	 ...
	 *
	 * <p>
	 * All occurrences of the class names in the copied body are replaced with the names specified by <code>map</code> if <code>map</code> is not <code>null</code>.
	 *
	 * @param src
	 ...
	 *
	 * <p>
	 * An occurrence of another constructor call <code>this()</code> or a super constructor call <code>super()</code> is eliminated from the resulting method.
	 *
	 * <p>
	 ...
	 *
	 * @param name
	 *            the name of the resulting method.
	 * @param declaring
	 ...
	 *            replaced while making a copy. <code>map</code> can be <code>null</code>.
	 */
