 ...
 *
 * <p>
 * This is a sample class loader using <code>ClassPool</code>. Unlike a regular class loader, this class loader obtains bytecode from a <code>ClassPool</code>.
 *
 * <p>
 ...
 *
 * <p>
 * This class loader does not allow the users to intercept the loading of <code>java.*</code> and <code>javax.*</code> classes (and <code>sun.*</code>, <code>org.xml.*</code>, ...) unless <code>Loader.doDelegation</code> is <code>false</code>. This is because the JVM prohibits a user class loader from loading a system class. Also see Note 2. If this behavior is not appropriate, a subclass of <code>Loader</code> must be defined and <code>loadClassByDelegation()</code> must be overridden.
 *
 * <p>
 * <b>Note 2:</b>
 *
 * <p>
 * If classes are loaded with different class loaders, they belong to separate name spaces. If class <code>C</code> is loaded by a class loader <code>CL</code>, all classes that the class <code>C</code> refers to are also loaded by <code>CL</code>. However, if <code>CL</code> delegates the loading of the class <code>C</code> to <code>CL'</code>, then those classes that the class <code>C</code> refers to are loaded by a parent class loader <code>CL'</code> instead of <code>CL</code>.
 *
 * <p>
 * If an object of class <code>C</code> is assigned to a variable of class <code>C</code> belonging to a different name space, then a <code>ClassCastException</code> is thrown.
 *
 * <p>
 * Because of the fact above, this loader delegates only the loading of <code>javassist.Loader</code> and classes included in package <code>java.*</code> and <code>javax.*</code> to the parent class loader. Other classes are directly loaded by this loader.
 *
 * <p>
 * For example, suppose that <code>java.lang.String</code> would be loaded by this loader while <code>java.io.File</code> is loaded by the parent class loader. If the constructor of <code>java.io.File</code> is called with an instance of <code>java.lang.String</code>, then it may throw an exception since it accepts an instance of only the <code>java.lang.String</code> loaded by the parent class loader.
 *
...
public class Loader extends ClassLoader {
	 ...
	 *
	 * <p>
	 * This class loader uses the parent class loader for <code>java.*</code> and <code>javax.*</code> classes. If this variable <code>doDelegation</code> is <code>false</code>, this class loader does not delegate those classes to the parent class loader.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If the given class name ends with <code>.</code> (dot), then that name is interpreted as a package name. All the classes in that package and the sub packages are delegated.
	 */
	 ...
	 *
	 * <p>
	 * This method can be overridden by a subclass of <code>Loader</code>. Note that the overridden method must not throw an exception when it just fails to find a class file.
	 *
