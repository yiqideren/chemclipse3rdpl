 ...
 * it searches various sources represented by <code>ClassPath</code> to find a class file and then it creates a <code>CtClass</code> object
 ...
 *
 * <p>
 * <code>ClassPool</code> objects hold all the <code>CtClass</code>es that have been created so that the consistency among modified classes can be guaranteed. Thus if a large number of <code>CtClass</code>es are processed, the <code>ClassPool</code> will consume a huge amount of memory. To avoid this, a <code>ClassPool</code> object should be recreated, for example, every hundred classes processed. Note that <code>getDefault()</code> is a singleton factory. Otherwise, <code>detach()</code> in
 * <code>CtClass</code> should be used to avoid huge memory consumption.
 *
 * <p>
 * <b><code>ClassPool</code> hierarchy:</b>
 *
 * <p>
 * <code>ClassPool</code>s can make a parent-child hierarchy as <code>java.lang.ClassLoader</code>s. If a <code>ClassPool</code> has a parent pool, <code>get()</code> first asks the parent pool to find a class file. Only if the parent could not find the class file, <code>get()</code> searches the <code>ClassPath</code>s of the child <code>ClassPool</code>. This search order is reversed if <code>ClassPath.childFirstLookup</code> is <code>true</code>.
 *
...
public class ClassPool {
	 ...
	 *
	 * <p>
	 * If this field is true, <code>get()</code> first searches the class path associated to this <code>ClassPool</code> and then the class path associated with the parent <code>ClassPool</code>. Otherwise, the class path associated with the parent is searched first.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If this field is true, <code>CtClass</code> objects are automatically pruned by default when <code>toBytecode()</code> etc. are called. The automatic pruning can be turned on/off individually for each <code>CtClass</code> object.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The default class pool searches the system search path, which usually includes the platform library, extension libraries, and the search path specified by the <code>-classpath</code> option or the <code>CLASSPATH</code> environment variable.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Since version 3.14, <code>packageName</code> can be a fully-qualified class name.
	 *
	 * <p>
	 * Note that <code>get()</code> in <code>ClassPool</code> does not search the recorded package. Only the compiler searches it.
	 *
	 * @param packageName
	 *            the package name.
	...
	/**
	 * Reads a class file and constructs a <code>CtClass</code> object with a new name.
	 ...
	 * to the <code>CtClass</code> object representing that class file. If that class file has been
	 * already read, this method returns a reference to the <code>CtClass</code> created when that class file was read at the
	 * first time.
	 *
	 * <p>
	 * If <code>classname</code> ends with "[]", then this method returns a <code>CtClass</code> object for that array type.
	 *
	 * <p>
	...
	/**
	 * Reads a class file from the source and returns a reference
	 * to the <code>CtClass</code> object representing that class file.
	 ...
	 *
	 * <p>
	 * Using this method is not recommended; this method should be used only to obtain the <code>CtClass</code> object with a name returned from <code>getClassInfo</code> in <code>javassist.bytecode.ClassPool</code>. <code>getClassInfo</code> returns a fully-qualified class name but, if the class is an array type, it returns a descriptor.
	 *
	 * @param classname
	...
	/**
	 * Reads class files from the source and returns an array of <code>CtClass</code> objects representing those class files.
	 *
	 * <p>
	 * If an element of <code>classnames</code> ends with "[]", then this method returns a <code>CtClass</code> object for that array type.
	 *
	 * @param classnames
	 ...
	 *
	 * <p>
	 * This method is used for creating a <code>CtClass</code> object directly from a class file. The qualified class name is obtained from the class file; you do not have to explicitly give the name.
	 *
	 * @param classfile
	 *            class file.
	 ...
	 * libraries, and the search path specified by the <code>-classpath</code> option or the <code>CLASSPATH</code> environment variable.
	 *
	 ...
	 * of the current thread. It is obtained by calling <code>getClassLoader()</code>.
	 * 
	 * <p>
	 * This behavior can be changed by subclassing the pool and changing the <code>getClassLoader()</code> method. If the program is running on some application server, the context class loader might be inappropriate to load the class.
	 *
	 * <p>
	...
	/**
	 * Get the classloader for <code>toClass()</code>, <code>getAnnotations()</code> in <code>CtClass</code>, etc.
	 * 
	 * <p>
	 ...
	 * Do not override this method any more at a subclass because <code>toClass(CtClass)</code> never calls this method.
	 *
	 * <p>
	 * <b>Warining:</b> A Class object returned by this method may not work with a security manager or a signed jar file because a protection domain is not specified.
	 *
	 ...
	 *
	 * <p>
	 * The class file represented by the given <code>CtClass</code> is loaded by the given class loader to construct a <code>java.lang.Class</code> object. Since a private method on the class loader is invoked through the reflection API, the caller must have permissions to do that.
	 *
	 * <p>
	 ...
	 *            For example, the loader returned by <code>getClassLoader()</code> can be used
	 *            for this parameter.
	 * @param domain
	 ...
	 *
	 * <p>
	 * You do not necessarily need to call this method. If this method is called, then <code>getPackage()</code> on the <code>Class</code> object returned by <code>toClass()</code> will return a non-null object.
	 *
	 * @param loader
