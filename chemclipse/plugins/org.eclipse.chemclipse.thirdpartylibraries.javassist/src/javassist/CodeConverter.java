 ...
 *
 * <p>
 * Instances of this class specifies how to instrument of the bytecodes representing a method body. They are passed to <code>CtClass.instrument()</code> or <code>CtMethod.instrument()</code> as a parameter.
 *
 * <p>
 * Example:
 * 
...
public class CodeConverter {
	 ...
	 * is replaced with a call to the specified static method. For example, <code>replaceNew(ctPoint, ctSingleton, "createPoint")</code> (where <code>ctPoint</code> and <code>ctSingleton</code> are
	 ...
	 *
	 * <p>
	 * The return type of the substituted static method must be the exactly same as the type of the instantiated class specified by <code>newClass</code>.
	 *
	 * @param newClass
	 ...
	 * specified by <code>oldClass</code> is replaced with instantiation of another class <code>newClass</code>.
	 * For example, <code>replaceNew(ctPoint, ctPoint2)</code> (where <code>ctPoint</code> and <code>ctPoint2</code> are
	 ...
	 *
	 * <p>
	 * Also, <code>clazz</code> and <code>newClass</code> must specify the class directly declaring the field. They must not specify a subclass of that class.
	 *
	 * @param field
	 *            the originally accessed field.
	 * @param newClass
	 *            the class declaring the substituted field.
	 * @param newFieldname
	 ...
	 * 
	 * <p>
	 * The <code>calledClass</code> parameter is the class containing the static methods to be used for array replacement. The <code>names</code> parameter points to an implementation of <code>ArrayAccessReplacementMethodNames</code> which specifies the names of the method to be used for access for each type of array. For example reading from an <code>int[]</code> will require a different method than if writing to an <code>int[]</code>, and writing to a <code>long[]</code> will require a
	 * different method than if writing to a <code>byte[]</code>. If the implementation of <code>ArrayAccessReplacementMethodNames</code> does not contain the name for access for a type of array, that access is not replaced.
	 * 
	 * <p>
	 * A default implementation of <code>ArrayAccessReplacementMethodNames</code> called <code>DefaultArrayAccessReplacementMethodNames</code> has been provided and is what is used in the following example. This also assumes that <code>'foo.ArrayAdvisor'</code> is the name of the <code>CtClass</code> passed in.
	 * 
	 * <p>
	 ...
	 * @param names
	 *            contains the names of the methods to replace
	 ...
	 * That "before" method must be static. The return type must be <code>void</code>. As parameters, the before method receives
	 ...
	 * That "after" method must be static. The return type must be <code>void</code>. As parameters, the after method receives
	 * the target object and all the parameters to the originally invoked
	 * method. For example, if the originally invoked method is <code>move()</code>:
	 *
	 * <pre>
	...
	/**
	 * Default implementation of the <code>ArrayAccessReplacementMethodNames</code> interface giving default values for method names to be used for replacing
