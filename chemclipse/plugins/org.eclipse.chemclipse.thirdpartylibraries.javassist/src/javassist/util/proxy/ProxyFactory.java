 ...
 *
 * <p>
 * This factory generates a class that extends the given super class and implements the given interfaces. The calls of the methods inherited from the super class are forwarded and then <code>invoke()</code> is called on the method handler associated with instances of the generated class. The calls of the methods from the interfaces are also forwarded to the method handler.
 *
 * <p>
 ...
 *
 * <p>
 * Caching is automatically disabled for any given proxy factory if deprecated instance method {@link ProxyFactory#setHandler(MethodHandler)} is called. This method was used to specify a default handler which newly created proxy classes should install when they create their instances. It is only retained to provide backward compatibility with previous releases of javassist. Unfortunately,this legacy behaviour makes caching and reuse of proxy classes impossible. The current programming model
 * expects javassist clients to set the handler of a proxy instance explicitly by calling method {@link Proxy#setHandler(MethodHandler)} as shown in the sample code above. New clients are strongly recommended to use this model rather than calling {@link ProxyFactory#setHandler(MethodHandler)}.
 *
 * <p>
 * A proxy object generated by <code>ProxyFactory</code> is serializable if its super class or any of its interfaces implement <code>java.io.Serializable</code>. However, a serialized proxy object may not be compatible with future releases. The serialization support should be used for short-term storage or RMI.
 *
 * <p>
 ...
 * <p>
 * An alternative method of serializing proxy objects is available which fixes these problems. It requires inhibiting generation of the writeReplace method and instead using instances of {@link javassist.util.proxy.ProxyObjectOutputStream} and {@link javassist.util.proxy.ProxyObjectInputStream} (which are subclasses of {@link java.io.ObjectOutputStream} and {@link java.io.ObjectInputStream}) to serialize and deserialize, respectively, the proxy. These streams recognise javassist proxies and
 * ensure that they are serialized and deserialized without the need for the proxy class to implement special methods such as writeReplace. Generation of the writeReplace method can be disabled globally by setting static field {@link ProxyFactory#useWriteReplace} to false. Alternatively, it may be configured per factory by calling instance method {@link ProxyFactory#setUseWriteReplace(boolean)}.
 *
 * @see MethodHandler
 * @since 3.1
...
public class ProxyFactory {
	 ...
	 * by this variable. For example, if the value is <code>"."</code>, then the class file is written under the current
	 ...
	 * proxy factory. The factory setting may be overwritten by calling factory instance method {@link #setUseCache(boolean)}
	 *
	 * @since 3.4
	 */
	 ...
	 * all fields. However, serialization must be performed via a {@link ProxyObjectOutputStream} and deserialization must be via {@link ProxyObjectInputStream}. Any attempt to serialize
	 * proxies whose class was created with useWriteReplace set to false via a normal {@link java.io.ObjectOutputStream} will fail.
	 *
	 * Note that this value merely specifies the initial setting employed by any newly created
	 * proxy factory. The factory setting may be overwritten by calling factory instance method {@link #setUseWriteReplace(boolean)}
	 *
	 * @since 3.4
	 */
	 ...
	 * a class loader. <code>get()</code> on this <code>ClassLoaderProvider</code> object
	...
	private void computeSignature(MethodFilter filter) // throws CannotCompileException
	{
		...
		for(int idx = 0; idx < l; idx++) {
			...
			if(!Modifier.isFinal(mod) && !Modifier.isStatic(mod) && isVisible(mod, basename, m) && (filter == null || filter.isHandled(m))) {
