...
public class Annotation extends Figure implements IAxisListener, IDataProviderListener {
	...
	public enum CursorLineStyle {
		NONE(0, "None"), UP_DOWN(1, "Up&Down"), LEFT_RIGHT(2, "Left&Right"), FOUR_DIRECTIONS(3, "Four Directions");
	...
	protected void paintFigure(Graphics graphics) {
		...
		Rectangle infoBounds = new Rectangle((int)(currentPosition.x + x0 - size.width / 2.0), (int)(currentPosition.y + y0 - size.height / 2.0), size.width, size.height);
		infoLabel.setBounds(infoBounds);
		pointer.setBounds(new Rectangle(currentPosition.x - POINT_SIZE, currentPosition.y - POINT_SIZE, 2 * POINT_SIZE, 2 * POINT_SIZE));
		if(infoLabelArmed) // draw infoLabel Armed rect
			graphics.drawRectangle(infoBounds);
		// draw indicate line
		graphics.drawLine(currentPosition.x + (int)dx, currentPosition.y + (int)dy, currentPosition.x, currentPosition.y);
		// draw Arrow
		...
		switch(cursorLineStyle) {
			case NONE:
				// left
				graphics.drawLine(currentPosition.x - POINT_SIZE / 2, currentPosition.y, currentPosition.x - POINT_SIZE / 2 - CURSOR_LINE_LENGTH, currentPosition.y);
				// right
				graphics.drawLine(currentPosition.x + POINT_SIZE / 2, currentPosition.y, currentPosition.x + POINT_SIZE / 2 + CURSOR_LINE_LENGTH, currentPosition.y);
				// up
				graphics.drawLine(currentPosition.x, currentPosition.y - POINT_SIZE / 2, currentPosition.x, currentPosition.y - POINT_SIZE / 2 - CURSOR_LINE_LENGTH);
				// down
				graphics.drawLine(currentPosition.x, currentPosition.y + POINT_SIZE / 2, currentPosition.x, currentPosition.y + POINT_SIZE / 2 + CURSOR_LINE_LENGTH);
				break;
			...
			case LEFT_RIGHT:
				// left
				graphics.drawLine(currentPosition.x - POINT_SIZE / 2, currentPosition.y, xAxis.getValuePosition(xAxis.getRange().getLower(), false), currentPosition.y);
				// right
				graphics.drawLine(currentPosition.x + POINT_SIZE / 2, currentPosition.y, xAxis.getValuePosition(xAxis.getRange().getUpper(), false), currentPosition.y);
				if(cursorLineStyle != CursorLineStyle.FOUR_DIRECTIONS)
					break;
			case UP_DOWN:
				// up
				graphics.drawLine(currentPosition.x, currentPosition.y - POINT_SIZE / 2, currentPosition.x, yAxis.getValuePosition(yAxis.getRange().getUpper(), false));
				// down
				graphics.drawLine(currentPosition.x, currentPosition.y + POINT_SIZE / 2, currentPosition.x, yAxis.getValuePosition(yAxis.getRange().getLower(), false));
				break;
			default:
				break;
		}
	...
	public void updateX0Y0Fromdxdy(Dimension size) {
		...
		if(!knowX0Y0) {
			...
			if(dy != 0) {
				...
				y0 = dy - h / 2.0;
				x0 = (dx / dy) * y0;
				if(new Range(0, x0).inRange(dx) && new Range(0, y0).inRange(dy) && new Range(x0 - w / 2.0, x0 + w / 2.0).inRange(dx))
					return;
				y0 = dy + h / 2.0;
				x0 = (dx / dy) * y0;
				if(new Range(0, x0).inRange(dx) && new Range(0, y0).inRange(dy) && new Range(x0 - w / 2.0, x0 + w / 2.0).inRange(dx))
			...
			if(dx != 0) {
				x0 = dx + w / 2.0;
				y0 = (dy / dx) * x0;
				if(new Range(0, x0).inRange(dx) && new Range(0, y0).inRange(dy) && new Range(y0 - h / 2.0, y0 + h / 2.0).inRange(dy))
					return;
				x0 = dx - w / 2.0;
				y0 = (dy / dx) * x0;
				if(new Range(0, x0).inRange(dx) && new Range(0, y0).inRange(dy) && new Range(y0 - h / 2.0, y0 + h / 2.0).inRange(dy))
	...
	private void updateToDefaultPosition() {
		...
		if(trace != null && trace.getHotSampleList().size() > 0) {
			...
			currentPosition = new Point(xAxis.getValuePosition(currentSnappedSample.getXValue(), false), yAxis.getValuePosition(currentSnappedSample.getXValue(), false));
	 ...
	 * @param y
	 *            Position as value on the Y axis
	...
	public void setCurrentSnappedSample(ISample currentSnappedSample, boolean keepLabelPosition) {
		...
		if(!trace.getHotSampleList().contains(currentSnappedSample))
			...
			Point newPosition = new Point(xAxis.getValuePosition(currentSnappedSample.getXValue(), false), yAxis.getValuePosition(currentSnappedSample.getYValue(), false));
			xValue = currentSnappedSample.getXValue();
			yValue = currentSnappedSample.getYValue();
	...
	public void dataChanged(IDataProvider dataProvider) {
		...
		if(trace == null)
			return;
		if(trace.getHotSampleList().contains(currentSnappedSample)) {
			if(yValue != currentSnappedSample.getYValue()) { // When waveform index is changed, Y value of the
	...
	class Pointer extends Figure {
		...
		class PointerDragger extends MouseMotionListener.Stub implements MouseListener {
			...
			public void mouseDragged(MouseEvent me) {
				...
				// free
				if(trace == null) {
					setCurrentPosition(me.getLocation(), me.getState() == (InputEvent.BUTTON1 | InputEvent.CONTROL));
				} else { // snap to trace
					...
					for(ISample s : trace.getHotSampleList()) {
						d = Math.sqrt(Math.pow(xAxis.getValuePosition(s.getXValue(), false) - me.getLocation().x, 2) + Math.pow(yAxis.getValuePosition(s.getYValue(), false) - me.getLocation().y, 2));
						if(minD > d) {
							minD = d;
							tempSample = s;
						}
					...
					if(tempSample != null && currentSnappedSample != tempSample)
						setCurrentSnappedSample(tempSample, me.getState() == (InputEvent.BUTTON1 | InputEvent.CONTROL));
