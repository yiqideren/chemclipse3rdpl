	...
	private void drawTraceLagend(Trace trace, Graphics graphics, int hPos, int vPos) {
		...
		switch(trace.getTraceType()) {
			...
			case AREA:
				...
				graphics.fillPolygon(new int[]{hPos, vPos + ICON_WIDTH / 2, hPos + ICON_WIDTH / 2, vPos + trace.getPointSize() / 2, hPos + ICON_WIDTH, vPos + ICON_WIDTH / 2, hPos + ICON_WIDTH, vPos + ICON_WIDTH, hPos, vPos + ICON_WIDTH});
				if(Preferences.useAdvancedGraphics())
		...
		// draw text
		graphics.drawText(trace.getName(), hPos + ICON_WIDTH + INNER_GAP, vPos + ICON_WIDTH / 2 - FigureUtilities.getTextExtents(trace.getName(), getFont()).height / 2);
		graphics.popState();
	}
