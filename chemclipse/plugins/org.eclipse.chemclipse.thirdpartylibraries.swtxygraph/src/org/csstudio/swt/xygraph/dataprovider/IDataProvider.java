 ...
 * </code> would be much easier to use by the XY Graph, but it forces the data provider to copy its samples into an array, which might be a performance and memory problem if considerable amounts of data are held in some other data structure that is more suitable to the application.
 * <p>
 ...
 * Since the application data might change dynamically, the XY Graph <code>synchronizes</code> on the <code>IDataProvider</code> like this to assert that the sample count does not change while accessing individual samples:
 * 
 * <pre>
 * IDataProvider data = ...;
 * synchronized (data)
 ...
 * </pre>
 * 
 * Implementations of the <code>IDataProvider</code> should likewise synchronize on it whenever the data is changed, and other methods like <code>getXDataMinMax</code> should probably be synchronized implementations.
 * 
 * @author Xihui Chen
...
public interface IDataProvider {
	 ...
	 * Get sample by index
	 * <p>
	 * <b>Synchronization:</b> Since the data might change dynamically, <code>synchronize</code> on the <code>IDataProvider</code> around calls to <code>getSize()</code> and <code>getSample()</code>.
	 *
	 * @param index
	 *            Sample index, 0...<code>getSize()-1</code>
	 * @return the sample.
	 */
	...
	/**
	 * @param listener
	 *            Listener to no longer notify when data changes
