...
public class CircularBufferDataProvider extends AbstractDataProvider {
	...
	private void tryToAddDataPoint() {
		...
		switch(updateMode) {
			case X_OR_Y:
				if((chronological && currentYDataChanged) || (!chronological && (currentXDataChanged || currentYDataChanged)))
					addDataPoint();
				break;
			case X_AND_Y:
				if((chronological && currentYDataChanged) || (!chronological && (currentXDataChanged && currentYDataChanged)))
	...
	private void tryToAddDataArray() {
		...
		switch(updateMode) {
			case X_OR_Y:
				if((chronological && currentYDataArrayChanged) || (!chronological && (currentXDataArrayChanged || currentYDataArrayChanged)))
					addDataArray();
				break;
			case X_AND_Y:
				if((chronological && currentYDataArrayChanged) || (!chronological && (currentXDataArrayChanged && currentYDataArrayChanged)))
	...
	private void addDataArray() {
		...
		if(chronological) {
			...
			for(int i = 0; i < Math.min(traceData.getBufferSize(), Math.min(newXValueArray.length, currentYDataArray.length)); i++) {
			...
			for(int i = 0; i < Math.min(traceData.getBufferSize(), Math.min(currentXDataArray.length, currentYDataArray.length)); i++) {
	...
	/**
	 * If xAxisDateEnable is true, you will need to use {@link #setCurrentYData(double, long)} or {@link #setCurrentYDataTimestamp(long)} to set the
	...
	/**
	 * @param updateDelay
	 *            Delay in milliseconds between plot updates. This may help to reduce CPU
