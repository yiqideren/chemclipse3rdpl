...
public class Axis extends LinearScale {
	...
	public Range getTraceDataRange() {
		...
		for(Trace trace : traceList) {
			...
			if(range == null)
				continue;
			if(Double.isInfinite(range.getLower()) || Double.isInfinite(range.getUpper()) || Double.isNaN(range.getLower()) || Double.isNaN(range.getUpper()))
				continue;
	 ...
	 *
	 * @param force
	 *            If true, the axis will be auto-scaled by force regardless the autoScale field.
	...
	public boolean performAutoScale(final boolean force) {
		...
		if(isLogScaleEnabled()) { // Transition into log space
		...
		if(((tempMin - min) >= 0 && (tempMin - min) < thr) && ((max - tempMax) >= 0 && (max - tempMax) < thr)) {
			return false;
	...
	private boolean isValidZoomType(final ZoomType zoom) {
		...
		return zoom == ZoomType.PANNING || zoom == ZoomType.RUBBERBAND_ZOOM || zoom == ZoomType.ZOOM_IN || zoom == ZoomType.ZOOM_OUT || (isHorizontal() && (zoom == ZoomType.HORIZONTAL_ZOOM || zoom == ZoomType.ZOOM_IN_HORIZONTALLY || zoom == ZoomType.ZOOM_OUT_HORIZONTALLY)) || (!isHorizontal() && (zoom == ZoomType.VERTICAL_ZOOM || zoom == ZoomType.ZOOM_OUT_VERTICALLY || zoom == ZoomType.ZOOM_IN_VERTICALLY));
	}
	 ...
	 * 
	 * @param onPrimarySide
	 *            set true if the axis on primary side(Bottom/Left);
