...
public class LinearScaleTickLabels extends Figure {
	...
	private void updateTickLabelForLogScale(int length) {
		...
		for(int i = digitMin; minBigger ? i >= digitMax : i <= digitMax; i += minBigger ? -1 : 1) {
			if(Math.abs(digitMax - digitMin) > 20) {// if the range is too big, skip minor ticks.
				...
				int tickLabelPosition = (int)((Math.log10(v.doubleValue()) - Math.log10(min)) / (Math.log10(max) - Math.log10(min)) * length) + scale.getMargin();
				...
				for(BigDecimal j = firstPosition; minBigger ? j.doubleValue() >= pow(10, i - 1).doubleValue() : j.doubleValue() <= pow(10, i).doubleValue(); j = minBigger ? j.subtract(tickStep) : j.add(tickStep)) {
					...
					int tickLabelPosition = (int)((Math.log10(j.doubleValue()) - Math.log10(min)) / (Math.log10(max) - Math.log10(min)) * length) + scale.getMargin();
					tickLabelPositions.add(tickLabelPosition);
				}
	...
	private void updateTickVisibility() {
		...
		for(int i = 0; i < tickLabelPositions.size(); i++) {
			...
			if(i != 0) {
				hasSpaceToDraw = hasSpaceToDraw(previousPosition, tickLabelPositions.get(i), previousLabel, tickLabels.get(i));
			}
	...
	private boolean hasSpaceToDraw(int previousPosition, int tickLabelPosition, String previousTickLabel, String tickLabel) {
		...
		int textLength = (int)(scale.isHorizontal() ? (tickLabelSize.width / 2.0 + previousTickLabelSize.width / 2.0) : tickLabelSize.height);
		...
		if(tickLabelPosition != tickLabelPositions.get(tickLabelPositions.size() - 1)) { // if it is not the end tick label.
			...
			Dimension endTickLabelSize = FigureUtilities.getTextExtents(tickLabels.get(tickLabels.size() - 1), scale.getFont());
			interval = tickLabelPositions.get(tickLabelPositions.size() - 1) - tickLabelPosition;
			textLength = (int)(scale.isHorizontal() ? (tickLabelSize.width / 2.0 + endTickLabelSize.width / 2.0) : tickLabelSize.height);
