...
public class RoundScaleTickLabels extends Figure {
	...
	private void updateTickLabelForLogScale(double lengthInDegrees) {
		...
		for(int i = digitMin; minBigger ? i >= digitMax : i <= digitMax; i += minBigger ? -1 : 1) {
			if(Math.abs(digitMax - digitMin) > 20) {// if the range is too big, skip minor ticks.
				...
				double tickLabelPosition = scale.getStartAngle() - ((Math.log10(v.doubleValue()) - Math.log10(min)) / (Math.log10(max) - Math.log10(min)) * lengthInDegrees);
				tickLabelPosition = tickLabelPosition * Math.PI / 180;
				tickLabelPositions.add(tickLabelPosition);
			} else {
				for(BigDecimal j = firstPosition; minBigger ? j.doubleValue() >= pow(10, i - 1).doubleValue() : j.doubleValue() <= pow(10, i).doubleValue(); j = minBigger ? j.subtract(tickStep) : j.add(tickStep)) {
					...
					double tickLabelPosition = scale.getStartAngle() - ((Math.log10(j.doubleValue()) - Math.log10(min)) / (Math.log10(max) - Math.log10(min)) * lengthInDegrees);
					tickLabelPosition = tickLabelPosition * Math.PI / 180;
					tickLabelPositions.add(tickLabelPosition);
				}
	...
	private void updateTickLabelForLinearScale(double lengthInDegrees, int lengthInPixels) {
		...
		for(BigDecimal b = firstPosition; max >= min ? b.doubleValue() <= max : b.doubleValue() >= max; b = b.add(tickStep)) {
			if(scale.isDateEnabled()) {
			...
			double tickLabelPosition = scale.getStartAngle() - ((b.doubleValue() - min) / (max - min) * lengthInDegrees);
			tickLabelPosition = tickLabelPosition * Math.PI / 180;
			tickLabelPositions.add(tickLabelPosition);
		}
		// add max
		if((minBigger ? max < tickLabelValues.get(tickLabelValues.size() - 1) : max > tickLabelValues.get(tickLabelValues.size() - 1))) {
			tickLabelValues.add(max);
			if(scale.isDateEnabled()) {
				Date date = new Date((long)max);
				tickLabels.add(scale.format(date));
		...
		for(int i = 0; i < tickLabelPositions.size(); i++) {
			...
			if(i != 0) {
				if(i != (tickLabelPositions.size() - 1))
					hasSpaceToDraw = hasSpaceToDraw(previousArea, tickLabelAreas.get(i)) && hasSpaceToDraw(tickLabelAreas.get(i), tickLabelAreas.get(tickLabelPositions.size() - 1));
				else
					hasSpaceToDraw = hasSpaceToDraw(previousArea, tickLabelAreas.get(i)) && hasSpaceToDraw(tickLabelAreas.get(0), tickLabelAreas.get(i));
			}
			// check if the same tick label is repeated
			String currentLabel = tickLabels.get(i);
			boolean isRepeatSameTickAndNotEnd = currentLabel.equals(previousLabel) && (i != 0 && i != tickLabelPositions.size() - 1);
	...
	private void updateTickLabelMaxOutLength() {
		...
		tickLabelMaxOutLength = Math.max(Math.max(maxRight - scale.getBounds().width, -minLeft), Math.max(maxDown - scale.getBounds().height, -minUp));
	}
	...
	private BigDecimal getGridStep(int lengthInPixels, final double minR, final double maxR) {
		...
		double gridStepHint = length / lengthInPixels * markStepHint;
		if(scale.isDateEnabled()) {
