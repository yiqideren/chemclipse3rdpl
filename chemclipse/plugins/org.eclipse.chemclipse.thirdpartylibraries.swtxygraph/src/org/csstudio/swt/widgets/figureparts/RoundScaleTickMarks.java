...
public class RoundScaleTickMarks extends Figure {
	...
	protected void paintClientArea(Graphics graphics) {
		...
		ArrayList<Double> tickLabelPositions = scale.getScaleTickLabels().getTickLabelPositions();
	...
	public void updateMinorTickParas() {
		...
		if(scale.getScaleTickLabels().getGridStepInRadians() / 5 >= scale.convertPixelToRadians(scale.getMinorTickMarkStepHint())) {
			minorTicksNumber = 5;
			minorGridStepInRadians = scale.getScaleTickLabels().getGridStepInRadians() / 5.0;
			return;
		}
		if(scale.getScaleTickLabels().getGridStepInRadians() / 4 >= scale.convertPixelToRadians(scale.getMinorTickMarkStepHint())) {
			minorTicksNumber = 4;
	...
	private void drawTickMarks(Graphics graphics, ArrayList<Double> tickLabelPositions) {
		...
		if(scale.isLogScaleEnabled()) {
			...
			for(int i = 0; i < tickLabelPositions.size(); i++) {
				...
				if(scale.getTickLablesSide() == LabelSide.Primary)
					endP = new PolarPoint(r + tickLength, theta).toRelativePoint(scale.getBounds());
				else
					endP = new PolarPoint(r - tickLength, theta).toRelativePoint(scale.getBounds());
			...
			for(int i = 0; i < tickLabelPositions.size(); i++) {
				...
				Point endP;
				if(scale.getTickLablesSide() == LabelSide.Primary)
					endP = new PolarPoint(r + MAJOR_TICK_LENGTH, theta).toRelativePoint(scale.getBounds());
				else
					endP = new PolarPoint(r - MAJOR_TICK_LENGTH, theta).toRelativePoint(scale.getBounds());
				...
				if(scale.isMinorTicksVisible()) {
					if(i > 0) {
						...
						// draw the first grid step which is start from min value
						if(i == 1 && (tickLabelPositions.get(0) - tickLabelPositions.get(1)) < scale.getScaleTickLabels().getGridStepInRadians()) {
						...
						else if(i == tickLabelPositions.size() - 1 && (tickLabelPositions.get(i - 1) - tickLabelPositions.get(i)) < scale.getScaleTickLabels().getGridStepInRadians()) {
							...
							for(int j = 1; j < minorTicksNumber; j++) {
								double t = tickLabelPositions.get(i - 1) + (tickLabelPositions.get(i) - tickLabelPositions.get(i - 1)) * j / minorTicksNumber;
								drawMinorTick(graphics, r, t);
							}
						}
					}
		...
		if(scale.isScaleLineVisible()) {
			graphics.drawArc(new Rectangle(bounds.x + bounds.width / 2 - scale.getRadius(), bounds.y + bounds.height / 2 - scale.getRadius(), scale.getRadius() * 2, scale.getRadius() * 2), (int)scale.getEndAngle(), (int)scale.getLengthInDegrees());
		}
	}

	private void drawMinorTick(Graphics graphics, int r, double t) {

		Point minorStartP = new PolarPoint(r, t).toRelativePoint(scale.getBounds());
		Point minorEndP;
		if(scale.getTickLablesSide() == LabelSide.Primary)
			minorEndP = new PolarPoint(r + MINOR_TICK_LENGTH, t).toRelativePoint(scale.getBounds());
		else
			minorEndP = new PolarPoint(r - MINOR_TICK_LENGTH, t).toRelativePoint(scale.getBounds());
