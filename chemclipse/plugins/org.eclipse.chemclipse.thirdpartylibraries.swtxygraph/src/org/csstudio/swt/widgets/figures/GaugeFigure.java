...
public class GaugeFigure extends AbstractRoundRampedFigure {
	...
	private final Font DEFAULT_LABEL_FONT = XYGraphMediaFactory.getInstance().getFont(new FontData("Arial", 12, SWT.BOLD));
	...
	protected void paintClientArea(Graphics graphics) {
		...
		if(effect3D && support3D) {
			...
			pattern = new Pattern(Display.getCurrent(), area.x, area.y, area.x + area.width, area.y + area.height, BORDER_COLOR, WHITE_COLOR);
		...
		if(effect3D && support3D) {
			...
			// add this to eliminate the repaint bug on Mac
			graphics.fillOval(new Rectangle());
			Pattern glossyPattern = new Pattern(Display.getCurrent(), area.x + area.width / 2, (float)(area.y + area.height / 2 - R * UD_FILL_PART), area.x + area.width / 2, (float)(area.y + area.height / 2 + R * UP_DOWN_RATIO), WHITE_COLOR, 90, WHITE_COLOR, 0);
			graphics.setBackgroundPattern(glossyPattern);
			Rectangle rectangle = new Rectangle((int)(area.x + area.width / 2 - R * LR_FILL_PART * Math.cos(UP_ANGLE)), (int)(area.y + area.height / 2 - R * UD_FILL_PART), (int)(2 * R * LR_FILL_PART * Math.cos(UP_ANGLE)), (int)(R * UD_FILL_PART + R * UP_DOWN_RATIO));
			graphics.fillOval(rectangle);
			glossyPattern.dispose();
			glossyPattern = new Pattern(Display.getCurrent(), area.x + area.width / 2, (float)(area.y + area.height / 2 + R * UP_DOWN_RATIO - 1), area.x + area.width / 2, (float)(area.y + area.height / 2 + R * UD_FILL_PART + 1), WHITE_COLOR, 0, WHITE_COLOR, 40);
			graphics.setBackgroundPattern(glossyPattern);
	...
	class NeedleCenter extends Ellipse {
		...
		protected void fillShape(Graphics graphics) {
			...
			if(effect3D && support3D) {
				// add this to eliminate the repaint bug on Mac
				graphics.fillOval(new Rectangle());
				pattern = new Pattern(Display.getCurrent(), bounds.x, bounds.y, bounds.x + bounds.width, bounds.y + bounds.height, WHITE_COLOR, BORDER_COLOR);
				graphics.setBackgroundPattern(pattern);
			}
	...
	class GaugeLayout extends AbstractLayout {
		...
		public void layout(IFigure container) {
			...
			if(valueLabel != null) {
				...
				valueLabel.setBounds(new Rectangle(area.x + area.width / 2 - labelSize.width / 2, area.y + area.height * 7 / 8 - labelSize.height / 2, labelSize.width, labelSize.height));
			}
			if(needle != null && scale != null) {
				...
				needlePoints.setPoint(new Point(center.x + area.width / 2 - RoundScaleTickMarks.MAJOR_TICK_LENGTH - GAP_BTW_NEEDLE_SCALE, center.y), 1);
			...
			if(needleCenter != null) {
				needleCenter.setBounds(new Rectangle(center.x - NeedleCenter.DIAMETER / 2, center.y - NeedleCenter.DIAMETER / 2, NeedleCenter.DIAMETER, NeedleCenter.DIAMETER));
			}
		}
	}
}
