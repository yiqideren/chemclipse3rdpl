...
public class RoundScaledRamp extends Figure {
	...
	private void updateThresholdPosition() {
		...
		if(dirty) {
			...
			normal.rightPoint = new PolarPoint(bounds.width / 2, (normal.absolutePosition - OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
			normal.leftPoint = new PolarPoint(bounds.width / 2, (normal.absolutePosition + OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
			// update min, max
			...
			if(lolo.visible) {
				...
				lolo.rightPoint = new PolarPoint(bounds.width / 2, (lolo.absolutePosition - OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
				lolo.leftPoint = new PolarPoint(bounds.width / 2, (lolo.absolutePosition + OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
			}
			if(lo.visible) {
				...
				lo.rightPoint = new PolarPoint(bounds.width / 2, (lo.absolutePosition - OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
				lo.leftPoint = new PolarPoint(bounds.width / 2, (lo.absolutePosition + OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
			}
			if(hi.visible) {
				...
				hi.rightPoint = new PolarPoint(bounds.width / 2, (hi.absolutePosition - OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
				hi.leftPoint = new PolarPoint(bounds.width / 2, (hi.absolutePosition + OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
			}
			if(hihi.visible) {
				...
				hihi.rightPoint = new PolarPoint(bounds.width / 2, (hihi.absolutePosition - OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
				hihi.leftPoint = new PolarPoint(bounds.width / 2, (hihi.absolutePosition + OVERLAP_DEGREE) * Math.PI / 180).toAbsolutePoint(bounds);
			}
			setDirty(false);
		}
	}
	...
	protected void paintClientArea(Graphics graphics) {
		...
		if(lo.visible) {
			if(support3D && gradient && lolo.visible) {
				try {
					pattern = new Pattern(Display.getCurrent(), lolo.leftPoint.x, lolo.leftPoint.y, lo.rightPoint.x, lo.rightPoint.y, lolo.color, lo.color);
		...
		if(gradient && leftMarkerVisible && support3D) {
			pattern = new Pattern(Display.getCurrent(), leftMarker.leftPoint.x, leftMarker.leftPoint.y, normal.rightPoint.x, normal.rightPoint.y, leftMarker.color, normal.color);
			graphics.setBackgroundPattern(pattern);
			overlap = OVERLAP_DEGREE / 2;
		} else {
		...
		if(gradient && rightMarkerVisible && support3D) {
			pattern = new Pattern(Display.getCurrent(), rightMarker.rightPoint.x, rightMarker.rightPoint.y, normal.leftPoint.x, normal.leftPoint.y, rightMarker.color, normal.color);
			graphics.setBackgroundPattern(pattern);
			overlap = OVERLAP_DEGREE / 2;
		} else {
			graphics.setBackgroundColor(normal.color);
		...
		if(hi.visible) {
			if(hihi.visible) {
				...
				rightMarkerVisible = false;
			if(gradient && rightMarkerVisible && support3D) {
				pattern = new Pattern(Display.getCurrent(), rightMarker.rightPoint.x, rightMarker.rightPoint.y, hi.leftPoint.x, hi.leftPoint.y, rightMarker.color, hi.color);
				graphics.setBackgroundPattern(pattern);
				overlap = OVERLAP_DEGREE / 2;
			} else {
	 ...
	 * 
	 * @param thresholdName
	 *            the threshold name which should be one of {@link Threshold}
	 * @param value
	 *            the value to set
	 */
