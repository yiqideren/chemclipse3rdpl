...
public class IntensityGraphFigure extends Figure implements Introspectable {
	...
	class SinglePixelProfileCrossHair extends Figure {
		...
		public boolean containsPoint(int x, int y) {
			...
			return hLine.containsPoint(x, y) || vLine.containsPoint(x, y) || crossPoint.containsPoint(x, y);
		}
	...
	public class GraphArea extends Figure {
		...
		private synchronized IPrimaryArrayWrapper cropDataArray(int left, int right, int top, int bottom) {
			...
			if((left != 0 || right != 0 || top != 0 || bottom != 0) && (dataWidth - left - right) * (dataHeight - top - bottom) > 0) {
				int i = 0;
				if((dataWidth - left - right) * (dataHeight - top - bottom) > MAX_ARRAY_SIZE)
					return dataArray;
				double[] result = null;
				if(inRGBMode) {
					result = new double[(dataWidth - left - right) * (dataHeight - top - bottom) * 3];
					...
					result = new double[(dataWidth - left - right) * (dataHeight - top - bottom)];
					for(int y = top; y < (dataHeight - bottom); y++) {
						for(int x = left; x < (dataWidth - right); x++) {
		...
		protected synchronized void paintClientArea(Graphics graphics) {
			...
			if(dataDirty || bufferedImage == null) {
				...
				if(clientArea.width < 0 || clientArea.height < 0)
					return;
				if(dataWidth == 0 || dataHeight == 0 || (!isInRGBMode() && dataArray.getSize() < dataWidth * dataHeight) || (isInRGBMode() && dataArray.getSize() < 3 * dataWidth * dataHeight)) {
					graphics.drawRectangle(new Rectangle(clientArea.x - (yAxis.isVisible() ? 1 : 0), clientArea.y, clientArea.width - (yAxis.isVisible() ? 0 : 1), clientArea.height - (xAxis.isVisible() ? 0 : 1)));
				...
				if(shrink) {
					if(bufferedImageData == null || bufferedImageData.width != clientArea.width || bufferedImageData.height != clientArea.height) {
						bufferedImageData = new ImageData(clientArea.width, clientArea.height, 24, colorMap.getPalette());
					}
				} else if(bufferedImageData == null || bufferedImageData.width != croppedDataWidth || bufferedImageData.height != croppedDataHeight)
					bufferedImageData = new ImageData(croppedDataWidth, croppedDataHeight, 24, colorMap.getPalette());
				ImageData imageData = null;
				if(inRGBMode)
					try {
						imageData = drawRGBImage(croppedDataArray, croppedDataWidth, croppedDataHeight, max, min, bufferedImageData, shrink);
					...
					imageData = colorMap.drawImage(croppedDataArray, croppedDataWidth, croppedDataHeight, max, min, bufferedImageData, shrink);
				if(imageData == null)
					return;
		...
		private synchronized void updateTextCursor(MouseEvent me) {
			...
			if(inRGBMode) {
				int index = (dataLocation.y) * croppedDataWidth * 3 + dataLocation.x * 3;
				valueUnderMouse = (croppedDataArray.get(index) + croppedDataArray.get(index + 1) + croppedDataArray.get(index + 2)) / 3;
			} else
				valueUnderMouse = croppedDataArray.get((dataLocation.y) * croppedDataWidth + dataLocation.x);
			String text = "(" + xAxis.format(xCoordinate) + ", " + yAxis.format(yCoordinate) + ", " + yAxis.format(valueUnderMouse) + ")";
			text = text + getPixelInfo(dataLocation.x + cropLeft, dataLocation.y + cropTop, xCoordinate, yCoordinate, valueUnderMouse);
			Dimension size = FigureUtilities.getTextExtents(text, Display.getDefault().getSystemFont());
			...
			gc.fillRectangle(CURSOR_SIZE, CURSOR_SIZE, image.getBounds().width - CURSOR_SIZE, image.getBounds().height - CURSOR_SIZE);
			...
			setCursor(new Cursor(Display.getCurrent(), imageData, CURSOR_SIZE / 2, CURSOR_SIZE / 2));
			gc.dispose();
			image.dispose();
		}
	}
		...
		if(isSingleLineProfiling()) {
			...
			for(int i = 0; i < dw; i++) {
				if(inRGBMode) {
					...
					output[i] = (data.get(index) + data.get(index + 1) + data.get(index + 2)) / 3;
				} else
	...
	private synchronized void fireProfileDataChanged(final IPrimaryArrayWrapper data, final int dw, final int dh) {
	...
	public Dimension getGraphAreaInsets() {
		...
		if(colorMapRamp.isVisible())
			width += (colorMapRamp.getPreferredSize(getClientArea().width, getClientArea().height).width + GAP);
		if(yAxis.isVisible()) {
	...
	protected void layout() {
		...
		if(yAxis.isVisible()) {
			...
			yAxisBounds = new Rectangle(clientArea.x, clientArea.y, yAxisSize.width, yAxisSize.height); // the height is not correct for now
		...
		if(xAxis.isVisible()) {
			...
			xAxisBounds = new Rectangle((yAxis.isVisible() ? yAxisBounds.x + yAxisBounds.width - xAxis.getMargin() - 1 : clientArea.x), clientArea.y + clientArea.height - xAxisSize.height, xAxisSize.width, xAxisSize.height); // the width is not correct for now
	...
	private void zoom() {
		...
		Point leftTop = graphArea.getDataLocation(Math.min(start.x, end.x), Math.min(start.y, end.y));
		Point rightBottom = graphArea.getDataLocation(Math.max(start.x, end.x), Math.max(start.y, end.y));
		...
		if(toBeCropLeft + toBeCropRight >= dataWidth || toBeCropBottom + toBeCropTop >= dataHeight)
			return;
