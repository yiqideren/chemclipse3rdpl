...
public class RoundScale extends AbstractScale {
	...
	private void calcEstimatedDonutWidth() {
		...
		estimatedDonutWidth = Math.max(FigureUtilities.getTextExtents(format(getRange().getLower()), getFont()).width, FigureUtilities.getTextExtents(format(getRange().getUpper()), getFont()).width) + SPACE_BTW_MARK_LABEL + RoundScaleTickMarks.MAJOR_TICK_LENGTH;
	}
	...
	public double getValuePosition(double value, boolean relative) {
		...
		if(isLogScaleEnabled()) {
			...
			valuePosition = startAngle - ((Math.log10(value) - Math.log10(min)) / (Math.log10(max) - Math.log10(min)) * lengthInDegrees);
		} else
