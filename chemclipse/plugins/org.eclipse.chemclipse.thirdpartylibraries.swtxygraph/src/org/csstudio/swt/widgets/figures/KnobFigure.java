	...
	class KnobLayout extends AbstractLayout {
		...
		public void layout(IFigure container) {
			...
			if(scale != null) {
				...
				bulbBounds.shrink(area.width / 2 - scale.getInnerRadius() + GAP_BTW_BULB_SCALE, area.height / 2 - scale.getInnerRadius() + GAP_BTW_BULB_SCALE);
			}
			if(scale != null && ramp != null && ramp.isVisible()) {
				Rectangle rampBounds = area.getCopy();
				ramp.setBounds(rampBounds.shrink(area.width / 2 - scale.getInnerRadius() - ramp.getRampWidth() + 2, area.height / 2 - scale.getInnerRadius() - ramp.getRampWidth() + 2));
			}
			if(valueLabel != null && valueLabel.isVisible()) {
				Dimension labelSize = valueLabel.getPreferredSize();
				valueLabel.setBounds(new Rectangle(bulbBounds.x + bulbBounds.width / 2 - labelSize.width / 2, bulbBounds.y + bulbBounds.height * 3 / 4 - labelSize.height / 2, labelSize.width, labelSize.height));
			}
			if(bulb != null && scale != null && bulb.isVisible()) {
				bulb.setBounds(bulbBounds);
			}
			if(scale != null && thumb != null && thumb.isVisible()) {
				Point thumbCenter = new Point(bulbBounds.x + bulbBounds.width * 7.0 / 8.0, bulbBounds.y + bulbBounds.height / 2);
				...
				thumb.setBounds(new Rectangle(thumbCenter.x - thumbDiameter / 2, thumbCenter.y - thumbDiameter / 2, thumbDiameter, thumbDiameter));
			}
		}
	...
	class Thumb extends Ellipse {
		...
		class ThumbDragger extends MouseMotionListener.Stub implements MouseListener {
			...
			public void mouseDragged(MouseEvent me) {
				...
				if(!armed)
					return;
				PolarPoint currentPP = PolarPoint.point2PolarPoint(pole, me.getLocation());
				...
				if(currentPP.theta * 180.0 / Math.PI > (((RoundScale)scale).getLengthInDegrees())) {
					if(Math.abs(((RoundScale)scale).getValuePosition(getCoercedValue(), true) - (((RoundScale)scale).getLengthInDegrees())) < ((RoundScale)scale).getLengthInDegrees() / 2.0)
		...
		protected void fillShape(Graphics graphics) {
			...
			if(support3D && effect3D) {
				try {
					...
					pattern = new Pattern(Display.getCurrent(), bounds.x, bounds.y, bounds.x + bounds.width, bounds.y + bounds.height, WHITE_COLOR, 0, WHITE_COLOR, 255);
