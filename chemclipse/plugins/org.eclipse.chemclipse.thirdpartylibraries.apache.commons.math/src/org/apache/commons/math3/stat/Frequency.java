 ...
 * </p>
 * <p>
 * Integer values (int, long, Integer, Long) are not distinguished by type -- i.e. <code>addValue(Long.valueOf(2)), addValue(2), addValue(2l)</code> all have the same effect (similarly for arguments to <code>getCount,</code> etc.).
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * char values are converted by <code>addValue</code> to Character instances. As such, these values are not comparable to integral values, so attempts to combine integral types with chars in a frequency distribution will fail.
 * </p>
 * <p>
 * Float is not coerced to Double. Since they are not Comparable with each other the user must do any necessary coercion. Float.NaN and Double.NaN are not treated specially; they may occur in input and will occur in output if appropriate. </b>
 * <p>
