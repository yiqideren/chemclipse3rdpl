 ...
 * using a method based on [1] with certain quick decisions for extreme values given in [2].
 * </p>
 * <p>
 * Two-sample tests are also supported, evaluating the null hypothesis that the two samples {@code x} and {@code y} come from the same underlying distribution. In this case, the test statistic is \(D_{n,m}=\sup_t | F_n(t)-F_m(t)|\) where \(n\) is the length of {@code x}, \(m\) is the length of {@code y}, \(F_n\) is the empirical distribution that puts mass \(1/n\) at each of the values in {@code x} and \(F_m\) is the empirical distribution of the {@code y} values. The default 2-sample test
 * method, {@link #kolmogorovSmirnovTest(double[], double[])} works as follows:
 * <ul>
 * <li>For very small samples (where the product of the sample sizes is less than {@value #SMALL_SAMPLE_PRODUCT}), the exact distribution is used to compute the p-value for the 2-sample test.</li>
 * <li>For mid-size samples (product of sample sizes greater than or equal to {@value #SMALL_SAMPLE_PRODUCT} but less than {@value #LARGE_SAMPLE_PRODUCT}), Monte Carlo simulation is used to compute the p-value. The simulation randomly generates partitions of \(m + n\) into an \(m\)-set and an \(n\)-set and reports the proportion that give \(D\) values exceeding the observed value.</li>
 * <li>When the product of the sample sizes exceeds {@value #LARGE_SAMPLE_PRODUCT}, the asymptotic distribution of \(D_{n,m}\) is used. See {@link #approximateP(double, int, int)} for details on the approximation.</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li>{@link #monteCarloP(double, int, int, boolean, int)} computes 2-sample p-values by Monte Carlo simulation</li>
 * <li>{@link #approximateP(double, int, int)} uses the asymptotic distribution The {@code boolean} arguments in the first two methods allow the probability used to estimate the p-value to be expressed using strict or non-strict inequality. See {@link #kolmogorovSmirnovTest(double[], double[], boolean)}.</li>
 * </ul>
 * </p>
 * <p>
 * References:
 * <ul>
 * <li>[1] <a href="http://www.jstatsoft.org/v08/i18/"> Evaluating Kolmogorov's Distribution</a> by George Marsaglia, Wai Wan Tsang, and Jingbo Wang</li>
 * <li>[2] <a href="http://www.jstatsoft.org/v39/i11/"> Computing the Two-Sided Kolmogorov-Smirnov Distribution</a> by Richard Simard and Pierre L'Ecuyer</li>
 * </ul>
 * <br/>
 * Note that [1] contains an error in computing h, refer to <a href="https://issues.apache.org/jira/browse/MATH-437">MATH-437</a> for details.
 * </p>
 *
 * @since 3.3
 */
public class KolmogorovSmirnovTest {
	 ...
	 * @return the p-value associated with the null hypothesis that {@code data} is a sample from {@code distribution}
	 ...
	 * <ul>
	 * <li>For very small samples (where the product of the sample sizes is less than {@value #SMALL_SAMPLE_PRODUCT}), the exact distribution is used to compute the p-value. This is accomplished by enumerating all partitions of the combined sample into two subsamples of the respective sample sizes, computing \(D_{n,m}\) for each partition and returning the proportion of partitions that give \(D\) values exceeding the observed value.</li>
	 ...
	 * @param y
	 *            second sample
	 * @return test statistic \(D_{n,m}\) used to evaluate the null hypothesis that {@code x} and {@code y} represent samples from the same underlying distribution
	 * @throws InsufficientDataException
	 *             if either {@code x} or {@code y} does not have length at
	 *             least 2
	 * @throws NullArgumentException
	 ...
	 * @return true iff the null hypothesis that {@code data} is a sample from {@code distribution} can be rejected with confidence 1 - {@code alpha}
	 * @throws InsufficientDataException
	 *             if {@code data} does not have length at least 2
	 * @throws NullArgumentException
	 *             if {@code data} is null
	 ...
	 *             if algorithm fails to convert {@code h} to a {@link org.apache.commons.math3.fraction.BigFraction} in expressing {@code d} as \((k
	 ...
	 * @return \(P(D_n < d)\)
	 * @throws MathArithmeticException
	 *             if the algorithm fails to convert {@code h} to a {@link org.apache.commons.math3.fraction.BigFraction} in expressing {@code d} as \((k
	 *             - h) / m\) for integer {@code k, m} and \(0 \le h < 1\)
	 */
	 ...
	 *            sample size
	 * @param exact
	 *            whether the probability should be calculated exact using {@link org.apache.commons.math3.fraction.BigFraction} everywhere at the expense of
	...
	public double pelzGood(double d, int n) {
		...
		for(k = 0; k < MAXIMUM_PARTIAL_SUM_COUNT; k++) {
			kTerm = k + 0.5;
			kTerm2 = kTerm * kTerm;
			increment = (z6Term + z4Term + PI_SQUARED * (z4Term - z2Term) * kTerm2 + pi4 * (1 - twoZ2) * kTerm2 * kTerm2) * FastMath.exp(-PI_SQUARED * kTerm2 / twoZ2);
			sum += increment;
			if(FastMath.abs(increment) < PG_SUM_RELATIVE_ERROR * FastMath.abs(sum)) {
				break;
			}
		...
		for(k = 0; k < MAXIMUM_PARTIAL_SUM_COUNT; k++) {
			...
			increment = (pi6 * kTerm6 * (5 - 30 * z2) + pi4 * kTerm4 * (-60 * z2 + 212 * z4) + PI_SQUARED * kTerm2 * (135 * z4 - 96 * z6) - 30 * z6 - 90 * z8) * FastMath.exp(-PI_SQUARED * kTerm2 / twoZ2);
			sum += increment;
			if(FastMath.abs(increment) < PG_SUM_RELATIVE_ERROR * FastMath.abs(sum)) {
				break;
			}
	 ...
	 * <p>
	 * The returned probability is exact, obtained by enumerating all partitions of {@code m + n} into {@code m} and {@code n} sets, computing \(D_{n,m}\) for each partition and counting the number of partitions that yield \(D_{n,m}\) values exceeding (resp. greater than or equal to) {@code d}.
	 * </p>
	 * <p>
	 * <strong>USAGE NOTE</strong>: Since this method enumerates all combinations in \({m+n} \choose {n}\), it is very slow if called for large {@code m, n}. For this reason, {@link #kolmogorovSmirnovTest(double[], double[])} uses this only for {@code m * n < } {@value #SMALL_SAMPLE_PRODUCT}.
	 * </p>
	 *
	 * @param d
	 *            D-statistic value
	 ...
	 * <p>
	 * <strong>Precondition:</strong> The first {@code n} elements of {@code nSetI} must be sorted in ascending order.
	 * </p>
	 *
	 * @param nSet
