 ...
 *
 * <p>
 * Since events are only problem-dependent and are triggered by the independent <i>time</i> variable and the state vector, they can occur at virtually any time, unknown in advance. The integrators will take care to avoid sign changes inside the steps, they will reduce the step size when such an event is detected in order to put this event exactly at the end of the current step. This guarantees that step interpolation (which always has a one step scope) is relevant even in presence of
 * discontinuities. This is independent from the stepsize control provided by integrators that monitor the local error (this event handling feature is available for all integrators, including fixed step ones).
 * </p>
 *
 * @since 1.2
 */
public interface EventHandler {
	...
	public enum Action {
		 ...
		 * <p>
		 * This value should be used as the return value of the {@link #eventOccurred eventOccurred} method when the integration should go on after the event ending the current step, with a new state vector (which will be retrieved thanks to the {@link #resetState
		 * resetState} method).
		 * </p>
		 */
		RESET_STATE,
	 ...
	 * </p>
	 * <p>
	 * Also note that the integrator expect that once an event has occurred, the sign of the switching function at the start of the next step (i.e. just after the event) is the opposite of the sign just before the event. This consistency between the steps <string>must</strong> be preserved, otherwise {@link org.apache.commons.math3.exception.NoBracketingException
	 * exceptions} related to root not being bracketed will occur.
	 * </p>
	 * <p>
	 * This need for consistency is sometimes tricky to achieve. A typical example is using an event to model a ball bouncing on the floor. The first idea to represent this would be to have {@code g(t) = h(t)} where h is the height above the floor at time {@code t}. When {@code g(t)} reaches 0, the ball is on the floor, so it should bounce and the typical way to do this is to reverse its vertical velocity. However, this would mean that before the event {@code g(t)} was decreasing from positive
	 * values to 0, and after the event {@code g(t)} would be increasing from 0 to positive values again. Consistency is broken here! The solution here is to have {@code g(t) = sign
	 ...
	 * resetState} method will be called once the step handler has finished its task, and the integrator will also recompute the derivatives,</li>
