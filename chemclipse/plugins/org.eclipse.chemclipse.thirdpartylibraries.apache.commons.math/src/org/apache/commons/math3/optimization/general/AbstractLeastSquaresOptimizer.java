 ...
 * Jacobian and error estimation. <br/>
 * This class constructs the Jacobian matrix of the function argument in method {@link BaseAbstractMultivariateVectorOptimizer#optimize(int, org.apache.commons.math3.analysis.MultivariateVectorFunction,OptimizationData[])
...
public abstract class AbstractLeastSquaresOptimizer extends BaseAbstractMultivariateVectorOptimizer<DifferentiableMultivariateVectorFunction> implements DifferentiableMultivariateVectorOptimizer {
	 ...
	 * @deprecated As of 3.1. Please use {@link #computeWeightedJacobian(double[])} instead.
	 */
	@Deprecated
	 ...
	 * criterion that is minimized by the optimizer as follows: if <em>c</em> if the criterion, and <em>n</em> is the number of
	...
	/**
	 * <p>
	 * Returns an estimate of the standard deviation of each parameter. The returned values are the so-called (asymptotic) standard errors on the parameters, defined as {@code sd(a[i]) = sqrt(S / (n - m) * C[i][i])}, where {@code a[i]} is the optimized value of the {@code i}-th parameter, {@code S} is the minimized value of the sum of squares objective function (as returned by {@link #getChiSquare()}), {@code n} is the number of observations, {@code m} is the number of parameters and {@code C}
	 * is the covariance matrix.
	 * </p>
	 * <p>
	 * See also <a href="http://en.wikipedia.org/wiki/Least_squares">Wikipedia</a>, or <a href="http://mathworld.wolfram.com/LeastSquaresFitting.html">MathWorld</a>, equations (34) and (35) for a particular case.
	 * </p>
	 *
	 ...
	 * The cost function to be minimized is <code>&sum;weight<sub>i</sub>(objective<sub>i</sub> - target<sub>i</sub>)<sup>2</sup></code>
	 *
	 * @param f
	 *            Objective function.
	 * @param target
	 ...
	 * @throws DimensionMismatchException
	 *             if the target, and weight arguments
