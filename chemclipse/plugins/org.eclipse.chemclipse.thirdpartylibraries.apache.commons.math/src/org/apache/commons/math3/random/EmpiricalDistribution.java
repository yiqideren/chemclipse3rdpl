...
/**
 * <p>
 * Represents an <a href="http://http://en.wikipedia.org/wiki/Empirical_distribution_function"> empirical probability distribution</a> -- a probability distribution derived from observed data without making any assumptions about the functional form of the population distribution that the data come from.
 * </p>
 *
 * <p>
 * An <code>EmpiricalDistribution</code> maintains data structures, called <i>distribution digests</i>, that describe empirical distributions and support the following operations:
 * <ul>
 ...
 * </ul>
 * Applications can use <code>EmpiricalDistribution</code> to build grouped frequency histograms representing the input data or to generate random values "like" those in the input file -- i.e., the values generated will follow the distribution of the values in the file.
 * </p>
 *
 * <p>
 * The implementation uses what amounts to the <a href="http://nedwww.ipac.caltech.edu/level5/March02/Silverman/Silver2_6.html"> Variable Kernel Method</a> with Gaussian smoothing:
 * <p>
 ...
 *
 * <p>
 * EmpiricalDistribution implements the {@link RealDistribution} interface as follows. Given x within the range of values in the dataset, let B be the bin containing x and let K be the within-bin kernel for B. Let P(B-) be the sum of the probabilities of the bins below B and let K(B) be the mass of B under K (i.e., the integral of the kernel density over B). Then set P(X < x) = P(B-) + P(B) * K(x) / K(B) where K(x) is the kernel distribution evaluated at x. This results in a cdf that matches the
 * grouped frequency distribution at the bin endpoints and interpolates within bins using within-bin kernels.
 * </p>
 *
 * <strong>USAGE NOTES:</strong>
 * <ul>
 * <li>The <code>binCount</code> is set by default to 1000. A good rule of thumb is to set the bin count to approximately the length of the input file divided by 10.</li>
...
public class EmpiricalDistribution extends AbstractRealDistribution {
	 ...
	 *             unless a {@code load} method has been
	 *             called beforehand.
	 */
	 ...
	 * <li>Compute K(B) = the mass of B with respect to the within-bin kernel (i.e., the integral of the kernel density over B).</li>
	 ...
	 * <li>Compute K(B) = the probability mass of B with respect to the within-bin kernel and K(B-) = the kernel distribution evaluated at the lower endpoint of B</li>
	 * <li>Return P(B-) + P(B) * [K(x) - K(B-)] / K(B) where K(x) is the within-bin kernel distribution function evaluated at x.</li>
	 * </ol>
