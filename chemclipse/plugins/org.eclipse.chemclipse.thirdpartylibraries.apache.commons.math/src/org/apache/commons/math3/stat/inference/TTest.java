 ...
 * </p>
 * <p>
 * Test statistics are available for all tests. Methods including "Test" in in their names perform tests, all other methods return t-statistics. Among the "Test" methods, <code>double-</code>valued methods return p-values; <code>boolean-</code>valued methods perform fixed significance level tests. Significance levels are always specified as numbers between 0 and 0.5 (e.g. tests at the 95% level use <code>alpha=0.05</code>).
 * </p>
 * <p>
...
public class TTest {
	 ...
	 * </p>
	 * <p>
	 * This test is equivalent to a one-sample t-test computed using {@link #tTest(double, double[])} with <code>mu = 0</code> and the sample array consisting of the signed differences between corresponding elements of <code>sample1</code> and <code>sample2.</code>
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Returns <code>true</code> iff the null hypothesis can be rejected with confidence <code>1 - alpha</code>. To perform a 1-sided test, use <code>alpha * 2</code>
	 * </p>
	 * <p>
	 * <strong>Usage Note:</strong><br>
	 ...
	 * <strong>Preconditions</strong>:
	 * <ul>
	 * <li><code>observed.getN() &ge; 2</code>.</li>
	 * </ul>
	 * </p>
	 *
	 * @param mu
	 ...
	 * @return t statistic
	 * @throws NullArgumentException
	 *             if <code>sampleStats</code> is <code>null</code>
	 * @throws NumberIsTooSmallException
	 ...
	 * </p>
	 * <p>
	 * where <strong><code>n1</code></strong> is the size of first sample; <strong><code> n2</code></strong> is the size of second sample; <strong><code> m1</code></strong> is the mean of first sample; <strong><code> m2</code></strong> is the mean of second sample</li> </ul> and <strong><code>var</code></strong> is the pooled variance estimate:
	 * </p>
	 * <p>
	 * <code>var = sqrt(((n1 - 1)var1 + (n2 - 1)var2) / ((n1-1) + (n2-1)))</code>
	 * </p>
	 * <p>
	 * with <strong><code>var1</code></strong> the variance of the first sample and <strong><code>var2</code></strong> the variance of the second sample.
	 * </p>
	 * <p>
	 * <strong>Preconditions</strong>:
	 * <ul>
	 ...
	 * </p>
	 * <p>
	 * where <strong><code>n1</code></strong> is the size of the first sample <strong><code> n2</code></strong> is the size of the second sample; <strong><code> m1</code></strong> is the mean of the first sample; <strong><code> m2</code></strong> is the mean of the second sample; <strong><code> var1</code></strong> is the variance of the first sample; <strong><code> var2</code></strong> is the variance of the second sample;
	 * </p>
	 * <p>
	 * <strong>Preconditions</strong>:
	 * <ul>
	 ...
	 * </p>
	 * <p>
	 * where <strong><code>n1</code></strong> is the size of first sample; <strong><code> n2</code></strong> is the size of second sample; <strong><code> m1</code></strong> is the mean of first sample; <strong><code> m2</code></strong> is the mean of second sample and <strong><code>var</code></strong> is the pooled variance estimate:
	 * </p>
	 * <p>
	 * <code>var = sqrt(((n1 - 1)var1 + (n2 - 1)var2) / ((n1-1) + (n2-1)))</code>
	 * </p>
	 ...
	 * <p>
	 * The number returned is the smallest significance level at which one can reject the null hypothesis that the mean equals <code>mu</code> in favor of the two-sided alternative that the mean is different from <code>mu</code>. For a one-sided test, divide the returned value by 2.
	 * </p>
	 * <p>
	 * <strong>Usage Note:</strong><br>
	 * The validity of the test depends on the assumptions of the parametric t-test procedure, as discussed <a href="http://www.basic.nwu.edu/statguidefiles/ttest_unpaired_ass_viol.html">here</a>
	 ...
	 * <li>To test the (one-sided) hypothesis <code> sample mean < mu </code> at the 99% level, first verify that the measured sample mean is less than <code>mu</code> and then use <br>
	 * <code>tTest(mu, sample, 0.02) </code></li>
	 * </ol>
	 * </p>
	 * <p>
	 * <strong>Usage Note:</strong><br>
	 * The validity of the test depends on the assumptions of the one-sample parametric t-test procedure, as discussed <a href="http://www.basic.nwu.edu/statguidefiles/sg_glos.html#one-sample">here</a>
	 * </p>
	 * <p>
	 * <strong>Preconditions</strong>:
	 * <ul>
	 ...
	 * use {@link #homoscedasticTTest(double[], double[])}.
	 * </p>
	 * <p>
	 * <strong>Usage Note:</strong><br>
	 * The validity of the p-value depends on the assumptions of the parametric t-test procedure, as discussed <a href="http://www.basic.nwu.edu/statguidefiles/ttest_unpaired_ass_viol.html"> here</a>
	 ...
	 * <p>
	 * Returns <code>true</code> iff the null hypothesis that the means are equal can be rejected with confidence <code>1 - alpha</code>. To perform a 1-sided test, use <code>alpha * 2</code>
	 * </p>
	 * <p>
	 * See {@link #t(double[], double[])} for the formula used to compute the t-statistic. Degrees of freedom are approximated using the <a href="http://www.itl.nist.gov/div898/handbook/prc/section3/prc31.htm"> Welch-Satterthwaite approximation.</a>
	 * </p>
	 * <p>
	 * <strong>Examples:</strong><br>
	 * <ol>
	 ...
	 * <li>To test the (one-sided) hypothesis <code> mean 1 < mean 2 </code>, at the 99% level, first verify that the measured mean of <code>sample 1</code> is less than the mean of <code>sample 2</code> and then use <br>
	 ...
	 * <p>
	 * Returns <code>true</code> iff the null hypothesis that the means are equal can be rejected with confidence <code>1 - alpha</code>. To perform a 1-sided test, use <code>alpha * 2.</code> To perform the test without the assumption of equal subpopulation variances, use {@link #tTest(double[], double[], double)}.
	 * </p>
	 * <p>
