...
public class RegionFactory<S extends Space> {
	...
	private BSPTree<S> recurseComplement(final BSPTree<S> node, final Map<BSPTree<S>, BSPTree<S>> map) {
		...
		if(node.getCut() == null) {
			...
			transformedNode = new BSPTree<S>(node.getCut().copySelf(), recurseComplement(node.getPlus(), map), recurseComplement(node.getMinus(), map), attribute);
		}
	...
	private class DifferenceMerger implements BSPTree.LeafMerger<S>, VanishingCutHandler<S> {
		...
		public BSPTree<S> fixNode(final BSPTree<S> node) {
			...
			return new BSPTree<S>(region1.checkPoint(p) == Location.INSIDE && region2.checkPoint(p) == Location.OUTSIDE);
		}
	}
