 ...
 * 
 * <p>
 * A line can also be used to completely define a reference frame in the plane. It is sufficient to select one specific point in the line (the orthogonal projection of the original reference frame on the line) and to use the unit vector in the line direction and the orthogonal vector oriented from left half plane to right half plane. We define two coordinates by the process, the <em>abscissa</em> along the line, and the <em>offset</em> across the line. All points of the plane are uniquely
 * identified by these two coordinates. The line is the set of points at zero offset, the left half plane is the set of points with negative offsets and the right half plane is the set of points with positive offsets.
 * </p>
 * 
 * @since 3.0
 */
public class Line implements Hyperplane<Euclidean2D>, Embedding<Euclidean2D, Euclidean1D> {
	...
	public Line getReverse() {
		...
		if(reverse == null) {
			reverse = new Line((angle < FastMath.PI) ? (angle + FastMath.PI) : (angle - FastMath.PI), -cos, -sin, -originOffset, tolerance);
	...
	public Vector2D intersection(final Line other) {
		...
		return new Vector2D(MathArrays.linearCombination(cos, other.originOffset, -other.cos, originOffset) / d, MathArrays.linearCombination(sin, other.originOffset, -other.sin, originOffset) / d);
	}
	...
	public double getOffset(final Line line) {
		...
		return originOffset + (MathArrays.linearCombination(cos, line.cos, sin, line.sin) > 0 ? -line.originOffset : line.originOffset);
	}
