 ...
 * </p>
 * <h3>Important note: Usage should not assume that this class is thread-safe even though some of the methods are {@code synchronized}. This qualifier will be dropped in the next major release (4.0).</h3>
 * <p>
 * The internal storage array starts with capacity determined by the {@code initialCapacity} property, which can be set by the constructor. The default initial capacity is 16. Adding elements using {@link #addElement(double)} appends elements to the end of the array. When there are no open entries at the end of the internal storage array, the array is expanded. The size of the expanded array depends on the {@code expansionMode} and {@code expansionFactor} properties. The {@code expansionMode}
 * determines whether the size of the array is multiplied by the {@code expansionFactor} ({@link ExpansionMode#MULTIPLICATIVE}) or if the expansion is additive ({@link ExpansionMode#ADDITIVE} -- {@code expansionFactor} storage locations added). The default {@code expansionMode} is {@code MULTIPLICATIVE} and the default {@code expansionFactor} is 2.
 * </p>
 * <p>
 * The {@link #addElementRolling(double)} method adds a new element to the end of the internal storage array and adjusts the "usable window" of the internal array forward by one position (effectively making what was the second element the first, and so on). Repeated activations of this method (or activation of {@link #discardFrontElements(int)}) will effectively orphan the storage locations at the beginning of the internal storage array. To reclaim this storage, each time one of these methods is
 * activated, the size of the internal storage array is compared to the number of addressable elements (the {@code numElements} property) and if the difference is too large, the internal array is contracted to size {@code numElements + 1}. The determination of when the internal storage array is "too large" depends on the {@code expansionMode} and {@code contractionFactor} properties. If the {@code expansionMode} is {@code MULTIPLICATIVE}, contraction is triggered when the ratio between storage
 * array length and {@code numElements} exceeds {@code contractionFactor.} If the {@code expansionMode} is {@code ADDITIVE}, the number of excess storage locations is compared to {@code contractionFactor}.
 * </p>
 * <p>
...
public class ResizableDoubleArray implements DoubleArray, Serializable {
	 ...
	 * @throws MathIllegalArgumentException
	 *             if parameters are not valid
	 * @deprecated As of 3.1. Please use {@link #ResizableDoubleArray(int,double,double,ExpansionMode,double[])} instead.
	 */
	@Deprecated
	 ...
	 * if the array contains the elements 1,2,3,4, invoking <code>discardFrontElements(2)</code> will cause the first two elements
	 ...
	 * if the array contains the elements 1,2,3,4, invoking <code>discardMostRecentElements(2)</code> will cause the last two elements
	 ...
	 * For example, if the array contains the elements 1,2,3,4, invoking <code>discardExtremeElements(2,false)</code> will cause the last two elements
	 * to be discarded, leaving 1,2 in the array.
	 * For example, if the array contains the elements 1,2,3,4, invoking <code>discardExtremeElements(2,true)</code> will cause the first two elements
	 ...
	 * <p>
	 * if <code>expansionMode</code> is set to MULTIPLICATIVE_MODE, the new array size will be <code>internalArray.length * expansionFactor.</code> If <code>expansionMode</code> is set to ADDITIVE_MODE, the length after expansion will be <code>internalArray.length + expansionFactor</code>
	 * </p>
	 */
	 ...
	 * needs to be expanded. The <code>expansionMode</code> determines whether the size of the array is multiplied by the <code>expansionFactor</code> (MULTIPLICATIVE_MODE) or if
	 ...
	 *
	 * @param expansionMode
	 *            Expansion mode to use for resizing the array.
	 * @deprecated As of 3.1 (to be removed in 4.0 as field will become "final").
	 */
	@Deprecated
