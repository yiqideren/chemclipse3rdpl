...
public class GTest {
	 ...
	 *
	 * <p>
	 * <strong>Note:</strong>This implementation rescales the {@code expected} array if necessary to ensure that the sum of the expected and observed counts are equal.
	 * </p>
	 *
	 * @param observed
	...
	public double gTestIntrinsic(final double[] expected, final long[] observed) throws NotPositiveException, NotStrictlyPositiveException, DimensionMismatchException, MaxCountExceededException {
		...
		// pass a null rng to avoid unneeded overhead as we will not sample from this distribution
		final ChiSquaredDistribution distribution = new ChiSquaredDistribution(null, expected.length - 2.0);
		return 1.0 - distribution.cumulativeProbability(g(expected, observed));
	}
	 ...
	 *
	 * <p>
	 * where {@code H} is the <a href="http://en.wikipedia.org/wiki/Entropy_%28information_theory%29"> Shannon Entropy</a> of the random variable formed by viewing the elements of the argument array as incidence counts; <br/>
	 ...
	 * <li>The arrays {@code observed1} and {@code observed2} must have the same length and their common length must be at least 2.</li>
	 * </ul>
	 * </p>
	 *
	 * <p>
	...
	/**
	 * <p>
	 * Returns the <i>observed significance level</i>, or <a href= "http://www.cas.lancs.ac.uk/glossary_v1.1/hyptest.html#pvalue"> p-value</a>, associated with a G-Value (Log-Likelihood Ratio) for two sample test comparing bin frequency counts in {@code observed1} and {@code observed2}.
	 * </p>
	 *
	 * <p>
	 ...
	 * @throws ZeroException
	 *             if either all counts of {@code observed1} or {@code observed2} are zero, or if the count at some index is
