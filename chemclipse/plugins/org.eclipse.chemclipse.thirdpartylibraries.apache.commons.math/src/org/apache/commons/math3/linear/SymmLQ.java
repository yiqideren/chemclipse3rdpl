...
/**
 * <p>
 * Implementation of the SYMMLQ iterative linear solver proposed by <a href="#PAIG1975">Paige and Saunders (1975)</a>. This implementation is largely based on the FORTRAN code by Pr. Michael A. Saunders, available <a href="http://www.stanford.edu/group/SOL/software/symmlq/f77/">here</a>.
 * </p>
 * <p>
 * SYMMLQ is designed to solve the system of linear equations A &middot; x = b where A is an n &times; n self-adjoint linear operator (defined as a {@link RealLinearOperator}), and b is a given vector. The operator A is not required to be positive definite. If A is known to be definite, the method of conjugate gradients might be preferred, since it will require about the same number of iterations as SYMMLQ but slightly less work per iteration.
 * </p>
 * <p>
 * SYMMLQ is designed to solve the system (A - shift &middot; I) &middot; x = b, where shift is a specified scalar value. If shift and b are suitably chosen, the computed vector x may approximate an (unnormalized) eigenvector of A, as in the methods of inverse iteration and/or Rayleigh-quotient iteration. Again, the linear operator (A - shift &middot; I) need not be positive definite (but <em>must</em> be self-adjoint). The work per iteration is very slightly less if shift = 0.
 * </p>
 * <h3>Preconditioning</h3>
 * <p>
 * Preconditioning may reduce the number of iterations required. The solver may be provided with a positive definite preconditioner M = P<sup>T</sup> &middot; P that is known to approximate (A - shift &middot; I)<sup>-1</sup> in some sense, where matrix-vector products of the form M &middot; y = x can be computed efficiently. Then SYMMLQ will implicitly solve the system of equations P &middot; (A - shift &middot; I) &middot; P<sup>T</sup> &middot; x<sub>hat</sub> = P &middot; b, i.e.
 * A<sub>hat</sub> &middot; x<sub>hat</sub> = b<sub>hat</sub>, where A<sub>hat</sub> = P &middot; (A - shift &middot; I) &middot; P<sup>T</sup>, b<sub>hat</sub> = P &middot; b, and return the solution x = P<sup>T</sup> &middot; x<sub>hat</sub>. The associated residual is r<sub>hat</sub> = b<sub>hat</sub> - A<sub>hat</sub> &middot; x<sub>hat</sub> = P &middot; [b - (A - shift &middot; I) &middot; x] = P &middot; r.
 * </p>
 * <p>
 * In the case of preconditioning, the {@link IterativeLinearSolverEvent}s that this solver fires are such that {@link IterativeLinearSolverEvent#getNormOfResidual()} returns the norm of the <em>preconditioned</em>, updated residual, ||P &middot; r||, not the norm of the <em>true</em> residual ||r||.
 * </p>
 * <h3><a id="stopcrit">Default stopping criterion</a></h3>
 * <p>
 * A default stopping criterion is implemented. The iterations stop when || rhat || &le; &delta; || Ahat || || xhat ||, where xhat is the current estimate of the solution of the transformed system, rhat the current estimate of the corresponding residual, and &delta; a user-specified tolerance.
 * </p>
 * <h3>Iteration count</h3>
 * <p>
 * In the present context, an iteration should be understood as one evaluation of the matrix-vector product A &middot; x. The initialization phase therefore counts as one iteration. If the user requires checks on the symmetry of A, this entails one further matrix-vector product in the initial phase. This further product is <em>not</em> accounted for in the iteration count. In other words, the number of iterations required to reach convergence will be identical, whether checks have been required
 * or not.
 * </p>
 * <p>
 * The present definition of the iteration count differs from that adopted in the original FOTRAN code, where the initialization phase was <em>not</em> taken into account.
 * </p>
 ...
 * </ul>
 * should not be considered as an initial guess, as it is set to zero in the initial phase. If x<sub>0</sub> is known to be a good approximation to x, one should compute r<sub>0</sub> = b - A &middot; x, solve A &middot; dx = r0, and set x = x<sub>0</sub> + dx.
 * </p>
 ...
 * <li>key {@code "vector2"} points to the second offending vector, say y, such that x<sup>T</sup> &middot; L &middot; y &ne; y<sup>T</sup> &middot; L &middot; x (within a certain accuracy).</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li>key {@code "vector"}, which points to the offending vector, say x, such that x<sup>T</sup> &middot; L &middot; x < 0.</li>
 * </ul>
 * </p>
 * <h3>References</h3>
 * <dl>
 ...
 * Solution of Sparse Indefinite Systems of Linear Equations</em></a>, SIAM Journal on Numerical Analysis 12(4): 617-629, 1975</dd>
 * </dl>
 *
 * @since 3.0
 */
public class SymmLQ extends PreconditionedIterativeLinearSolver {
	 ...
	 * </p>
	 * <p>
	 * {@code shift} should be zero if the system A &middot; x = b is to be solved. Otherwise, it could be an approximation to an eigenvalue of A, such as the Rayleigh quotient b<sup>T</sup> &middot; A &middot; b / (b<sup>T</sup> &middot; b) corresponding to the vector b. If b is sufficiently like an eigenvector corresponding to an eigenvalue near shift, then the computed x may have very large components. When normalized, x may be closer to an eigenvector than b.
	 * </p>
	 *
	 * @param a
	 *            the linear operator A of the system
