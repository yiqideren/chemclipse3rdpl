 ...
 *
 * <p>
 * Since the specification of the {@code Comparator} interface allows only {@code ClassCastException} errors, some arbitrary choices have been made to handle specific cases. The rationale for these choices is to keep <em>regular</em> and consistent T-uples together.
 * </p>
 * <ul>
 * <li>instances with different dimensions are sorted according to their dimension regardless of their components values</li>
 * <li>instances with {@code Double.NaN} components are sorted after all other ones (even after instances with positive infinite components</li>
...
public class OrderedTuple implements Comparable<OrderedTuple> {
	...
	private static long mantissa(final long bits) {
		...
		return ((bits & EXPONENT_MASK) == 0) ? ((bits & MANTISSA_MASK) << 1) : // subnormal number
