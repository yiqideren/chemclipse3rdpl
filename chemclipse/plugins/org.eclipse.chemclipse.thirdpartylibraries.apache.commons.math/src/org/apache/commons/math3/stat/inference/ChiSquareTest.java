 ...
 *
 * <p>
 * Two samples tests can be used when the distribution is unknown <i>a priori</i> but provided by one sample, or when the hypothesis under test is that the two samples come from the same underlying distribution.
 * </p>
 *
 */
public class ChiSquareTest {
	 ...
	 * </p>
	 * <p>
	 * <strong>Note: </strong>This implementation rescales the <code>expected</code> array if necessary to ensure that the sum of the expected and observed counts are equal.
	 * </p>
	 *
	 * @param observed
	 ...
	 * <p>
	 * <strong>Example:</strong><br>
	 * To test the null hypothesis that the counts in <code>count[0], ... , count[count.length - 1] </code> all correspond to the same underlying probability distribution at the 99% level, use
	 * </p>
	 * <p>
	 ...
	 * <li>The arrays <code>observed1</code> and <code>observed2</code> must have the same length and their common length must be at least 2.</li>
	 * </ul>
	 * </p>
	 * <p>
	 * If any of the preconditions are not met, an <code>IllegalArgumentException</code> is thrown.
	 ...
	 *             if either all counts of <code>observed1</code> or <code>observed2</code> are zero, or if the count at some index is zero
	 *             for both arrays
	 * @since 1.2
	 */
	...
	/**
	 * <p>
	 * Returns the <i>observed significance level</i>, or <a href= "http://www.cas.lancs.ac.uk/glossary_v1.1/hyptest.html#pvalue"> p-value</a>, associated with a Chi-Square two sample test comparing bin frequency counts in <code>observed1</code> and <code>observed2</code>.
	 * </p>
	 * <p>
	 ...
	 *             if any entries in <code>observed1</code> or <code>observed2</code> are negative
	 * @throws ZeroException
	 *             if either all counts of <code>observed1</code> or <code>observed2</code> are zero, or if the count at the same index is zero
	 *             for both arrays
	 * @throws MaxCountExceededException
	 *             if an error occurs computing the p-value
	 * @since 1.2
	...
	public double chiSquareTestDataSetsComparison(long[] observed1, long[] observed2) throws DimensionMismatchException, NotPositiveException, ZeroException, MaxCountExceededException {
		...
		// pass a null rng to avoid unneeded overhead as we will not sample from this distribution
		final ChiSquaredDistribution distribution = new ChiSquaredDistribution(null, (double)observed1.length - 1);
