...
public class SpearmansCorrelation {
	 ...
	 * <p>
	 * From version 4.0 onwards this constructor will throw an exception if the provided {@link NaturalRanking} uses a {@link NaNStrategy#REMOVED} strategy.
	 *
	 ...
	 * ranked input data. That is, <code>new SpearmansCorrelation(matrix).getRankCorrelation()</code> is equivalent to <code>new PearsonsCorrelation(rankTransform(matrix))</code> where <code>rankTransform(matrix)</code> is the result of applying the
	...
	/**
	 * Applies rank transform to each of the columns of <code>matrix</code> using the current <code>rankingAlgorithm</code>.
	 *
	 * @param matrix
	 *            matrix to transform
	 ...
	 *            the input array
	 * @param indices
	 *            a set containing the indices to be removed
