...
public class LeastSquaresFactory {
	...
	/**
	 * Create a {@link org.apache.commons.math3.fitting.leastsquares.LeastSquaresProblem} from the given elements. There will be no weights applied (unit weights).
	 *
	 * @param model
	 *            the model function. Produces the computed values.
	 * @param observed
	 ...
	 * @param maxIterations
	 *            the maximum number to times to iterate in the algorithm
	 * @param lazyEvaluation
	 *            Whether the call to {@link Evaluation#evaluate(RealVector)} will defer the evaluation until access to the value is requested.
	 ...
	 * Create a {@link org.apache.commons.math3.fitting.leastsquares.LeastSquaresProblem} from the given elements.
	 * <p>
	 * This factory method is provided for continuity with previous interfaces. Newer applications should use {@link #create(MultivariateJacobianFunction, RealVector, RealVector, ConvergenceChecker, int, int)}, or {@link #create(MultivariateJacobianFunction, RealVector, RealVector, RealMatrix, ConvergenceChecker, int, int)}.
	 *
	 * @param model
	 *            the model function. Produces the computed values.
	 * @param jacobian
	 ...
	 * @return a new {@link LeastSquaresProblem} with the weights applied. The original {@code problem} is not modified.
	 */
	 ...
	 *
	 * @param checker
	 *            the convergence checker to adapt.
	...
	private static class LocalValueAndJacobianFunction implements ValueAndJacobianFunction {
		...
		public Pair<RealVector, RealMatrix> value(final RealVector point) {
			...
			// Evaluate.
			return new Pair<RealVector, RealMatrix>(computeValue(p), computeJacobian(p));
		}
	...
	private static class LocalLeastSquaresProblem extends AbstractOptimizationProblem<Evaluation> implements LeastSquaresProblem {
		...
		public Evaluation evaluate(final RealVector point) {
			...
			final RealVector p = paramValidator == null ? point.copy() : paramValidator.validate(point.copy());
			if(lazyEvaluation) {
		...
		private static class UnweightedEvaluation extends AbstractEvaluation {
			 ...
			 * @param jacobian
			 *            the computed function Jacobian
			 * @param target
			 *            the observed values
			 * @param point
			 *            the abscissa
