 ...
 * <ul>
 * <li>The <code>getResult</code> method computes the variance using updating formulas based on West's algorithm, as described in <a href="http://doi.acm.org/10.1145/359146.359152"> Chan, T. F. and J. G. Lewis 1979, <i>Communications of the ACM</i>, vol. 22 no. 9, pp. 526-531.</a></li>
 ...
 * </ul>
 * Note that adding values using <code>increment</code> or <code>incrementAll</code> and then executing <code>getResult</code> will sometimes give a different, less accurate, result than executing <code>evaluate</code> with the full array of values. The former approach should only be used when the full array of values is not available.
 * </p>
 * <p>
 * The "population variance" ( sum((x_i - mean)^2) / n ) can also be computed using this statistic. The <code>isBiasCorrected</code> property determines whether the "population" or "sample" value is returned by the <code>evaluate</code> and <code>getResult</code> methods. To compute population variances, set this property to <code>false.</code>
 * </p>
 * <p>
...
public class Variance extends AbstractStorelessUnivariateStatistic implements Serializable, WeightedEvaluation {
	 ...
	 * {@inheritDoc}
	 * <p>
	 * If all values are available, it is more accurate to use {@link #evaluate(double[])} rather than adding values one at a time using this method and then executing {@link #getResult}, since <code>evaluate</code> leverages the fact that is has the full list of values together to execute a two-pass algorithm. See {@link Variance}.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * <p>
	 * Returns the weighted variance of the entries in the specified portion of the input array, or <code>Double.NaN</code> if the designated subarray is empty.
	 * </p>
	 * <p>
	 * Uses the formula
	 * 
	 ...
	 * </p>
	 * <p>
	 * If <code>isBiasCorrected</code> is <code>true</code> the formula used assumes that the supplied mean value is the arithmetic mean of the sample data, not a known population parameter. If the mean is a known population parameter, or if the "population" version of the variance is desired, set <code>isBiasCorrected</code> to <code>false</code> before invoking this method.
	 * </p>
	 * <p>
	 * Returns 0 for a single-value (i.e. length = 1) sample.
	 * </p>
	 ...
	 * the input array, using the precomputed weighted mean value. Returns <code>Double.NaN</code> if the designated subarray is empty.
	 * <p>
	 * Uses the formula
	 * 
