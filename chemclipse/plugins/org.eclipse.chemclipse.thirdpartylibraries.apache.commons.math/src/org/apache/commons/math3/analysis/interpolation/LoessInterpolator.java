 ...
 * <p/>
 * For reference, see <a href="http://www.math.tau.ac.il/~yekutiel/MA seminar/Cleveland 1979.pdf"> William S. Cleveland - Robust Locally Weighted Regression and Smoothing Scatterplots</a>
 * <p/>
...
public class LoessInterpolator implements UnivariateInterpolator, Serializable {
	 ...
	 *            a two-element array {left, right} such that: {@code (left==0 or xval[i] - xval[left-1] > xval[right] - xval[i])} and {@code (right==xval.length-1 or xval[right+1] - xval[i] > xval[i] - xval[left])}.
