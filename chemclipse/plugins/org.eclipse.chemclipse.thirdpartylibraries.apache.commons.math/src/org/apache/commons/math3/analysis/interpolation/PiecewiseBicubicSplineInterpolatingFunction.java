 ...
 * insufficient data will raise an exception when the {@link #value(double,double) value} method is called.
 *
 * @since 3.4
 */
public class PiecewiseBicubicSplineInterpolatingFunction implements BivariateFunction {
	...
	public PiecewiseBicubicSplineInterpolatingFunction(double[] x, double[] y, double[][] f) throws DimensionMismatchException, NullArgumentException, NoDataException, NonMonotonicSequenceException {
		...
		if(xLen < MIN_NUM_POINTS || yLen < MIN_NUM_POINTS || f.length < MIN_NUM_POINTS || f[0].length < MIN_NUM_POINTS) {
	 ...
	 *            Coordinate samples.
	 * @param offset
	 *            how far back from found value to offset for querying
	 * @param count
