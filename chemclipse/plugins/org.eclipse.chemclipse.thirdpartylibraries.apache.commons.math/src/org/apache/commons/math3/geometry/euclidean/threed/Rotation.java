 ...
 * constructors and getters). In addition, a rotation can also be built implicitly from a set of vectors and their image.
 * </p>
 * <p>
 ...
 *
 * <p>
 * Since a rotation is basically a vectorial operator, several rotations can be composed together and the composite operation <code>r = r<sub>1</sub> o
 * r<sub>2</sub></code> (which means that for each vector <code>u</code>, <code>r(u) = r<sub>1</sub>(r<sub>2</sub>(u))</code>) is also a rotation. Hence we can consider that in addition to vectors, a rotation can be applied to other rotations as well (or to itself). With our previous notations, we would say we can apply <code>r<sub>1</sub></code> to <code>r<sub>2</sub></code> and the result we get is <code>r = r<sub>1</sub> o r<sub>2</sub></code>. For this purpose, the class provides the
 * methods: {@link #applyTo(Rotation) applyTo(Rotation)} and {@link #applyInverseTo(Rotation) applyInverseTo(Rotation)}.
 * </p>
 *
 * <p>
...
public class Rotation implements Serializable {
	 ...
	 * <p>
	 * A rotation can be built from a <em>normalized</em> quaternion, i.e. a quaternion for which q<sub>0</sub><sup>2</sup> + q<sub>1</sub><sup>2</sup> + q<sub>2</sub><sup>2</sup> + q<sub>3</sub><sup>2</sup> = 1. If the quaternion is not normalized, the constructor can normalize it in a preprocessing step.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * Another way to represent our convention is to say that a rotation of angle &theta; about the unit vector (x, y, z) is the same as the rotation build from quaternion components { cos(-&theta;/2), x * sin(-&theta;/2), y * sin(-&theta;/2), z * sin(-&theta;/2) }. Note the minus sign on the angle!
	 * </p>
	 * <p>
