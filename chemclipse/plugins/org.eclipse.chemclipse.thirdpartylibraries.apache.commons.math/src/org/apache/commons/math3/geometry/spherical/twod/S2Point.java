 ...
 * <p>
 * We use the mathematical convention to use the azimuthal angle \( \theta \) in the x-y plane as the first coordinate, and the polar angle \( \varphi \) as the second coordinate (see <a href="http://mathworld.wolfram.com/SphericalCoordinates.html">Spherical Coordinates</a> in MathWorld).
 * </p>
 * <p>
...
public class S2Point implements Point<Sphere2D> {
	 ...
	 *                if \( \varphi \) is not in the [\( 0; \pi \)] range
	 */
	 ...
	 * <p>
	 * If all coordinates of two points are exactly the same, and none are <code>Double.NaN</code>, the two points are considered to be equal.
	 * </p>
	 * <p>
	 * <code>NaN</code> coordinates are considered to affect globally the vector and be equals to each other - i.e, if either (or all) coordinates of the 2D vector are equal to <code>Double.NaN</code>, the 2D vector is equal to {@link #NaN}.
	 * </p>
	 *
	 * @param other
