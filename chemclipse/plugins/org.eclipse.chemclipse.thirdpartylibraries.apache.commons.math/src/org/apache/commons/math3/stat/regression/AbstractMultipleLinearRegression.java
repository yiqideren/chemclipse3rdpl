...
public abstract class AbstractMultipleLinearRegression implements MultipleLinearRegression {
	 ...
	 * </p>
	 * <p>
	 * Note that there is no need to add an initial unitary column (column of 1's) when specifying a model including an intercept term. If {@link #isNoIntercept()} is <code>true</code>, the X matrix will be created without an initial column of "1"s; otherwise this column will be added.
	 * </p>
	 * <p>
	 ...
	 *             if <code>nobs</code> is less than <code>nvars + 1</code>
	 */
	 ...
	 * <li>x has at least one more row than it has columns (i.e. there is sufficient data to estimate regression coefficients for each of the columns in x plus an intercept.</li>
	 * </ul>
	 *
	 * @param x
