...
public class ArcsSet extends AbstractRegion<Sphere1D, Sphere1D> implements Iterable<double[]> {
	 ...
	 * {@link org.apache.commons.math3.geometry.partitioning.Region#checkPoint(org.apache.commons.math3.geometry.Point)
	...
	private static BSPTree<Sphere1D> buildTree(final double lower, final double upper, final double tolerance) throws NumberIsTooLargeException {
		...
		if(normalizedUpper <= MathUtils.TWO_PI) {
			...
			final SubHyperplane<Sphere1D> upperCut = new LimitAngle(new S1Point(normalizedUpper - MathUtils.TWO_PI), true, tolerance).wholeHyperplane();
	...
	private class SubArcsIterator implements Iterator<double[]> {
		...
		private void selectPending() {
			...
			if(end != null) {
				...
				pending = new double[]{getAngle(start), getAngle(end) + MathUtils.TWO_PI};
	 ...
	 *            arc limit
	 * @param isStart
	 *            if true, the limit is the start of an arc
	 */
	...
	public static class Split {
		 ...
		 * 
		 * @param plus
		 *            part of the arcs set on the plus side of the
		 *            splitting arc
		 * @param minus
		 *            part of the arcs set on the minus side of the
		 *            splitting arc
		 */
