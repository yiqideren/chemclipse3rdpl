 ...
 * 
 * <p>
 * The application-defined attributes are shared among copied instances and propagated to split parts. These attributes are not used by the BSP-tree algorithms themselves, so the application can use them for any purpose. Since the tree visiting method holds internal and leaf nodes differently, it is possible to use different classes for internal nodes attributes and leaf nodes attributes. This should be used with care, though, because if the tree is modified in any way after attributes have been
 * set, some internal nodes may become leaf nodes and some leaf nodes may become internal nodes.
 * </p>
 * 
 * <p>
...
public class BSPTree<S extends Space> {
	 ...
	 * <p>
	 * This method does <em>not</em> perform any verification on consistency of its arguments, it should therefore be used only when then caller knows what it is doing.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * All trees are modified (parts of them are reused in the new tree), it is the responsibility of the caller to ensure a copy has been done before if any of the former tree should be preserved, <em>no</em> such copy is done here!
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * The tree returned is not upward-consistent: despite all of its sub-trees cut sub-hyperplanes (including its own cut sub-hyperplane) are bounded to the current cell, it is <em>not</em> attached to any parent tree yet. This tree is intended to be later inserted into an higher level tree.
	 * </p>
	 * <p>
	...
	public BSPTree<S> split(final SubHyperplane<S> sub) {
		...
		switch(sub.side(cHyperplane)) {
			case PLUS: { // the partitioning sub-hyperplane is entirely in the plus sub-tree
			...
			case MINUS: { // the partitioning sub-hyperplane is entirely in the minus sub-tree
	 ...
	 *            attribute to set for the internal nodes
