 ...
 *
 * <p>
 * The constructors that take <code>RealMatrix</code> or <code>double[][]</code> arguments generate correlation matrices. The columns of the input matrices are assumed to represent variable values. Correlations are given by the formula
 * </p>
 *
 * <p>
 * <code>cor(X, Y) = &Sigma;[(x<sub>i</sub> - E(X))(y<sub>i</sub> - E(Y))] / [(n - 1)s(X)s(Y)]</code> where <code>E(X)</code> is the mean of <code>X</code>, <code>E(Y)</code> is the mean of the <code>Y</code> values and s(X), s(Y) are standard deviations.
 * </p>
 *
 * <p>
...
public class PearsonsCorrelation {
	 ...
	 * <code>SE<sub>r</sub> = ((1 - r<sup>2</sup>) / (n - 2))<sup>1/2</sup></code> where <code>r</code> is the estimated correlation coefficient and <code>n</code> is the number of observations in the source dataset.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <code>getCorrelationPValues().getEntry(i,j)</code> is the probability that a random variable distributed as <code>t<sub>n-2</sub></code> takes a value with absolute value greater than or equal to <br>
	 * <code>|r|((n - 2) / (1 - r<sup>2</sup>))<sup>1/2</sup></code>
	 * </p>
	 *
	 * <p>
	 * The values in the matrix are sometimes referred to as the <i>significance</i> of the corresponding correlation coefficients.
	 * </p>
	 *
	 * <p>
	 * To use this method, one of the constructors that supply an input matrix must have been used to create this instance.
	 ...
	 * <p>
	 * Uses the formula <br/>
	 * <code>r(X,Y) = cov(X,Y)/s(X)s(Y)</code> where <code>r(&middot,&middot;)</code> is the correlation coefficient and <code>s(&middot;)</code> means standard deviation.
	 * </p>
	 *
	 * @param covarianceMatrix
	 *            the covariance matrix
