...
/**
 * Implements the {@link RandomData} interface using a {@link RandomGenerator} instance to generate non-secure data and a {@link java.security.SecureRandom} instance to provide data for the <code>nextSecureXxx</code> methods. If no <code>RandomGenerator</code> is provided in the constructor, the default is
 ...
 * <ul>
 * <li>
 * Instance variables are used to maintain <code>RandomGenerator</code> and <code>SecureRandom</code> instances used in data generation. Therefore, to generate a random sequence of values or strings, you should use just <strong>one</strong> <code>RandomDataImpl</code> instance repeatedly.</li>
 * <li>
 ...
 * <li>
 * When a new <code>RandomDataImpl</code> is created, the underlying random number generators are <strong>not</strong> initialized. If you do not explicitly seed the default non-secure generator, it is seeded with the current time in milliseconds plus the system identity hash code on first use. The same holds for the secure generator. If you provide a <code>RandomGenerator</code> to the constructor, however, this generator is not reseeded by the constructor nor is it reseeded on first use.</li>
 * <li>
...
public class RandomDataGenerator implements RandomData, Serializable {
	 ...
	 *            positive.
	 * @return a pseudorandom, uniformly distributed {@code long} value between 0 (inclusive) and n (exclusive).
	 ...
	 * </p>
	 *
	 * @return the SecureRandom used to generate secure random data, wrapped in a {@link RandomGenerator}.
	 */
