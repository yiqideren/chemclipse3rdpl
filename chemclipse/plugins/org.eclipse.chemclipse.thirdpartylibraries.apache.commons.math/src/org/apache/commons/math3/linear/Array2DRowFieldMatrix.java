 ...
 * <p>
 * As specified in the {@link FieldMatrix} interface, matrix element indexing is 0-based -- e.g., <code>getEntry(0, 0)</code> returns the element in the first row, first column of the matrix.</li></ul>
 * </p>
 *
 * @param <T>
...
public class Array2DRowFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T> implements Serializable {
	 ...
	 * data array.
	 * <p>
	 * If an array is built specially in order to be embedded in a {@code FieldMatrix<T>} and not used directly, the {@code copyArray} may be set to {@code false}. This will prevent the copying and improve performance as no new array will be built and no data will be copied.
	 * </p>
	 *
	 * @param d
	 *            Data for the new matrix.
