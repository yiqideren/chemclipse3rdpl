 ...
 * <ul>
 * <li>
 * the first one is in a node-oriented way using the {@link BSPTree#getPlus}, {@link BSPTree#getMinus} and {@link BSPTree#getParent} methods. Terminal nodes without associated {@link SubHyperplane sub-hyperplanes} can be visited this way, there is no constraint in the visit order, and it is possible to visit either all nodes or only a subset of the nodes</li>
 * <li>
 * the second one is in a sub-hyperplane-oriented way using classes implementing this interface which obeys the visitor design pattern. The visit order is provided by the visitor as each node is first encountered. Each node is visited exactly once.</li>
 * </ul>
 * 
 * @param <S>
 *            Type of the space.
