 ...
 * <li>Compute the estimated percentile position <code> pos = p * (n + 1) / 100</code> and the difference, <code>d</code> between <code>pos</code> and <code>floor(pos)</code> (i.e. the fractional part of <code>pos</code>).</li>
 ...
 * <li>Else let <code>lower</code> be the element in position <code>floor(pos)</code> in the array and let <code>upper</code> be the next element in the array. Return <code>lower + d * (upper - lower)</code></li>
 * </ol>
 * </p>
 * <p>
 ...
 * evaluates to <code>2.5.</code>
 * </p>
 * <p>
 * Since percentile estimation usually involves interpolation between array elements, arrays containing <code>NaN</code> or infinite values will often result in <code>NaN</code> or infinite values returned.
 * </p>
 * <p>
 * Further, to include different estimation types such as R1, R2 as mentioned in <a href="http://en.wikipedia.org/wiki/Quantile">Quantile page(wikipedia)</a>, a type specific NaN handling strategy is used to closely match with the typically observed results from popular tools like R(R1-R9), Excel(R7).
 * </p>
 * <p>
 * Since 2.2, Percentile uses only selection instead of complete sorting and caches selection algorithm state between calls to the various {@code evaluate} methods. This greatly improves efficiency, both for a single percentile and multiple percentile computations. To maximize performance when multiple percentiles are computed based on the same data, users should set the data array once using either one of the {@link #evaluate(double[], double)} or {@link #setData(double[])} methods and
 * thereafter {@link #evaluate(double)} with just the percentile provided.
 * </p>
 * <p>
 * <strong>Note that this implementation is not synchronized.</strong> If multiple threads access an instance of this class concurrently, and at least one of the threads invokes the <code>increment()</code> or <code>clear()</code> method, it must be synchronized externally.
 * </p>
 *
 */
public class Percentile extends AbstractUnivariateStatistic implements Serializable {
	 ...
	 * <li>default NaN strategy: {@link NaNStrategy#REMOVED}, can be reset with {@link #withNaNStrategy(NaNStrategy)}</li>
	 * <li>a KthSelector that makes use of {@link MedianOf3PivotingStrategy}, can be reset with {@link #withKthSelector(KthSelector)}</li>
	 * </ul>
	 */
	public Percentile() {
	...
	/**
	 * Constructs a Percentile with the specific quantile value, {@link EstimationType}, {@link NaNStrategy} and {@link KthSelector}.
	 *
	 * @param quantile
	 *            the quantile to be computed
	 * @param estimationType
	 *            one of the percentile {@link EstimationType estimation types}
	 * @param nanStrategy
	 *            one of {@link NaNStrategy} to handle with NaNs
	 * @param kthSelector
	 *            a {@link KthSelector} to use for pivoting during search
	 * @throws MathIllegalArgumentException
	 ...
	 * <li>Throws <code>MathIllegalArgumentException</code> if <code>values</code> is null or p is not a valid quantile value (p must be greater than 0 and less than or equal to 100)</li>
	 * </ul>
	 * </p>
	 * <p>
	 ...
	 * <li>Throws <code>MathIllegalArgumentException</code> if <code>values</code> is null , <code>begin</code> or <code>length</code> is invalid, or <code>p</code> is not a valid quantile value (p must be greater than 0 and less than or equal to 100)</li>
	 * </ul>
	 * </p>
	 * <p>
	 * See {@link Percentile} for a description of the percentile estimation algorithm used.
	...
	public double evaluate(final double[] values, final int begin, final int length, final double p) throws MathIllegalArgumentException {
		...
		return work.length == 0 ? Double.NaN : estimationType.evaluate(work, pivotsHeap, p, kthSelector);
	}
	 ...
	 *                always thrown since 3.4
	 * @deprecated as of 3.4 this method does not work anymore, as it fails to
	 *             copy internal states between instances configured with different {@link EstimationType estimation type}, {@link NaNStrategy NaN handling strategies} and {@link KthSelector kthSelector}, it therefore always
	 ...
	 * @param begin
	 *            index to start reading the array
	 * @param length
	 *            the length of array to be read from the begin index
	 ...
	 * @param length
	 *            number of elements to include from begin
	 ...
	 *            number of elements to include from begin
	 * @param removedValue
	 *            the value to be removed from the sliced array
	 ...
	 *
	 * @param values
	 *            array containing the input numbers
	...
	/**
	 * Build a new instance similar to the current one except for the {@link EstimationType estimation type}.
	 * <p>
	...
	/**
	 * Build a new instance similar to the current one except for the {@link NaNStrategy NaN handling} strategy.
	 * <p>
	 * This method is intended to be used as part of a fluent-type builder pattern. Building finely tune instances should be done as follows:
	 * </p>
	 * 
	...
	/**
	 * Build a new instance similar to the current one except for the {@link KthSelector kthSelector} instance specifically set.
	 * <p>
	 * This method is intended to be used as part of a fluent-type builder pattern. Building finely tune instances should be done as follows:
	 * </p>
	 * 
	 ...
	 * <ul>
	 * <li>An <b>index method</b> to calculate approximate index of the estimate</li>
	 * <li>An <b>estimate method</b> to estimate a value found at the earlier computed index</li>
	 * <li>A <b> minLimit</b> on the quantile for which first element of sorted input is returned as an estimate</li>
	 * <li>A <b> maxLimit</b> on the quantile for which last element of sorted input is returned as an estimate</li>
	 * </ul>
	 * <p>
	 ...
	 * <ol>
	 * <li>
	 * <a href="http://en.wikipedia.org/wiki/Quantile">Wikipedia on quantile</a></li>
	 * <li>
	 * <a href="https://www.amherst.edu/media/view/129116/.../Sample+Quantiles.pdf"> Hyndman, R. J. and Fan, Y. (1996) Sample quantiles in statistical packages, American Statistician 50, 361–365</a></li>
	 * <li>
	 * <a href="http://stat.ethz.ch/R-manual/R-devel/library/stats/html/quantile.html"> R-Manual </a></li>
	 * </ol>
	 *
	 */
	public static enum EstimationType {
		...
		LEGACY("Legacy Apache Commons Math") {
			...
			protected double index(final double p, final int length) {
				...
				return Double.compare(p, minLimit) == 0 ? 0 : Double.compare(p, maxLimit) == 0 ? length : p * (length + 1);
			}
		},
		/**
		...
		R_1("R-1") {
			 ...
			 */
			@Override
			protected double estimate(final double[] values, final int[] pivotsHeap, final double pos, final int length, final KthSelector kthSelector) {
		...
		R_2("R-2") {
			...
			protected double index(final double p, final int length) {
				...
				final double minLimit = 0d;
				final double maxLimit = 1d;
				return Double.compare(p, maxLimit) == 0 ? length : Double.compare(p, minLimit) == 0 ? 0 : length * p + 0.5;
			}
			...
			protected double estimate(final double[] values, final int[] pivotsHeap, final double pos, final int length, final KthSelector kthSelector) {
				...
				final double low = super.estimate(values, pivotsHeap, FastMath.ceil(pos - 0.5), length, kthSelector);
				final double high = super.estimate(values, pivotsHeap, FastMath.floor(pos + 0.5), length, kthSelector);
				return (low + high) / 2;
			}
		},
		/**
		...
		R_3("R-3") {
			...
			protected double index(final double p, final int length) {
				...
				return Double.compare(p, minLimit) <= 0 ? 0 : FastMath.rint(length * p);
			}
		},
		/**
		...
		R_4("R-4") {
			...
			protected double index(final double p, final int length) {
				...
				final double maxLimit = 1d;
				return Double.compare(p, minLimit) < 0 ? 0 : Double.compare(p, maxLimit) == 0 ? length : length * p;
			}
		},
		/**
		...
		R_5("R-5") {
			...
			protected double index(final double p, final int length) {
				...
				return Double.compare(p, minLimit) < 0 ? 0 : Double.compare(p, maxLimit) >= 0 ? length : length * p + 0.5;
			}
		},
		/**
		...
		R_6("R-6") {
			...
			protected double index(final double p, final int length) {
				...
				return Double.compare(p, minLimit) < 0 ? 0 : Double.compare(p, maxLimit) >= 0 ? length : (length + 1) * p;
			}
		},
		/**
		...
		R_7("R-7") {
			...
			protected double index(final double p, final int length) {
				...
				final double minLimit = 0d;
				final double maxLimit = 1d;
				return Double.compare(p, minLimit) == 0 ? 0 : Double.compare(p, maxLimit) == 0 ? length : 1 + (length - 1) * p;
			}
		},
		/**
		...
		R_8("R-8") {
			...
			protected double index(final double p, final int length) {
				...
				final double maxLimit = (length - 1d / 3) / (length + 1d / 3);
				return Double.compare(p, minLimit) < 0 ? 0 : Double.compare(p, maxLimit) >= 0 ? length : (length + 1d / 3) * p + 1d / 3;
			}
		},
		/**
		...
		R_9("R-9") {
			...
			protected double index(final double p, final int length) {
				...
				return Double.compare(p, minLimit) < 0 ? 0 : Double.compare(p, maxLimit) >= 0 ? length : (length + 0.25) * p + 3d / 8;
			}
		},
		;
		 ...
		 *
		 * @param type
		 *            name of estimation type as per wikipedia
		 */
		...
		/**
		 * Finds the index of array that can be used as starting index to {@link #estimate(double[], int[], double, int, KthSelector) estimate} percentile. The calculation of index calculation is specific to each {@link EstimationType}.
		 *
		 * @param p
		 *            the p<sup>th</sup> quantile
		 * @param length
		 *            the total number of array elements in the work array
		 ...
		 *
		 * @param work
		 *            array of numbers to be used for finding the percentile
		 * @param pos
		 *            indicated positional index prior computed from calling {@link #index(double, int)}
		 * @param pivotsHeap
		 *            an earlier populated cache if exists; will be used
		 * @param length
		 *            size of array considered
		 * @param kthSelector
		 *            a {@link KthSelector} used for pivoting during search
		 * @return estimated percentile
		 */
		protected double estimate(final double[] work, final int[] pivotsHeap, final double pos, final int length, final KthSelector kthSelector) {
		 ...
		 * This basically calls the {@link #index(double, int) index} and then {@link #estimate(double[], int[], double, int, KthSelector) estimate} functions to return the estimated percentile value.
		 *
		 * @param work
		 *            array of numbers to be used for finding the percentile
		 * @param pivotsHeap
		 *            a prior cached heap which can speed up estimation
		 * @param p
		 *            the p<sup>th</sup> quantile to be computed
		 * @param kthSelector
		 *            a {@link KthSelector} used for pivoting during search
		 * @return estimated percentile
		 ...
		 */
		protected double evaluate(final double[] work, final int[] pivotsHeap, final double p, final KthSelector kthSelector) {
		...
		/**
		 * Evaluate method to compute the percentile for a given bounded array.
		 * This basically calls the {@link #index(double, int) index} and then {@link #estimate(double[], int[], double, int, KthSelector) estimate} functions to return the estimated percentile value. Please
