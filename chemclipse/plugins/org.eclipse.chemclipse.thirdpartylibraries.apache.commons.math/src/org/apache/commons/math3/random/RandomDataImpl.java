...
/**
 * Generates random deviates and other random data using a {@link RandomGenerator} instance to generate non-secure data and a {@link java.security.SecureRandom} instance to provide data for the <code>nextSecureXxx</code> methods. If no <code>RandomGenerator</code> is provided in the constructor, the default is
 * to use a {@link Well19937c} generator. To plug in a different
 * implementation, either implement <code>RandomGenerator</code> directly or
 * extend {@link AbstractRandomGenerator}.
 * <p>
 * Supports reseeding the underlying pseudo-random number generator (PRNG). The <code>SecurityProvider</code> and <code>Algorithm</code> used by the <code>SecureRandom</code> instance can also be reset.
 * </p>
 * <p>
 ...
 * <ul>
 * <li>
 * Instance variables are used to maintain <code>RandomGenerator</code> and <code>SecureRandom</code> instances used in data generation. Therefore, to generate a random sequence of values or strings, you should use just <strong>one</strong> <code>RandomDataGenerator</code> instance repeatedly.</li>
 * <li>
 * The "secure" methods are *much* slower. These should be used only when a cryptographically secure random sequence is required. A secure random sequence is a sequence of pseudo-random values which, in addition to being well-dispersed (so no subsequence of values is an any more likely than other subsequence of the the same length), also has the additional property that knowledge of values generated up to any point in the sequence does not make it any easier to predict subsequent values.</li>
 * <li>
 * When a new <code>RandomDataGenerator</code> is created, the underlying random number generators are <strong>not</strong> initialized. If you do not explicitly seed the default non-secure generator, it is seeded with the current time in milliseconds plus the system identity hash code on first use. The same holds for the secure generator. If you provide a <code>RandomGenerator</code> to the constructor, however, this generator is not reseeded by the constructor nor is it reseeded on first use.</li>
 * <li>
 * The <code>reSeed</code> and <code>reSeedSecure</code> methods delegate to the corresponding methods on the underlying <code>RandomGenerator</code> and <code>SecureRandom</code> instances. Therefore, <code>reSeed(long)</code> fully resets the initial state of the non-secure random number generator (so that reseeding with a specific value always results in the same subsequent random sequence); whereas reSeedSecure(long) does <strong>not</strong> reinitialize the secure random number generator
 * (so secure sequences started with calls to reseedSecure(long) won't be identical).</li>
 * <li>
 * This implementation is not synchronized. The underlying <code>RandomGenerator</code> or <code>SecureRandom</code> instances are not protected by synchronization and are not guaranteed to be thread-safe. Therefore, if an instance of this class is concurrently utilized by multiple threads, it is the responsibility of client code to synchronize access to seeding and data generation methods.</li>
 * </ul>
 * </p>
 * 
...
public class RandomDataImpl implements RandomData, Serializable {
	 ...
	 * <ul>
	 * <li>For small means, uses simulation of a Poisson process using Uniform deviates, as described <a href="http://irmi.epfl.ch/cmos/Pmmi/interactive/rng7.htm"> here.</a> The Poisson process (and hence value returned) is bounded by 1000 * mean.</li>
	 *
	 ...
	 *
	 * <p>
	 * <strong>Algorithm Description</strong>: Uses the Algorithm SA (Ahrens) from p. 876 in: [1]: Ahrens, J. H. and Dieter, U. (1972). Computer methods for sampling from the exponential and normal distributions. Communications of the ACM, 15, 873-882.
	 * </p>
	 */
	 ...
	 * <p>
	 * For 0 < shape < 1: <br/>
	 * Ahrens, J. H. and Dieter, U., <i>Computer methods for sampling from gamma, beta, Poisson and binomial distributions.</i> Computing, 12, 223-246, 1974.
	 * </p>
	 *
	 * <p>
	 * For shape >= 1: <br/>
	 * Marsaglia and Tsang, <i>A Simple Method for Generating Gamma Variables.</i> ACM Transactions on Mathematical Software, Volume 26 Issue 3, September, 2000.
	 * </p>
	 *
	 * @param shape
	 ...
	 *
	 * <p>
	 * Uses a 2-cycle permutation shuffle. The shuffling process is described <a href="http://www.maths.abdn.ac.uk/~igc/tch/mx4002/notes/node83.html"> here</a>.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * <strong>Algorithm Description</strong>: Uses a 2-cycle permutation shuffle to generate a random permutation of <code>c.size()</code> and then returns the elements whose indexes correspond to the elements of the generated permutation. This technique is described, and proven to generate random samples <a href="http://www.maths.abdn.ac.uk/~igc/tch/mx4002/notes/node83.html"> here</a>
	 * </p>
	 */
	 ...
	 *
	 * @param distribution
	 *            Continuous distribution to generate a random value from
	 * @return a random value sampled from the given distribution
	 * @throws MathIllegalArgumentException
	 *             if the underlynig distribution throws one
	 * @since 2.2
	 ...
	 *
	 * @param distribution
	 *            Integer distribution to generate a random value from
	 * @return a random value sampled from the given distribution
	 * @throws MathIllegalArgumentException
	 *             if the underlynig distribution throws one
	 * @since 2.2
