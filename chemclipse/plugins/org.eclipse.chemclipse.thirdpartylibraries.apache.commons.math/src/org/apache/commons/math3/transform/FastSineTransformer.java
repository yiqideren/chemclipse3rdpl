 ...
 * <p>
 * There are several variants of the discrete sine transform. The present implementation corresponds to DST-I, with various normalization conventions, which are specified by the parameter {@link DstNormalization}. <strong>It should be noted that regardless to the convention, the first element of the dataset to be transformed must be zero.</strong>
 * <p>
 * DST-I is equivalent to DFT of an <em>odd extension</em> of the data series. More precisely, if x<sub>0</sub>, &hellip;, x<sub>N-1</sub> is the data set to be sine transformed, the extended data set x<sub>0</sub><sup>&#35;</sup>, &hellip;, x<sub>2N-1</sub><sup>&#35;</sup> is defined as follows
 * <ul>
 ...
 * </ul>
 * <p>
 * Then, the standard DST-I y<sub>0</sub>, &hellip;, y<sub>N-1</sub> of the real data set x<sub>0</sub>, &hellip;, x<sub>N-1</sub> is equal to <em>half</em> of i (the pure imaginary number) times the N first elements of the DFT of the extended data set x<sub>0</sub><sup>&#35;</sup>, &hellip;, x<sub>2N-1</sub><sup>&#35;</sup> <br />
 * y<sub>n</sub> = (i / 2) &sum;<sub>k=0</sub><sup>2N-1</sup> x<sub>k</sub><sup>&#35;</sup> exp[-2&pi;i nk / (2N)] &nbsp;&nbsp;&nbsp;&nbsp;k = 0, &hellip;, N-1.
 * <p>
