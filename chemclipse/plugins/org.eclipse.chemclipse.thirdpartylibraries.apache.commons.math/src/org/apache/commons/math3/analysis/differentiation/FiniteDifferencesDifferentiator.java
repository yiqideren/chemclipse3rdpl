 ...
 * quickly (there are several differences of close points). Choosing the number of points and the step size is highly problem dependent.
 * </p>
 * <p>
...
public class FiniteDifferencesDifferentiator implements UnivariateFunctionDifferentiator, UnivariateVectorFunctionDifferentiator, UnivariateMatrixFunctionDifferentiator, Serializable {
	 ...
	 *                if {@code stepsize <= 0} (note that {@link NotPositiveException} extends {@link NumberIsTooSmallException})
	 ...
	 *            step size (gap between each point)
	 * @param tLower
	 *            lower bound for independent variable (may be {@code Double.NEGATIVE_INFINITY} if there are no lower bounds)
	 * @param tUpper
	 *            upper bound for independent variable (may be {@code Double.POSITIVE_INFINITY} if there are no upper bounds)
	 * @exception NotPositiveException
	 *                if {@code stepsize <= 0} (note that {@link NotPositiveException} extends {@link NumberIsTooSmallException})
	 * @exception NumberIsTooSmallException
	 *                {@code nbPoint <= 1}
