 ...
 * <p>
 * Like {@link java.util.Hashtable} but unlike {@link HashMap}, this class does <em>not</em> allow <tt>null</tt> to be used as a key or value. Unlike {@link java.util.LinkedHashMap}, this class does <em>not</em> provide predictable iteration order. A snapshot of the keys and entries may be obtained in ascending and descending order of retention.
 *
...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	 ...
	 *
	 * @param bufferIndex
	 *            the index to the chosen read buffer
	 * @param node
	 *            the entry in the page replacement policy
	...
	/**
	 * Atomically transitions the node from the <tt>alive</tt> state to the <tt>retired</tt> state, if a valid transition.
	 *
	 * @param node
	 *            the entry in the page replacement policy
	 */
	...
	/**
	 * Returns the value to which the specified key is mapped, or {@code null} if this map contains no mapping for the key. This method differs from {@link #get(Object)} in that it does not record the operation with the
	 ...
	 * <p>
	 * Beware that, unlike in {@link #keySet()}, obtaining the set is <em>NOT</em> a constant-time operation. Because of the asynchronous nature of the page replacement policy, determining the retention ordering requires a traversal of the keys.
	 *
	 ...
	 * most-likely.
	 * <p>
	 * Beware that obtaining the mappings is <em>NOT</em> a constant-time operation. Because of the asynchronous nature of the page replacement policy, determining the retention ordering requires a traversal of the entries.
	 *
