...
public class Equation {
	 ...
	 *
	 * @param variable
	 *            Matrix which is to be assigned to name
	 * @param name
	 ...
	 * @param debug
	 *            if true it will print out debugging information
	 ...
	 *
	 * @param variables
	 *            Variables which describe the selected range
	 */
	 ...
	 *
	 * @param ops
	 *            List of operations which should be parsed
	 * @param tokens
	 *            List of all the tokens
	 * @param sequence
	 *            List of operation sequence
			...
			if(type == TokenType.WORD) {
				...
				if(isSymbol(c)) {
					...
					if(c == '-') {
						...
						if(i + 1 < equation.length() && Character.isDigit(equation.charAt(i + 1)) && (tokens.last == null || isOperatorLR(tokens.last.getSymbol()))) {
	...
	protected static enum TokenType {
		WORD, INTEGER, FLOAT, FLOAT_EXP, UNKNOWN
	}
	 ...
	 *            Token being checked
	 * @param ops
	 *            List of allowed character operations
