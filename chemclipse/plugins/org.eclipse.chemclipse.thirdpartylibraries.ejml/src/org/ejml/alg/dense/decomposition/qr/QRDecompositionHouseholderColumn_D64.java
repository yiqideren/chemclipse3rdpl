...
public class QRDecompositionHouseholderColumn_D64 implements QRDecomposition<DenseMatrix64F> {
	 ...
	 *
	 * @param R
	 *            Storage for upper triangular matrix.
	 * @param compact
	 *            If true then a compact matrix is expected.
	 */
	@Override
		...
		if(R == null) {
			...
			if(compact) {
				if(R.numCols != numCols || R.numRows != minLength)
					throw new IllegalArgumentException("Unexpected dimensions: found( " + R.numRows + " " + R.numCols + " ) expected( " + minLength + " " + numCols + " )");
			} else {
	...
	/**
	 * <p>
	 * To decompose the matrix 'A' it must have full rank. 'A' is a 'm' by 'n' matrix. It requires about 2n*m<sup>2</sup>-2m<sup>2</sup>/3 flops.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param A
	 *            original matrix that is to be decomposed.
	 */
