 ...
 * Assumptions:
 * <ul>
 * <le> All submatrices are aligned along the inner blocks of the {@link org.ejml.data.BlockMatrix64F}. <le> Some times vectors are assumed to have leading zeros and a one.
 * </ul>
 *
 * @author Peter Abeles
 */
public class BlockHouseHolder {
	...
	/**
	 * <p>
	 * Applies a householder reflector stored in column 'col' to the remainder of the columns in the block after it. Takes in account leading zeros and one.<br>
	 * <br>
	 * A = (I - &gamma;*u*u<sup>T</sup>)*A<br>
	 * </p>
	 *
	 * @param A
	 *            submatrix that is at most one block wide and aligned along inner blocks
	 * @param col
	...
	/**
	 * <p>
	 * Applies a householder reflector stored in column 'col' to the top block row (excluding the first column) of A. Takes in account leading zeros and one.<br>
	 * <br>
	 * A = (I - &gamma;*u*u<sup>T</sup>)*A<br>
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Applies a householder reflector stored in row 'row' to the remainder of the row in the block after it. Takes in account leading zeros and one.<br>
	 * <br>
	 * A = A*(I - &gamma;*u*u<sup>T</sup>)<br>
	 * </p>
	 *
	 * @param A
	 *            submatrix that is block aligned
	 * @param row
	 *            The row in A containing 'u'
	 * @param colStart
	...
	/**
	 * <p>
	 * Applies a householder reflector stored in row 'row' to the left column block. Takes in account leading zeros and one.<br>
	 * <br>
	 * A = A*(I - &gamma;*u*u<sup>T</sup>)<br>
	 * </p>
	 *
	 ...
	 *
	 */
	public static void rank1UpdateMultL_LeftCol(final int blockLength, final D1Submatrix64F A, final int row, final double gamma, int zeroOffset) {
	 ...
	 * @param colA
	 *            Column inside the block of first column vector.
	 * @param widthA
	 *            how wide the column block that colA is inside of.
	 * @param colB
	 *            Column inside the block of second column vector.
	 * @param widthB
	 *            how wide the column block that colB is inside of.
	 ...
	 *            block aligned submatrix.
	 * @param rowA
	 *            Row index inside the sub-matrix of first row vector has zeros and ones..
	 * @param rowB
	 *            Row index inside the sub-matrix of second row vector.
	 * @return dot product of the two vectors.
	 */
	...
	public static void add_row(final int blockLength, D1Submatrix64F A, int rowA, double alpha, D1Submatrix64F B, int rowB, double beta, D1Submatrix64F C, int rowC, int zeroOffset, int end) {
	 ...
	 *
	 * @param row
	 *            Which row in the block will be processed
	 * @param colStart
	 *            The first column that computation of tau will start at
	 * @param max
	 ...
	 * end<br>
	 * <br>
	 * where v<sup>(.)</sup> are the house holder vectors, and r is the block length. Note that Y already contains the householder vectors so it does not need to be modified.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param Y
	 *            Input matrix containing householder vectors. Not modified.
	 * @param W
	 *            Resulting W matrix. Modified.
	 * @param temp
	 *            Used internally. Must have W.numCols elements.
	 * @param beta
	 *            Beta's for householder vectors.
	 * @param betaIndex
	...
	/**
	 * <p>
	 * Sets W to its initial value using the first column of 'y' and the value of 'b': <br>
	 * W = -&beta;v<br>
	 * <br>
	 * where v = Y(:,0).
	 * </p>
	 ...
	 * @param W
	 *            Submatrix being initialized.
	 * @param Y
	 *            Contains householder vector
	 * @param widthB
	 *            How wide the W block matrix is.
	 * @param b
	 *            beta
	 */
	...
	/**
	 * <p>
	 * Inner block mult add operation that takes in account the zeros and on in dataA, which is the top part of the Y block vector that has the householder vectors.<br>
	 * <br>
	 * C = C + A * B
	 * </p>
	 */
	public static void multBlockAdd_zerosone(double[] dataA, double[] dataB, double[] dataC, int indexA, int indexB, int indexC, final int heightA, final int widthA, final int widthC) {
	...
	/**
	 * <p>
	 * Performs a matrix multiplication on the block aligned submatrices. A is assumed to be block column vector that is lower triangular with diagonal elements set to 1.<br>
	 * <br>
	 * C = A^T * B
	 * </p>
	 */
	public static void multTransA_vecCol(final int blockLength, D1Submatrix64F A, D1Submatrix64F B, D1Submatrix64F C) {
	 ...
	 * C = A^T * B
	 */
	protected static void multTransABlockSet_lowerTriag(double[] dataA, double[] dataB, double[] dataC, int indexA, int indexB, int indexC, final int heightA, final int widthA, final int widthC) {
