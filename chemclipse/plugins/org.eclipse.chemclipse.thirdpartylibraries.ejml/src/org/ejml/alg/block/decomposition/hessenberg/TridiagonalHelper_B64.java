...
public class TridiagonalHelper_B64 {
	 ...
	 *
	 * <p>
	 * For each row 'a' in 'A': Compute 'u' the householder reflector. y(:) = A*u v(i) = y - (1/2)*(y^T*u)*u a(i+1) = a(i) - u*&gamma;*v^T - v*u^t
	 * </p>
	 *
	 * @param blockLength
	 *            Size of a block
	 * @param A
	 *            is the row block being decomposed. Modified.
	 * @param gammas
	 *            Householder gammas.
	 * @param V
	...
	/**
	 * <p>
	 * Applies the reflectors that have been computed previously to the specified row. <br>
	 * A = A + u*v^T + v*u^T only along the specified row in A.
	 * </p>
	 *
	 * @param blockLength
	 * @param A
	 *            Contains the reflectors and the row being updated.
	 * @param V
	 *            Contains previously computed 'v' vectors.
	 * @param row
	 *            The row of 'A' that is to be updated.
	 */
	public static void applyReflectorsToRow(final int blockLength, final D1Submatrix64F A, final D1Submatrix64F V, int row) {
	 ...
	 *            The row of 'A' that is to be updated.
	 */
	public static void computeY(final int blockLength, final D1Submatrix64F A, final D1Submatrix64F V, int row, double gamma) {
	...
	/**
	 * <p>
	 * Multiples the appropriate submatrix of A by the specified reflector and stores the result ('y') in V.<br>
	 * <br>
	 * y = A*u<br>
	 * </p>
	 *
	 * @param blockLength
	 * @param A
	 *            Contains the 'A' matrix and 'u' vector.
	 * @param V
	 *            Where resulting 'y' row vectors are stored.
	 * @param row
	 *            row in matrix 'A' that 'u' vector and the row in 'V' that 'y' is stored in.
	 */
	public static void multA_u(final int blockLength, final D1Submatrix64F A, final D1Submatrix64F V, int row) {
	 ...
	 * @param gamma
	 */
	public static void computeRowOfV(final int blockLength, final D1Submatrix64F A, final D1Submatrix64F V, int row, double gamma) {
