...
public class BlockMultiplication {
	 ...
	 *
	 * @param blockLength
	 *            Size of the blocks in the submatrix.
	 * @param A
		...
		for(int i = A.row0; i < A.row1; i += blockLength) {
			...
			for(int j = B.col0; j < B.col1; j += blockLength) {
				...
				for(int k = A.col0; k < A.col1; k += blockLength) {
					...
					if(k == A.col0)
						blockMultSet(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthB);
					else
						blockMultPlus(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthB);
				}
			}
		}
	}
		...
		for(int i = A.row0; i < A.row1; i += blockLength) {
			...
			for(int j = B.col0; j < B.col1; j += blockLength) {
				...
				for(int k = A.col0; k < A.col1; k += blockLength) {
					...
					int indexA = i * A.original.numCols + k * heightA;
					int indexB = (k - A.col0 + B.row0) * B.original.numCols + j * widthA;
					blockMultMinus(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthB);
				}
			}
		}
	}
		...
		for(int i = A.col0; i < A.col1; i += blockLength) {
			...
			for(int j = B.col0; j < B.col1; j += blockLength) {
				...
				for(int k = A.row0; k < A.row1; k += blockLength) {
					...
					if(k == A.row0)
						blockMultSetTransA(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthB);
					else
						blockMultPlusTransA(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthB);
				}
			}
		}
	}
		...
		for(int i = A.col0; i < A.col1; i += blockLength) {
			...
			for(int j = B.col0; j < B.col1; j += blockLength) {
				...
				for(int k = A.row0; k < A.row1; k += blockLength) {
					...
					int indexA = k * A.original.numCols + i * heightA;
					int indexB = (k - A.row0 + B.row0) * B.original.numCols + j * heightA;
					blockMultMinusTransA(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthB);
				}
			}
		}
	}
	 ...
	 *
	 * @param blockLength
	 *            Length of the blocks in the submatrix.
	 * @param A
	 *            A submatrix. Not modified.
	 * @param B
	 *            A submatrix. Not modified.
		...
		for(int i = A.row0; i < A.row1; i += blockLength) {
			...
			for(int j = B.row0; j < B.row1; j += blockLength) {
				...
				for(int k = A.col0; k < A.col1; k += blockLength) {
					...
					if(k == A.col0)
						blockMultSetTransB(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthC);
					else
						blockMultPlusTransB(A.original.data, B.original.data, C.original.data, indexA, indexB, indexC, heightA, widthA, widthC);
				}
			}
		}
	}
