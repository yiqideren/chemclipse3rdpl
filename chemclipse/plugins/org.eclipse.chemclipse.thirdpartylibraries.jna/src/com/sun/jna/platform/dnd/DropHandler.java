 ...
 * <p>
 * <ul>
 * <li>Accept drops where the action is the default (i.e. no modifiers) but the intersection of source and target actions is <i>not</i> the default. Doing so allows the source to adjust the cursor appropriately.
 * <li>Refuse drops where the user modifiers request an action that is not supported (this works for all cases except when the drag source is not a {@link DragHandler} and the user explicitly requests a MOVE operation; this is indistinguishable from a drag with no modifiers unless we have access to the key modifiers, which {@link DragHandler} provides).
 * <li>Drops may be refused based on data flavor, location, intended drop action, or any combination of those, by overriding {@link #canDrop}.
 * <li>Custom decoration of the drop area may be performed in {@link #paintDropTarget(DropTargetEvent, int, Point)} or by providing a {@link DropTargetPainter}.
 * </ul>
 * 
 * The method {@link #getDropAction(DropTargetEvent)} follows these steps to determine the appropriate action (if any).
 * <ul>
 * <li>{@link #isSupported(DataFlavor[])} determines if there are any supported flavors
 * <li>{@link #getDropActionsForFlavors(DataFlavor[])} reduces the supported actions based on available flavors. For instance, a text field for file paths might support {@link DnDConstants#ACTION_COPY_OR_MOVE} on a plain string, but {@link DnDConstants#ACTION_LINK} might be the only action supported on a file.
 * <li>{@link #getDropAction(DropTargetEvent, int, int, int)} relax the action if it's the default, or restrict it for user requested actions.
 * <li>{@link #canDrop(DropTargetEvent, int, Point)} change the action based on the location in the drop target component, or any other criteria.
 * </ul>
 * 
 * Override {@link #drop(DropTargetDropEvent, int)} to handle the drop. You should invoke {@link DropTargetDropEvent#dropComplete} as soon as the {@link Transferable} data is obtained, to avoid making the DnD operation look suspended.
 * 
 * @see DragHandler
 * @author twall
 */
...
public abstract class DropHandler implements DropTargetListener {
	 ...
	 * 
	 * @param c
	 *            The component to receive drops
	 * @param acceptedActions
	 *            Allowed actions for drops
	 * @param acceptedFlavors
	 *            Allowed data flavors for drops
	 * @see #isSupported
	 */
	 ...
	 * @see #paintDropTarget
	 */
	public DropHandler(final Component c, int acceptedActions, DataFlavor[] acceptedFlavors, DropTargetPainter painter) {
	 ...
	 * behavior). The drop action is forced to {@link DnDConstants#ACTION_NONE} if there is no supported data flavor.
	 * <p>
	 * 
	 ...
	 * standard drop validity checking and handling, then invokes {@link #drop(DropTargetDropEvent,int)} if the drop looks acceptable.
	 */
	 ...
	 * should be painted only if the event is an instance of {@link DropTargetDragEvent} with an action that is not {@link DragHandler#NONE}. Otherwise the decoration should be removed
	 * or hidden.
	 * <p>
	 * For an easy way to highlight the drop target, consider using a single instance of <code>AbstractComponentDecorator</code> and moving it according to the intended drop location.
	 * 
	 * @param e
	 ...
	 * location. This method is the last check performed by {@link #getDropAction(DropTargetEvent)}.
