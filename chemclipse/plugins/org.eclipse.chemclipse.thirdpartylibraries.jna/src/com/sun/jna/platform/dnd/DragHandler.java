 ...
 * <li>Adjusts the cursor on drags with no modifier for which the default action is disallowed but where one or more non-default actions are allowed, e.g. a drag (with no modifiers) to a target which supports "link" should change the cursor to "link" (prior to 1.6, the JRE behavior is to display a "not allowed" cursor, even though the action actually depends on how the drop target responds). <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4869264"> The bug</a> is fixed in java 1.6.
 * <li>Disallow drops to targets if the non-default (user-requested) action is not supported by the target, e.g. the user requests a "copy" when the target only supports "move". This is generally the responsibility of the drop handler, which decides whether or not to accept a drag. The DragHandler provides static modifier state information since the drop handler doesn't have access to it.
 * </ul>
...
public abstract class DragHandler implements DragSourceListener, DragSourceMotionListener, DragGestureListener {
	...
	static final int LINK_MASK = OSX ? InputEvent.ALT_DOWN_MASK | InputEvent.META_DOWN_MASK : InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK;
	/** Modifier mask for any user-requested action. */
	static final int KEY_MASK = InputEvent.ALT_DOWN_MASK | InputEvent.META_DOWN_MASK | InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK | InputEvent.ALT_GRAPH_DOWN_MASK;
	 ...
	 * the same VM started the drag. Otherwise, {@link #UNKNOWN_MODIFIERS} will be returned.
	 */
	...
	/**
	 * Override this to provide a custom image. The {@link Icon} returned by this method by default is <code>null</code>, which results
	 * in no drag image.
	 * 
	 * @param srcOffset
