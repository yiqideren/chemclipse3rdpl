 ...
 * <li>Transparency (constant alpha applied to window contents or transparent background)
 * <li>Fully transparent window (the transparency of all painted pixels is applied to the window).
 * </ul>
 ...
 * <p>
 * NOTE: Neither shaped windows nor transparency currently works with Java 1.4 under X11. This is at least partly due to 1.4 using multiple X11 windows for a single given Java window. It *might* be possible to remedy by applying the window region/transparency to all descendants, but I haven't tried it. In addition, windows must be both displayable <em>and</em> visible before the corresponding native Drawable may be obtained; in later Java versions, the window need only be displayable.
 * <p>
 * NOTE: If you use {@link #setWindowMask(Window,Shape)} and override {@link Window#paint(Graphics)} on OS X, you'll need to explicitly set the clip mask on the <code>Graphics</code> object with the window mask; only the content pane of the window and below have the window mask automatically applied.
 * <p>
 * NOTE: On OSX, the property <code>apple.awt.draggableWindowBackground</code> is set automatically when a window's background color has an alpha component. That property must be set to its final value <em>before</em> the heavyweight peer for the Window is created. Once {@link Component#addNotify} has been called on the component, causing creation of the heavyweight peer, changing this property has no effect.
 * 
...
public class WindowUtils {
	...
	/**
	 * This class forces a heavyweight popup on the parent {@link Window}. See the implementation of {@link PopupFactory};
	 * a heavyweight is forced if there is an occluding subwindow on the
	 * target window.
	 * <p>
	 * Ideally we'd have more control over {@link PopupFactory} but this is a fairly simple, lightweight workaround. Note that, at least as of JDK 1.6, the following do not have the desired effect:<br>
	 * <code><pre>
	...
	public static abstract class NativeWindowUtils {
		 ...
		 * be treated as a bitmap (zero/nonzero values only). A value of <code>null</code> means to remove the mask.
		 */
	...
	private static class W32WindowUtils extends NativeWindowUtils {
		...
		/**
		 * W32 makes the client responsible for repainting the <em>entire</em> window on any change. It also does not paint window decorations
	...
	private static class MacWindowUtils extends NativeWindowUtils {
		...
		/**
		 * Note that the property <code>apple.awt.draggableWindowBackground</code> must be set to its
	...
	private static class X11WindowUtils extends NativeWindowUtils {
		...
		private static Pixmap createBitmap(final Display dpy, X11.Window win, Raster raster) {
	 ...
	 * <p>
	 * NOTE: Windows requires that <code>sun.java2d.noddraw=true</code> in order for alpha to work.
	 * <p>
	 ...
	 * 
	 * On OSX, the property <code>apple.awt.draggableWindowBackground</code> must be set to its final value <em>before</em> the heavyweight peer for
