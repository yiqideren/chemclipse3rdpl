...
public abstract class OleAutoUtil {
	...
	public static SAFEARRAY.ByReference createVarArray(int size) {
		...
		psa = OleAuto.INSTANCE.SafeArrayCreate(new VARTYPE(Variant.VT_VARIANT), 1, rgsabound);
		return psa;
	}
	 ...
	 *            the arg
	 */
	public static void SafeArrayPutElement(SAFEARRAY array, long index, VARIANT arg) {
	...
	public static VARIANT SafeArrayGetElement(SAFEARRAY array, long index) {
		...
		HRESULT hr = OleAuto.INSTANCE.SafeArrayGetElement(array, idx, result.getPointer());
		COMUtils.SUCCEEDED(hr);
		return result;
	}
}
