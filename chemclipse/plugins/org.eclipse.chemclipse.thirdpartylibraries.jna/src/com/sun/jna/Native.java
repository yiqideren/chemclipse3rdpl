 ...
 * When JNA classes are loaded, the native shared library (jnidispatch) is loaded as well. An attempt is made to load it from the any paths defined in <code>jna.boot.library.path</code> (if defined), then the system library path using {@link System#loadLibrary}, unless <code>jna.nosys=true</code>. If not found, the appropriate library will be extracted from the class path (into a temporary directory if found within a jar file) and loaded from there, unless <code>jna.noclasspath=true</code>. If
 * your system has additional security constraints regarding execution or load of files (SELinux, for example), you should probably install the native library in an accessible location and configure your system accordingly, rather than relying on JNA to extract the library from its own jar file.
 * <p/>
 * To avoid the automatic unpacking (in situations where you want to force a failure if the JNA native library is not properly installed on the system), set the system property <code>jna.nounpack=true</code>.
 * <p/>
 ...
 * Native libraries loaded via {@link #loadLibrary(Class)} may be found in <a href="NativeLibrary.html#library_search_paths">several locations</a>.
 * 
 * @see Library
...
public final class Native implements Version {
	...
	private static final UncaughtExceptionHandler DEFAULT_HANDLER = new UncaughtExceptionHandler() {
	 ...
	 * <p/>
	 * NOTE: The preferred method of obtaining the last error result is to declare your mapped method to throw {@link LastErrorException}.
	 *
	 ...
	 * after every invocation. Always returns <code>true</code>
	 * <p>
	 * NOTE: The preferred method of obtaining the last error result is to declare your mapped method to throw {@link LastErrorException}.
	 *
	 * @deprecated Last error is always preserved
	...
	/**
	 * Utility method to get the native window ID for a Java {@link Window} as a <code>long</code> value.
	 * This method is primarily for X11-based systems, which use an opaque <code>XID</code> (usually <code>long int</code>) to identify windows.
	 * 
	...
	/**
	 * Utility method to get the native window ID for a heavyweight Java {@link Component} as a <code>long</code> value.
	 * This method is primarily for X11-based systems, which use an opaque <code>XID</code> (usually <code>long int</code>) to identify windows.
	 * 
	 * @throws HeadlessException
	 *             if the current VM is running headless
	...
	/**
	 * Utility method to get the native window pointer for a Java {@link Window} as a {@link Pointer} value. This method is primarily for
	 * w32, which uses the <code>HANDLE</code> type (actually <code>void *</code>) to identify windows.
	 * 
	 * @throws HeadlessException
	 *             if the current VM is running headless
	 */
	...
	/**
	 * Utility method to get the native window pointer for a heavyweight Java {@link Component} as a {@link Pointer} value. This method is primarily
	 * for w32, which uses the <code>HWND</code> type (actually <code>void *</code>) to identify windows.
	 * 
	 * @throws HeadlessException
	 *             if the current VM is running headless
	 */
	 ...
	 * <p>
	 * If <code>name</code> is null, attempts to map onto the current process. Native libraries loaded via this method may be found in <a href="NativeLibrary.html#library_search_paths">several locations</a>.
	 * 
	 * @param name
	 * @param interfaceClass
	 * @param options
	 ...
	 * from the fields (by order of precedence) <code>OPTIONS</code> (a {@link Map}), <code>TYPE_MAPPER</code> (a {@link TypeMapper}), <code>STRUCTURE_ALIGNMENT</code> (an {@link Integer}), and <code>STRING_ENCODING</code> (a {@link String}).
	 * 
	 * @see Library
	 */
	 ...
	 *            name is given, the resource path is attempted both with and without {@link Platform#RESOURCE_PREFIX}, after mapping the library name via {@link NativeLibrary#mapSharedLibraryName(String)}.
	...
	/**
	 * Retrieve the last error set by the OS. This corresponds to <code>GetLastError()</code> on Windows, and <code>errno</code> on
	 ...
	 * result is undefined if {@link #getPreserveLastError} is <code>false</code>.
	 * <p>
	 * The preferred method of obtaining the last error result is to declare your mapped method to throw {@link LastErrorException} instead.
	 */
	 ...
	 * library. This value may be used to properly set <code>jna.library.path</code> so that JNA can load libraries identified
	 * by the &lt;nativelib&gt; tag in the JNLP configuration file. Returns <code>null</code> if the Web Start native library cache location can not
	 * be determined. Note that the path returned may be different for any
	 * given library name.
	 * <p>
	 * Use <code>System.getProperty("javawebstart.version")</code> to detect whether your code is running under Web Start.
	 * 
	 * @throws UnsatisfiedLinkError
	 ...
	 * assumed to be <code>struct</code> pointers unless they implement {@link Structure.ByValue}.
	 */
	...
	private static native long registerMethod(Class cls, String name, String signature, int[] conversions, long[] closure_arg_types, long[] arg_types, int rconversion, long closure_rtype, long rtype, Class rclass, long fptr, int callingConvention, boolean throwLastError, ToNativeConverter[] toNative, FromNativeConverter fromNative, String encoding);
	 ...
	 */
	static synchronized native long createNativeCallback(Callback callback, Method method, Class[] parameterTypes, Class returnType, int callingConvention, int flags, String encoding);
	 ...
	 *            Arguments to pass to the native function
	 * @param memory
	 *            Memory for pre-allocated structure to hold the result
	 * @param typeInfo
	 *            Native type information for the Structure
	 */
	private static native void invokeStructure(long fp, int callFlags, Object[] args, long memory, long type_info);
