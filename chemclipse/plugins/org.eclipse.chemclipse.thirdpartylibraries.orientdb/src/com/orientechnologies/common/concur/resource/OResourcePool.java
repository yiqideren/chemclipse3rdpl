...
public class OResourcePool<K, V> {
	...
	public boolean returnResource(final V res) {
		...
		if(resourcesOut.remove(res)) {
			...
			sem.release();
			if(OLogManager.instance().isDebugEnabled())
				OLogManager.instance().debug(this, "pool:'%s' returned resource '%s' available %d out %d", this, res, sem.availablePermits(), resourcesOut.size());
		}
		return true;
	}
	...
	public void remove(final V res) {
		...
		if(resourcesOut.remove(res)) {
			...
			sem.release();
			if(OLogManager.instance().isDebugEnabled())
				OLogManager.instance().debug(this, "pool:'%s' removed resource '%s' available %d out %d", this, res, sem.availablePermits(), resourcesOut.size());
		}
	}
