...
public class OUnsafeBinaryConverter implements OBinaryConverter {
	...
	public short getShort(byte[] buffer, int index, ByteOrder byteOrder) {
		...
		if(byteOrder.equals(ByteOrder.BIG_ENDIAN))
			return (short)((theUnsafe.getByte(buffer, index + BYTE_ARRAY_OFFSET)) << 8 | (theUnsafe.getByte(buffer, index + BYTE_ARRAY_OFFSET + 1) & 0xff));
	...
	public char getChar(byte[] buffer, int index, ByteOrder byteOrder) {
		...
		if(byteOrder.equals(ByteOrder.BIG_ENDIAN))
			return (char)((theUnsafe.getByte(buffer, index + BYTE_ARRAY_OFFSET) << 8) | (theUnsafe.getByte(buffer, index + BYTE_ARRAY_OFFSET + 1) & 0xff));
