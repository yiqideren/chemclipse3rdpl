...
public class OObjectProxyMethodHandler implements MethodHandler {
	 ...
	 * @throws NoSuchMethodException
	 */
	public void detachAll(final Object self, final boolean nonProxiedInstance, final Map<Object, Object> alreadyDetached) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
	...
	public void attach(final Object self) throws IllegalArgumentException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
		...
		for(Class<?> currentClass = self.getClass(); currentClass != Object.class;) {
			if(ProxyObject.class.isAssignableFrom(currentClass)) {
	...
	protected Object manageGetMethod(final Object self, final Method m, final Method proceed, final Object[] args) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, SecurityException, IllegalArgumentException, NoSuchFieldException {
	...
	protected Object getValue(final Object self, final String fieldName, final boolean idOrVersionField, Object value) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
			...
			if(value == null) {
				...
				if(((value instanceof Collection<?> || value instanceof Map<?, ?>) && !(value instanceof OObjectLazyMultivalueElement)) || value.getClass().isArray()) {
					...
					if(genericMultiValueType == null || !OReflectionHelper.isJavaType(genericMultiValueType)) {
						...
						} else if((value instanceof Set || value instanceof Map) && loadedFields.get(fieldName).compareTo(doc.getRecordVersion()) < 0 && !OReflectionHelper.isJavaType(genericMultiValueType)) {
	...
	protected Object manageObjectCollections(final Object self, final String fieldName, Object value) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
	...
	protected Object manageArraySave(final String iFieldName, final Object[] value) {
		...
		if(value.length > 0) {
			...
			if(o instanceof ProxyObject || o.getClass().isEnum()) {
	...
	protected void convertArray(final Object[] value, Object[] newValue, boolean isEnum) {
		...
		for(int i = 0; i < value.length; i++) {
			if(isEnum) {
				...
				newValue[i] = value[i] != null ? OObjectEntitySerializer.getDocument((ProxyObject)value[i]) : null;
			}
		}
	}
		...
		if(((Object[])value).length > 0) {
			...
			if(o instanceof OIdentifiable) {
				...
				for(int i = 0; i < ((Object[])value).length; i++) {
					...
					newValue[i] = OObjectEntitySerializer.getDocument((ProxyObject)doc);
				}
				value = newValue;
			} else {
				final Class genericType = OReflectionHelper.getGenericMultivalueType(f);
	...
	protected Object convertDocumentToObject(final ODocument value, final Object self) {
		...
		if(value == null)
			return null;
		return OObjectEntityEnhancer.getInstance().getProxiedInstance(value.getClassName(), getDatabase().getEntityManager(), value, (self instanceof ProxyObject ? (ProxyObject)self : null));
	}

	protected Object manageSetMethod(final Object self, final Method m, final Method proceed, final Object[] args) throws IllegalAccessException, InvocationTargetException {
	...
	protected Object setValue(final Object self, final String fieldName, Object valueToSet) {
		...
		if(valueToSet == null) {
			...
			if(valueToSet instanceof ProxyObject) {
				ODocument docToSet = OObjectEntitySerializer.getDocument((ProxyObject)valueToSet);
				...
				if(OObjectEntitySerializer.isToSerialize(valueToSet.getClass())) {
					...
					final ODocument docToSet = OObjectEntitySerializer.getDocument((ProxyObject)valueToSet);
					if(OObjectEntitySerializer.isEmbeddedField(self.getClass(), fieldName))
