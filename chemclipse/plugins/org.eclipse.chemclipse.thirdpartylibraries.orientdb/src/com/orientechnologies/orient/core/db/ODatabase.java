 ...
 * <li>Maximum records per database = <b>302.231.454.903.657 Billions</b>: 2^15 clusters x 2^63 records = (2^78) 32.768 * 9,223.372.036.854.775.808 = 302.231,454.903.657.293.676.544 records</li>
 * <li>Maximum storage per database = <b>19.807.040.628.566.084 Terabytes</b>: 2^31 data-segments x 2^63 bytes = (2^94) 2.147.483.648 x 9,223.372.036.854.775.808 Exabytes = 19.807,040.628.566.084.398.385.987.584 Yottabytes</li>
 * </ul>
 *
 * @author Luca Garulli
 *
...
public interface ODatabase<T> extends OBackupable, Closeable, OUserObject2RecordHandler {
	 ...
	 * will be called or exception will be thrown on attempt to modify DB data. Concrete behaviour depends on <code>throwException</code> parameter.
	 *
	 * IMPORTANT: This command is not reentrant.
	 *
	 * @param throwException
	 *            If <code>true</code> {@link com.orientechnologies.common.concur.lock.OModificationOperationProhibitedException} exception will be thrown in case of write command will be performed.
	 */
	 ...
	 *
	 * After this call users can perform only select queries. All write-related commands will queued till {@link #releaseCluster(int)} command will be called.
	 *
	 * Given command waits till all on going modifications in indexes or DB will be finished.
	 *
	 * IMPORTANT: This command is not reentrant.
	 ...
	 */
	@Deprecated
	<RET extends T> RET load(T iObject, String iFetchPlan, boolean iIgnoreCache, boolean loadTombstone, OStorage.LOCKING_STRATEGY iLockingStrategy);
	 ...
	 */
	@Deprecated
	<RET extends T> RET load(T iObject, String iFetchPlan, boolean iIgnoreCache, boolean iUpdateCache, boolean loadTombstone, OStorage.LOCKING_STRATEGY iLockingStrategy);
	 ...
	 * @deprecated Usage of this method may lead to deadlocks.
	 */
	<RET extends T> RET load(ORID iRecordId, String iFetchPlan, boolean iIgnoreCache, boolean loadTombstone, OStorage.LOCKING_STRATEGY iLockingStrategy);

	@Deprecated
	/**
	 * @deprecated Usage of this method may lead to deadlocks.
	 */
	<RET extends T> RET load(ORID iRecordId, String iFetchPlan, boolean iIgnoreCache, boolean iUpdateCache, boolean loadTombstone, OStorage.LOCKING_STRATEGY iLockingStrategy);
	 ...
	 */
	<RET extends T> RET save(T iObject, OPERATION_MODE iMode, boolean iForceCreate, ORecordCallback<? extends Number> iRecordCreatedCallback, ORecordCallback<ORecordVersion> iRecordUpdatedCallback);
	 ...
	 * @param iRecordUpdatedCallback
	 */
	<RET extends T> RET save(T iObject, String iClusterName, OPERATION_MODE iMode, boolean iForceCreate, ORecordCallback<? extends Number> iRecordCreatedCallback, ORecordCallback<ORecordVersion> iRecordUpdatedCallback);
	 ...
	 * closed before to start a new one. A transaction once begun has to be closed by calling the {@link #commit()} or {@link #rollback()}.
	 *
	 * @return The Database instance itself giving a "fluent interface". Useful to call multiple methods in chain.
	 */
	ODatabase<T> begin();
	 ...
	 *
	 * @param iObject
	 *            The object passed change based on the Database implementation: records for {@link com.orientechnologies.orient.core.db.document.ODatabaseDocument} implementations and POJO for {@link com.orientechnologies.orient.core.db.object.ODatabaseObject} implementations.
