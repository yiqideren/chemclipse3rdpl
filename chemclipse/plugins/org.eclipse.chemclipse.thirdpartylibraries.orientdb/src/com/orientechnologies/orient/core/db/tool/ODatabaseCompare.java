...
public class ODatabaseCompare extends ODatabaseImpExpAbstract {
	...
	public ODatabaseCompare(String iDb1URL, String iDb2URL, final String userName, final String userPassword, final OCommandOutputListener iListener) throws IOException {
	...
	private void compareSchama() {
		...
		for(OClass clazz : schema1.getClasses()) {
			...
			if((clazz.getJavaClass() == null && clazz2.getJavaClass() != null) || (clazz.getJavaClass() != null && !clazz.getJavaClass().equals(clazz2.getJavaClass()))) {
		...
		for(String clusterName : storage1.getClusterNames()) {
			...
			while(physicalPositions.length > 0) {
				for(OPhysicalPosition physicalPosition : physicalPositions) {
					...
					if(isDocumentDatabases() && rid.equals(new ORecordId(storage1.getConfiguration().indexMgrRecordId)) && rid.equals(new ORecordId(storage2.getConfiguration().indexMgrRecordId)))
						continue;
					if(isDocumentDatabases() && rid.equals(new ORecordId(storage1.getConfiguration().schemaRecordId)) && rid.equals(new ORecordId(storage2.getConfiguration().schemaRecordId)))
						continue;
