...
public class OMVRBTreeEntryPersistent<K, V> extends OMVRBTreeEntry<K, V> implements OIdentityChangedListener {
	 ...
	 */
	public OMVRBTreeEntryPersistent(final OMVRBTreePersistent<K, V> iTree, final OMVRBTreeEntryPersistent<K, V> iParent, final ORID iRecordId) {
	 ...
	 */
	public OMVRBTreeEntryPersistent(final OMVRBTreePersistent<K, V> iTree, final K iKey, final V iValue, final OMVRBTreeEntryPersistent<K, V> iParent) {
	...
	public void checkEntryStructure() {
		...
		if(left != null) {
			...
			if(left.parent != this)
				OLogManager.instance().error(this, "checkEntryStructure: Left node is not correctly connected to the parent" + dataProvider.getLeft());
		}
		if(right != null) {
			...
			if(right.parent != this)
				OLogManager.instance().error(this, "checkEntryStructure: Right node is not correctly connected to the parent" + dataProvider.getRight());
		}
	}
		...
		if(parent != null) {
			...
			if(canDisconnectFrom(parent) || iForceDirty) {
				if(parent.left == this) {
					...
					OLogManager.instance().warn(this, "Node " + rid + " has the parent (" + parent + ") unlinked to itself. It links to " + parent);
		...
		if(left != null) {
			...
			if(canDisconnectFrom(left) || iForceDirty) {
				if(left.parent == this)
					...
					OLogManager.instance().warn(this, "Node " + rid + " has the left (" + left + ") unlinked to itself. It links to " + left.parent);
		...
		if(right != null) {
			...
			if(canDisconnectFrom(right) || iForceDirty) {
				if(right.parent == this)
					...
					OLogManager.instance().warn(this, "Node " + rid + " has the right (" + right + ") unlinked to itself. It links to " + right.parent);
