...
public abstract class ODatabaseRecordWrapperAbstract<DB extends ODatabaseDocumentInternal> extends ODatabaseWrapperAbstract<DB, ORecord> implements ODatabaseDocumentInternal {
	...
	public <REC extends ORecord> ORecordIteratorCluster<REC> browseCluster(final String iClusterName, final Class<REC> iRecordClass, long startClusterPosition, long endClusterPosition, final boolean loadTombstones) {
	...
	public <RET extends ORecord> RET save(final ORecord iRecord, final String iClusterName, final OPERATION_MODE iMode, boolean iForceCreate, final ORecordCallback<? extends Number> iRecordCreatedCallback, ORecordCallback<ORecordVersion> iRecordUpdatedCallback) {
	...
	public <DBTYPE extends ODatabaseDocument> DBTYPE checkSecurity(final ORule.ResourceGeneric iResourceGeneric, final int iOperation, final Object iResourceSpecific) {

		return (DBTYPE)underlying.checkSecurity(iResourceGeneric, iOperation, iResourceSpecific);
	}

	public <DBTYPE extends ODatabaseDocument> DBTYPE checkSecurity(final ORule.ResourceGeneric iResourceGeneric, final int iOperation, final Object... iResourcesSpecific) {
	...
	protected void checkClusterBoundedToClass(final int iClusterId) {
		...
		for(OClass clazz : ((OMetadataInternal)getMetadata()).getImmutableSchemaSnapshot().getClasses()) {
			if(clazz.getDefaultClusterId() == iClusterId)
				throw new OSchemaException("Cannot drop the cluster '" + getClusterNameById(iClusterId) + "' because the classes ['" + clazz.getName() + "'] are bound to it. Drop these classes before dropping the cluster");
