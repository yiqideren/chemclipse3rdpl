...
public abstract class OAbstractFile implements OFile {
	...
	public boolean open() throws IOException {
		...
		try {
			...
			if(filledUpTo > fileSize || filledUpTo < 0)
				OLogManager.instance().error(this, "Invalid filledUp size (=" + filledUpTo + "). The file could be corrupted", null, OStorageException.class);
			if(failCheck) {
	...
	public void lock() throws IOException {
		...
		try {
			for(int i = 0; i < LOCK_MAX_RETRIES; ++i) {
				...
				if(fileLock == null)
					throw new OLockException("File '" + osFile.getPath() + "' is locked by another process, maybe the database is in use by another process. Use the remote mode with a OrientDB server to allow multiple access to the same database.");
			}
		} finally {
			releaseWriteLock();
		}
	...
	protected void checkSize(final long iSize) throws IOException {
		...
		try {
			...
			if(iSize < filledUpTo)
				OLogManager.instance().error(this, "You cannot resize down the file to " + iSize + " bytes, since it is less than current space used: " + filledUpTo + " bytes", OIOException.class);
		} finally {
			releaseReadLock();
		}
	}
		...
		try {
			...
			if(getFreeSpace() < iSize) {
				if(maxSize > 0 && maxSize - size < iSize)
					throw new IllegalArgumentException("Cannot enlarge file since the configured max size (" + OFileUtils.getSizeAsString(maxSize) + ") was reached! " + toString());
				// MAKE ROOM
	...
	protected long checkRegions(final long iOffset, final long iLength) {
		...
		try {
			if(iOffset < 0 || iOffset + iLength > getFilledUpTo())
				throw new OIOException("You cannot access outside the file size (" + getFilledUpTo() + " bytes). You have requested portion " + iOffset + "-" + (iOffset + iLength) + " bytes. File: " + toString());
			return iOffset;
		} finally {
			releaseReadLock();
		}
