 ...
 * algorithm</a>. The directory for extindible hashing is implemented in {@link com.orientechnologies.orient.core.index.hashindex.local.OHashTableDirectory} class. Directory is not implemented according
 ...
 * <li>The more amount of memory is affected in operation the less speed we will have. In worst case 60 kb of memory should be moved.</li>
 * </ol>
 * 
...
public class OLocalHashTable<K, V> extends ODurableComponent {
	...
	public OLocalHashTable(String metadataConfigurationFileExtension, String treeStateFileExtension, String bucketFileExtension, String nullBucketFileExtension, OHashFunction<K> keyHashFunction, boolean durableInNonTxMode, OAbstractPaginatedStorage abstractPaginatedStorage) {
	...
	public void create(String name, OBinarySerializer<K> keySerializer, OBinarySerializer<V> valueSerializer, OType[] keyTypes, boolean nullKeyIsSupported) {
	...
	public void deleteWithoutLoad(String name, OAbstractPaginatedStorage storageLocal) {
		...
		try {
			...
			if(isFileExists(atomicOperation, name + metadataConfigurationFileExtension, diskCache)) {
				...
				if(isFileExists(atomicOperation, fileStateId, diskCache))
	...
	private BucketPath nextLevelUp(BucketPath bucketPath) throws IOException {
		...
		if(nextParentIndex < MAX_LEVEL_SIZE)
			return new BucketPath(parent.parent, 0, nextParentIndex, parent.nodeIndex, parent.nodeLocalDepth, parent.nodeGlobalDepth);
		return nextLevelUp(new BucketPath(parent.parent, 0, MAX_LEVEL_SIZE - 1, parent.nodeIndex, parent.nodeLocalDepth, parent.nodeGlobalDepth));
	}
		...
		while(nodePath != null) {
			...
			for(int i = endIndex; i >= startIndex; i--) {
				...
				if(position < 0) {
					...
					final BucketPath parent = new BucketPath(nodePath.parent, 0, i, nodePath.nodeIndex, nodePath.nodeLocalDepth, nodePath.nodeGlobalDepth);
	...
	private void mergeBucketsAfterDeletion(BucketPath nodePath, OHashIndexBucket<K, V> bucket, OAtomicOperation atomicOperation) throws IOException {
		...
		try {
			...
			hashStateEntry.acquireExclusiveLock();
			try {
				buddyBucket = new OHashIndexBucket<K, V>(buddyCacheEntry, keySerializer, valueSerializer, keyTypes, getChangesTree(atomicOperation, buddyCacheEntry));
				...
				try {
					final OHashIndexBucket<K, V> newBuddyBucket = new OHashIndexBucket<K, V>(bucketDepth - 1, newBuddyCacheEntry, keySerializer, valueSerializer, keyTypes, getChangesTree(atomicOperation, newBuddyCacheEntry));
	...
	private void doPut(K key, V value, OAtomicOperation atomicOperation) throws IOException {
		...
		if(key == null) {
			...
			cacheEntry.acquireExclusiveLock();
			try {
				ONullBucket<V> nullBucket = new ONullBucket<V>(cacheEntry, getChangesTree(atomicOperation, cacheEntry), valueSerializer, isNew);
				...
				if(bucketDepth <= bucketPath.nodeGlobalDepth) {
					...
					if(bucketPath.nodeLocalDepth < MAX_LEVEL_DEPTH) {
						...
						if(updatedOffset < MAX_LEVEL_SIZE) {
							...
							final BucketPath updatedBucketPath = new BucketPath(bucketPath.parent, updatedOffset, updatedItemIndex, bucketPath.nodeIndex, nodeLocalDepth, updatedGlobalDepth);
							...
							final BucketPath newBucketPath = new BucketPath(bucketPath.parent, updatedOffset - MAX_LEVEL_SIZE, updatedItemIndex, newNodeIndex, nodeLocalDepth, updatedGlobalDepth);
							updateNodeAfterBucketSplit(newBucketPath, bucketDepth, newBucketPointer, updatedBucketPointer);
						}
						updateNodesAfterSplit(bucketPath, bucketPath.nodeIndex, newNode, nodeLocalDepth, hashMapSize, allLeftHashMapsEqual, allRightHashMapsEqual, newNodeIndex);
	...
	private void updateNodesAfterSplit(BucketPath bucketPath, int nodeIndex, long[] newNode, int nodeLocalDepth, int hashMapSize, boolean allLeftHashMapEquals, boolean allRightHashMapsEquals, int newNodeIndex) throws IOException {
		...
		if(allRightHashMapsEquals) {
			...
			for(int i = 0; i < pointersSize; i++)
				directory.setNodePointer(parentNodeIndex, startIndex + pointersSize + i, (newNodeIndex << 8) | (i * hashMapSize) | Long.MIN_VALUE);
		}
	...
	private BucketSplitResult splitBucket(OHashIndexBucket<K, V> bucket, int fileLevel, long pageIndex, OAtomicOperation atomicOperation) throws IOException {
		...
		try {
			...
			if(tombstoneIndex >= 0) {
				...
				try {
					final OHashIndexBucket<K, V> tombstone = new OHashIndexBucket<K, V>(tombstoneCacheEntry, keySerializer, valueSerializer, keyTypes, getChangesTree(atomicOperation, tombstoneCacheEntry));
			...
			try {
				...
				try {
					final OHashIndexBucket<K, V> updatedBucket = new OHashIndexBucket<K, V>(newBucketDepth, updatedBucketCacheEntry, keySerializer, valueSerializer, keyTypes, getChangesTree(atomicOperation, updatedBucketCacheEntry));
