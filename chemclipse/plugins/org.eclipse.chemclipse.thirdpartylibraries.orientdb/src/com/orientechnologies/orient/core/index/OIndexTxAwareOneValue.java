...
public class OIndexTxAwareOneValue extends OIndexTxAware<OIdentifiable> {
		...
		if(!database.getTransaction().isActive()) {
			...
			if(previousRecord != null && !previousRecord.equals(iRecord)) {
				...
				if(mergeSameKey) {
					...
					throw new ORecordDuplicatedException(String.format("Cannot index record %s: found duplicated key '%s' in index '%s' previously assigned to the record %s", iRecord, iKey, getName(), previousRecord), previousRecord.getIdentity());
			}
	...
	@Override
	public OIndexCursor iterateEntriesBetween(final Object fromKey, final boolean fromInclusive, final Object toKey, final boolean toInclusive, final boolean ascOrder) {
