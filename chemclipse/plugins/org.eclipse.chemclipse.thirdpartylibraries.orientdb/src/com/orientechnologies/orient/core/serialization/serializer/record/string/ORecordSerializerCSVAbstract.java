...
public abstract class ORecordSerializerCSVAbstract extends ORecordSerializerStringAbstract {
		...
		if(iLinked instanceof ORID) {
			...
			if((rid.isNew() && !rid.isTemporary()) || iLinkedRecord.isDirty()) {
				...
				if(iLinkedRecord instanceof ODocument) {
					...
					database.save(iLinkedRecord, schemaClass != null && database.getStorage().isAssigningClusterIds() ? database.getClusterNameById(schemaClass.getClusterForNewInstance((ODocument)iLinkedRecord)) : null);
				} else
	...
	public void fieldToStream(final ODocument iRecord, final StringBuilder iOutput, OUserObject2RecordHandler iObjHandler, final OType iType, final OClass iLinkedClass, final OType iLinkedType, final String iName, final Object iValue, final Map<ODocument, Boolean> iMarshalledRecords, final boolean iSaveOnlyDirty) {
		...
		switch(iType) {
			...
			case EMBEDDED:
				...
				break;
			case EMBEDDEDLIST:
				embeddedCollectionToStream(null, iObjHandler, iOutput, iLinkedClass, iLinkedType, iValue, iMarshalledRecords, iSaveOnlyDirty, false);
				...
				break;
			case EMBEDDEDSET:
				embeddedCollectionToStream(null, iObjHandler, iOutput, iLinkedClass, iLinkedType, iValue, iMarshalledRecords, iSaveOnlyDirty, true);
	...
	public void embeddedMapToStream(ODatabase<?> iDatabase, final OUserObject2RecordHandler iObjHandler, final StringBuilder iOutput, final OClass iLinkedClass, OType iLinkedType, final Object iValue, final Map<ODocument, Boolean> iMarshalledRecords, final boolean iSaveOnlyDirty) {
