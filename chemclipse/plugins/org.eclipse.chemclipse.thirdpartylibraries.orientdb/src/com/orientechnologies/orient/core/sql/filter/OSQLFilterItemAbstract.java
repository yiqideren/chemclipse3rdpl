...
public abstract class OSQLFilterItemAbstract implements OSQLFilterItem {
	...
	public OSQLFilterItemAbstract(final OBaseParser iQueryToParse, final String iText) {
		...
		final List<String> parts = OStringSerializerHelper.smartSplit(iText, new char[]{'.', '[', ']'}, new boolean[]{false, false, true}, new boolean[]{false, true, false}, 0, -1, false, true, false, false, new char[]{});
		...
		if(parts.size() > 1) {
			...
			for(int i = 1; i < parts.size(); ++i) {
				...
				if(part.charAt(0) == '[')
					operationsChain.add(new OPair<OSQLMethodRuntime, Object[]>(new OSQLMethodRuntime(OSQLEngine.getMethod(OSQLMethodMultiValue.NAME)), new Object[]{part}));
				else if(pindex > -1) {
					...
					if(method != null) {
						if(method.getMaxParams() == -1 || method.getMaxParams() > 0) {
							...
							if(arguments.length < method.getMinParams() || (method.getMaxParams() > -1 && arguments.length > method.getMaxParams()))
								throw new OQueryParsingException(iQueryToParse.parserText, "Syntax error: field operator '" + method.getName() + "' needs " + (method.getMinParams() == method.getMaxParams() ? method.getMinParams() : method.getMinParams() + "-" + method.getMaxParams()) + " argument(s) while has been received " + arguments.length, 0);
						} else
							arguments = null;
					} else {
						// LOOK FOR FUNCTION
						...
						if(f.getMaxParams() == -1 || f.getMaxParams() > 0) {
							...
							if(arguments.length + 1 < f.getMinParams() || (f.getMaxParams() > -1 && arguments.length + 1 > f.getMaxParams()))
								throw new OQueryParsingException(iQueryToParse.parserText, "Syntax error: function '" + f.getName() + "' needs " + (f.getMinParams() == f.getMaxParams() ? f.getMinParams() : f.getMinParams() + "-" + f.getMaxParams()) + " argument(s) while has been received " + arguments.length, 0);
						} else
							arguments = null;
