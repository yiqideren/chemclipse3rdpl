...
public class O2QCache implements OReadCache, O2QCacheMXBean {
	...
	@Override
	public OCacheEntry load(long fileId, final long pageIndex, final boolean checkPinnedPages, OWriteCache writeCache) throws IOException {
	...
	private UpdateCacheResult updateCache(final long fileId, final long pageIndex, final boolean addNewPages, OWriteCache writeCache) throws IOException {
		...
		while(am.size() + a1in.size() > maxSize && iterationsCounter < 1000) {
			...
			if(a1in.size() > K_IN) {
				...
				if(removedFromAInEntry == null) {
					...
					try {
						pageLock = pageLockManager.acquireExclusiveLock(new PageKey(removedFromAInEntry.getFileId(), removedFromAInEntry.getPageIndex()));
						try {
