 ...
 * </ol>
 * <p>
 * If you want to add new data but not to change existing one and you do not have enough space to add new data use method {@link #allocateNewPage(long, OWriteCache)} instead of {@link #load(long, long, boolean, OWriteCache)}.
 * <p>
 * {@link #load(long, long, boolean, OWriteCache)} method has checkPinnedPages parameter. Pinned pages are pages which are kept always loaded in RAM ,this class of pages is needed for some data structures usually this attribute should be set to <code>false</code> and it is set to <code>true</code> when storage goes through data restore procedure after system crash.
 *
