...
public class OIndexRebuildOutputListener implements OProgressListener {
	...
	public boolean onProgress(final Object iTask, final long iCounter, final float iPercent) {
		...
		if(now - lastDump > 10000) {
			...
			if(rebuild)
				OLogManager.instance().info(this, "--> %3.2f%% progress, %,d indexed so far (%,d items/sec)", iPercent, iCounter, ((iCounter - lastCounter) / 10));
			else
				OLogManager.instance().debug(this, "--> %3.2f%% progress, %,d indexed so far (%,d items/sec)", iPercent, iCounter, ((iCounter - lastCounter) / 10));
			lastDump = now;
	...
	public void onCompletition(final Object iTask, final boolean iSucceed) {
		...
		if(idxSize > 0)
			if(rebuild)
				...
				OLogManager.instance().debug(this, "--> OK, indexed %,d items in %,d ms", idxSize, (System.currentTimeMillis() - startTime));
	}
}
