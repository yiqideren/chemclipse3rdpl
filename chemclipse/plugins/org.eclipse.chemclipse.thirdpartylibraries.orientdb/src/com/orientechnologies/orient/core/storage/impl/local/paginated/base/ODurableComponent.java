 ...
 * <ol>
 * <li>"atomic operation" - set of operations which should be either applied together or not. It includes not only changes on current data structure but on all durable data structures which are used by current one during implementation of specific operation.</li>
 ...
 * <li>If changes should be isolated till operation completes call also {@link #lockTillAtomicOperationCompletes()} which will apply exclusive lock on existing data structure till atomic operation completes. This lock is not replacement of data structure locking system it is used to serialize access to set of components which participate in single atomic operation. It is kind of rudiment lock manager which is used to isolate access to units which participate in single transaction and which is
 * going to be created to provide efficient multi core scalability feature. It is recommended to always call it just after start of atomic operation but always remember it is not replacement of thread safety mechanics for current data structure it is a mean to provide isolation between atomic operations.</li>
 * <li>Call {@link #endAtomicOperation(boolean)} method when atomic operation completes, passed in parameter should be <code>false</code> if atomic operation completes with success and <code>true</code> if there were some exceptions and it is needed to rollback given operation.</li>
 * </ol>
 * 
 * 
