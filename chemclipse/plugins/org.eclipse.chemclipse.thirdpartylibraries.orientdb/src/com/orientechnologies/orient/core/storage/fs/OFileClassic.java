	...
	public int readInt(long iOffset) throws IOException {
		...
		while(true) {
			try {
				...
				OLogManager.instance().error(this, "Error during read of int data for file " + getName() + " " + attempts + "-th attempt.", e);
				reopenFile(attempts, e);
			}
		}
	}
	...
	public void writeLong(long iOffset, final long iValue) throws IOException {
		...
		while(true) {
			try {
				...
				OLogManager.instance().error(this, "Error during write of long data for file " + getName() + " " + attempts + "-th attempt", e);
				reopenFile(attempts, e);
			}
		}
	}
	...
	private long checkRegions(final long iOffset, final long iLength) {
		...
		try {
			if(iOffset < 0 || iOffset + iLength > size)
				throw new OIOException("You cannot access outside the file size (" + size + " bytes). You have requested portion " + iOffset + "-" + (iOffset + iLength) + " bytes. File: " + toString());
