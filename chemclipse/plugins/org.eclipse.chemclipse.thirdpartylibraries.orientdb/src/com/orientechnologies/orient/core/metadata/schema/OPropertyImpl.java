...
public class OPropertyImpl extends ODocumentWrapperNoClass implements OProperty {
	...
	void setLinkedClassInternal(final OClass iLinkedClass) {
		...
		try {
			...
			if(type == OType.LINK || type == OType.LINKSET || type == OType.LINKLIST || type == OType.LINKMAP || type == OType.EMBEDDED || type == OType.EMBEDDEDSET || type == OType.EMBEDDEDLIST || type == OType.EMBEDDEDMAP)
			...
			if((this.collate != null && !this.collate.equals(oldCollate)) || (this.collate == null && oldCollate != null)) {
				...
				if(!indexesToRecreate.isEmpty()) {
					...
					for(OIndex<?> indexToRecreate : indexesToRecreate) {
						final OIndexInternal.IndexMetadata indexMetadata = indexToRecreate.getInternal().loadMetadata(indexToRecreate.getConfiguration());
