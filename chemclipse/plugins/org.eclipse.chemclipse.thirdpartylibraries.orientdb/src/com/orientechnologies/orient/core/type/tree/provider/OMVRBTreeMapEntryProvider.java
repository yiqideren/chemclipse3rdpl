...
public class OMVRBTreeMapEntryProvider<K, V> extends OMVRBTreeEntryDataProviderAbstract<K, V> {
	...
	public K getKeyAt(final int iIndex) {
		...
		if(k == null)
			try {
				...
				OLogManager.instance().error(this, "Cannot lazy load the key #" + iIndex + " in tree node " + this, e, OSerializationException.class);
			}
		return k;
	}
	...
	public V getValueAt(final int iIndex) {
		...
		if(v == null)
			try {
				...
				OLogManager.instance().error(this, "Cannot lazy load the value #" + iIndex + " in tree node " + this, e, OSerializationException.class);
			}
		return v;
	}
	...
	private int deserializeMetadata(byte[] inBuffer) {
		...
		if(currentVersion != CURRENT_VERSION)
			throw new OSerializationException("MVRBTree node is stored using " + currentVersion + " version of serialization format but current version is " + CURRENT_VERSION + ".");
		...
		offset += OIntegerSerializer.INT_SIZE;
		if(size > pageSize)
			throw new OConfigurationException("Loaded index with page size set to " + pageSize + " while the loaded was built with: " + size);
		return offset;
	}
	...
	private byte[] convertIntoNewSerializationFormat(byte[] stream) throws IOException {
		...
		try {
			...
			byte[] result;
			if(((OMVRBTreeMapProvider<K, V>)treeDataProvider).valueSerializer instanceof OBinarySerializer)
				result = convertNewSerializationFormatBinarySerializer(oldSize, oldPageSize, oldParentRid, oldLeftRid, oldRightRid, oldColor, oldKeys, oldValues);
			else
				result = convertNewSerializationFormatStreamSerializer(oldSize, oldPageSize, oldParentRid, oldLeftRid, oldRightRid, oldColor, oldKeys, oldValues);
			return result;
		} finally {
			oldStream.close();
		}
