...
public class OSBTreeBucket<K, V> extends ODurablePage {
	...
	public OSBTreeBucket(OCacheEntry cacheEntry, boolean isLeaf, OBinarySerializer<K> keySerializer, OType[] keyTypes, OBinarySerializer<V> valueSerializer, OWALChangesTree changesTree) throws IOException {
	...
	public OSBTreeBucket(OCacheEntry cacheEntry, OBinarySerializer<K> keySerializer, OType[] keyTypes, OBinarySerializer<V> valueSerializer, OWALChangesTree changesTree) {
	...
	public long remove(int entryIndex) throws IOException {
		...
		if(isLeaf) {
			if(valueSerializer.isFixedLength()) {
				...
				if(!isLink)
					entrySize = keySize + getObjectSizeInDirectMemory(valueSerializer, entryPosition + keySize + OByteSerializer.BYTE_SIZE) + OByteSerializer.BYTE_SIZE;
				else {
