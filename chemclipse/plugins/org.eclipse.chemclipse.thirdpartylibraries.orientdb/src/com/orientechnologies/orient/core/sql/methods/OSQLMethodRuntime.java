	...
	public Object execute(final Object iThis, final OIdentifiable iCurrentRecord, final Object iCurrentResult, final OCommandContext iContext) {
		...
		if(configuredParameters != null) {
				...
				if(method.evaluateParameters()) {
					if(configuredParameters[i] instanceof OSQLFilterItemField) {
						...
						if(runtimeParameters[i] == null && iCurrentResult instanceof OIdentifiable)
							...
							runtimeParameters[i] = ((OSQLFilterItemField)configuredParameters[i]).getValue((OIdentifiable)iCurrentResult, iCurrentResult, iContext);
					} else if(configuredParameters[i] instanceof OSQLMethodRuntime)
						runtimeParameters[i] = ((OSQLMethodRuntime)configuredParameters[i]).execute(iThis, iCurrentRecord, iCurrentResult, iContext);
						...
						if(runtimeParameters[i] == null && iCurrentResult instanceof OIdentifiable)
							// LOOK INTO THE CURRENT RESULT
							runtimeParameters[i] = ((OSQLFilterItemVariable)configuredParameters[i]).getValue((OIdentifiable)iCurrentResult, iCurrentResult, iContext);
			...
			if(method.getMaxParams() == -1 || method.getMaxParams() > 0) {
				if(runtimeParameters.length < method.getMinParams() || (method.getMaxParams() > -1 && runtimeParameters.length > method.getMaxParams()))
					throw new OCommandExecutionException("Syntax error: function '" + method.getName() + "' needs " + (method.getMinParams() == method.getMaxParams() ? method.getMinParams() : method.getMinParams() + "-" + method.getMaxParams()) + " argument(s) while has been received " + runtimeParameters.length);
			}
		}
			...
			for(int i = 0; i < iParameters.length; ++i) {
				...
				if(iParameters[i] != null) {
					if(iParameters[i] instanceof String && !iParameters[i].toString().startsWith("[")) {
						...
						if(v == OSQLHelper.VALUE_NOT_PARSED || (v != null && OMultiValue.isMultiValue(v) && OMultiValue.getFirstValue(v) == OSQLHelper.VALUE_NOT_PARSED))
							continue;
