...
public class OStreamSerializerSBTreeIndexRIDContainer implements OStreamSerializer, OBinarySerializer<OIndexRIDContainer> {
	...
	public static final int SBTREE_CONTAINER_SIZE = 2 * OBooleanSerializer.BOOLEAN_SIZE + 2 * OLongSerializer.LONG_SIZE + OIntegerSerializer.INT_SIZE;
	...
	public OIndexRIDContainer deserializeNativeObject(byte[] stream, int offset) {
		...
		if(BOOLEAN_SERIALIZER.deserializeNative(stream, offset + EMBEDDED_OFFSET)) {
			...
			final OIndexRIDContainerSBTree underlying = new OIndexRIDContainerSBTree(fileId, rootPointer, durable, (OAbstractPaginatedStorage)db.getStorage());
			return new OIndexRIDContainer(fileId, underlying, durable);
		}
	}
