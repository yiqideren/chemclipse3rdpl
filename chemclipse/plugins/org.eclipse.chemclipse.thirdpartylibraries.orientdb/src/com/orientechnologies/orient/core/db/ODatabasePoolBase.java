...
public abstract class ODatabasePoolBase<DB extends ODatabaseInternal> extends Thread {
	...
	public ODatabasePoolBase<DB> setup(final int iMinSize, final int iMaxSize) {
		...
		if(dbPool == null)
			setup(iMinSize, iMaxSize, OGlobalConfiguration.DB_POOL_IDLE_TIMEOUT.getValueAsLong(), OGlobalConfiguration.DB_POOL_IDLE_CHECK_DELAY.getValueAsLong());
		return this;
	}

	public ODatabasePoolBase<DB> setup(final int iMinSize, final int iMaxSize, final long idleTimeout, final long timeBetweenEvictionRunsMillis) {
	 ...
	 * @return A pooled database instance
	 */
	public DB acquire(final String iName, final String iUserName, final String iUserPassword, final Map<String, Object> iOptionalParams) {
