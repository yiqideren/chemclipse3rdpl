...
public class OCommandExecutorSQLSelect extends OCommandExecutorSQLResultsetAbstract {
	...
	protected void searchInClasses() {
		...
		if(!searchForIndexes(cls)) {
			...
			final boolean browsingOrderAsc = !(orderedFields.size() == 1 && orderedFields.get(0).getKey().equalsIgnoreCase("@rid") && orderedFields.get(0).getValue().equalsIgnoreCase("DESC"));
		...
		if(projectionString.trim().length() > 0) {
			...
			for(String projectionItem : items) {
				...
				if(p.startsWith("FLATTEN(") || p.startsWith("EXPAND(")) {
					...
					projections = null;
					if(groupedResult == null && expandTarget instanceof OSQLFunctionRuntime && ((OSQLFunctionRuntime)expandTarget).aggregateResults())
						getProjectionGroup(null);
					continue;
				}
