...
public class ODatabaseImport extends ODatabaseImpExpAbstract {
	...
	public ODatabaseImport(final ODatabaseDocumentInternal database, final String iFileName, final OCommandOutputListener iListener) throws IOException {
	...
	public ODatabaseImport(final ODatabaseDocumentInternal database, final InputStream iStream, final OCommandOutputListener iListener) throws IOException {
	...
	private void removeDefaultNonSecurityClasses() {
		...
		for(OClass dbClass : classes) {
			...
			if(!className.equalsIgnoreCase(ORole.CLASS_NAME) && !className.equalsIgnoreCase(OUser.CLASS_NAME) && !className.equalsIgnoreCase(OIdentity.CLASS_NAME)) {
	...
	private void importSchema() throws IOException, ParseException {
		...
		int schemaVersion = jsonReader.readNext(OJSONReader.FIELD_ASSIGNMENT).checkContent("\"version\"").readNumber(OJSONReader.ANY_NUMBER, true);
		jsonReader.readNext(OJSONReader.COMMA_SEPARATOR);
