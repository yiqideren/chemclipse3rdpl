...
public class OSecurityShared implements OSecurity, OCloseable {
	...
	public boolean dropUser(final String iUserName) {
		...
		final Number removed = getDatabase().<OCommandRequest> command(new OCommandSQL("delete from OUser where name = '" + iUserName + "'")).execute();
	...
	public ORole getRole(final String roleName, final boolean allowRepair) {
		...
		List<ODocument> result = getDatabase().<OCommandRequest> command(new OSQLSynchQuery<ODocument>("select from ORole where name = '" + roleName + "' limit 1")).execute();
	...
	protected OUser getUser(final String iUserName, final boolean iAllowRepair) {
		...
		if(iUserName == null)
			return null;
		List<ODocument> result = getDatabase().<OCommandRequest> command(new OSQLSynchQuery<ODocument>("select from OUser where name = '" + iUserName + "' limit 1").setFetchPlan("roles:1")).execute();
		if(result != null && !result.isEmpty())
