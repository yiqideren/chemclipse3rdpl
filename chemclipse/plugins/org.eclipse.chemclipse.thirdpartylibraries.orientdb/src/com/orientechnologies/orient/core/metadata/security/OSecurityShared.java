...
public class OSecurityShared implements OSecurity, OCloseable {
	...
	private void createOrUpdateORestrictedClass(final ODatabaseDocument database) {
		...
		if(!restrictedClass.existsProperty(ALLOW_ALL_FIELD))
			((OClassImpl)restrictedClass).createProperty(ALLOW_ALL_FIELD, OType.LINKSET, database.getMetadata().getSchema().getClass(OIdentity.CLASS_NAME), checkData);
		if(!restrictedClass.existsProperty(ALLOW_READ_FIELD))
			((OClassImpl)restrictedClass).createProperty(ALLOW_READ_FIELD, OType.LINKSET, database.getMetadata().getSchema().getClass(OIdentity.CLASS_NAME), checkData);
		if(!restrictedClass.existsProperty(ALLOW_UPDATE_FIELD))
			((OClassImpl)restrictedClass).createProperty(ALLOW_UPDATE_FIELD, OType.LINKSET, database.getMetadata().getSchema().getClass(OIdentity.CLASS_NAME), checkData);
		if(!restrictedClass.existsProperty(ALLOW_DELETE_FIELD))
			((OClassImpl)restrictedClass).createProperty(ALLOW_DELETE_FIELD, OType.LINKSET, database.getMetadata().getSchema().getClass(OIdentity.CLASS_NAME), checkData);
	}
